...
O conjunto de todas as matrizes reais de dimensão $n\times p$,
...
é uma base ortogonal de um subespaço $S\subseteq\mathbb{R}^p$, então, para qualquer $\mathbf{x}\in S$,

$$
\mathbf{x}
=
\sum_{j=1}^{k}
\frac{\mathbf{b}_j^\top\mathbf{x}}{\mathbf{b}_j^\top\mathbf{b}_j}
\mathbf{b}_j.
$$
...
Se um espaço vetorial $V$ possui uma base finita, então todas as bases de $V$ são finitas e possuem a mesma quantidade de vetores.
:::
...
é uma base de $W$, então existem vetores $\mathbf{w}_1,\ldots,\mathbf{w}_r\in V$ tais que
...
Caso contrário, existe um vetor $\mathbf{w}_1\in V$ que não pertence ao subespaço gerado por $\mathcal{B}_W$. Assim,

$$
\{
\mathbf{v}_1,\ldots,\mathbf{v}_s,\mathbf{w}_1
\}
$$

é linearmente independente.

Se esse conjunto ainda não gera $V$, escolhe-se um vetor $\mathbf{w}_2\in V$ que não pertença ao subespaço gerado pelos vetores já selecionados. O novo conjunto continua linearmente independente.
...
de acordo com as dimensões dos espaços ambientes $\mathbb{R}^3$ e $\mathbb{R}^2$, respectivamente.
...
Esta seção generaliza o produto interno euclidiano por meio de matrizes positivas definidas. Os conceitos apresentados serão retomados no estudo da distância de Mahalanobis, de geometrias determinadas por matrizes de covariâncias e de problemas de mínimos quadrados ponderados.
:::
...
As seções de aprofundamento generalizam a geometria euclidiana por produtos internos induzidos por matrizes positivas definidas e desenvolvem a matriz de projeção residual, as equações normais, o caso de posto deficiente e a pseudoinversa. Esses resultados serão retomados quando sua utilização se tornar necessária nos capítulos posteriores.
