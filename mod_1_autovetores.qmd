# Autovalores, autovetores e decomposição espectral {#sec-cap-autovalores}

Uma transformação linear com domínio e contradomínio iguais é um operador linear. Em $\mathbb{R}^p$, considere o operador
...
Pela @prp-transformacao-bases-gerais, a representação matricial de um operador depende da base escolhida. Uma base formada por direções invariantes pode transformar essa representação em uma matriz diagonal.

Uma direção gerada por um vetor não nulo $\mathbf v$ é invariante quando $\mathbf A\mathbf v$ permanece na reta gerada por $\mathbf v$. Essa condição define autovalores, autovetores e autoespaços.

Quando existe uma base de autovetores, a matriz do operador é diagonal nessa base. Para matrizes simétricas reais, o Teorema Espectral garante uma base ortonormal de autovetores e uma diagonalização ortogonal. A decomposição espectral resultante sustenta a análise de formas quadráticas, positividade, elipsoides, funções de matrizes e problemas de otimização presentes em covariâncias, ACP, MANOVA e análise discriminante.
...
Salvo indicação contrária, as interpretações geométricas referem-se a espaços vetoriais reais. Autovalores e autovetores complexos aparecem apenas quando explicitamente indicados.
:::

## Direções invariantes, autovalores e autoespaços

Uma direção determinada por um vetor não nulo $\mathbf v$ é preservada por $\mathbf A$ quando a imagem $\mathbf A\mathbf v$ permanece na reta gerada por $\mathbf v$. Nesse caso, existe um escalar $\lambda$ tal que
...
A condição $\mathbf A\mathbf v=\lambda\mathbf v$ define autovalores e autovetores.
...
é denominada **equação característica** de $\mathbf{A}$. [@searleKhuri2017matrix]
:::
...
O polinômio característico de uma matriz de ordem $p$ possui grau $p$. Contando as raízes repetidas, ele possui $p$ raízes no conjunto dos números complexos. Uma matriz real pode ter autovalores reais, autovalores complexos ou ambos.
...
Pela @prp-transformacao-bases-gerais, a representação matricial de um operador depende das bases escolhidas. A diagonalização corresponde à escolha de uma base formada por autovetores.
...
Pela @prp-transformacao-bases-gerais,
...
A diagonalização pode falhar para matrizes quadradas arbitrárias: uma matriz pode não possuir autovalores reais ou autovetores linearmente independentes em número suficiente para formar uma base.

Pela @def-matriz-simetrica, uma matriz
...
Pelo @thm-teorema-espectral, no caso simétrico a matriz da base de autovetores pode ser escolhida ortogonal:
...
Como $\mathbf{Q}_{\mathbf{A}}^{-1}=\mathbf{Q}_{\mathbf{A}}^{\top}$, as coordenadas definidas na @def-coordenadas-base são
...
Quando um autovalor é repetido e seu autoespaço possui dimensão maior que 1, os autovetores individuais associados a ele não são únicos. O autoespaço, entretanto, é determinado pela matriz.
...
Quando o autoespaço possui dimensão maior que 1, o projetor representa todo o autoespaço e possui posto igual à sua dimensão.
...
listados com repetição quando necessário, então:
...
Considere a forma quadrática definida na @def-forma-quadratica,
...
em que $\left\langle\cdot,\cdot\right\rangle_{\mathbf A}$ é o produto interno induzido por $\mathbf A$, definido na @def-produto-interno-induzido.
:::
...
Quando o autoespaço associado a um autovalor $\lambda$ possui dimensão maior que 1, a quantidade intrínseca ao autoespaço é a soma dos pesos associados aos vetores de uma base ortonormal desse subespaço. Essa soma é igual à proporção do comprimento ao quadrado da projeção de $\mathbf{a}$ sobre $\mathcal{E}_{\lambda}$.
...
O máximo é atingido por todo vetor não nulo do autoespaço associado a $\lambda_1$, e o mínimo é atingido por todo vetor não nulo do autoespaço associado a $\lambda_p$. [@gentle2017matrix]
:::
...
Se o autoespaço associado ao autovalor extremo possui dimensão maior que 1, todo vetor não nulo desse autoespaço produz o mesmo valor máximo. Nesse caso, a solução é um subespaço, e não uma direção individual única.
...
O quociente de Rayleigh assume máximo igual a $4$ na direção de $\mathbf q_1$ e mínimo igual a $2$ na direção de $\mathbf q_2$, coincidindo com os autovalores extremos de $\mathbf A$.
:::
...
Em diferentes problemas da Estatística Multivariada, procura-se uma **direção** no espaço das variáveis que maximize ou minimize um critério escalar. Esse critério pode representar variabilidade, separação entre grupos ou associação entre conjuntos de variáveis.
...
#### Ponto estacionário sob uma restrição
...
Portanto, as direções estacionárias são precisamente as direções dos autovetores, e o valor da forma quadrática em cada direção estacionária é o autovalor correspondente.

Como $\mathbf A$ é simétrica, seus autovalores são reais. Além disso, pelo @thm-extremos-quociente-rayleigh, para todo vetor unitário $\mathbf a$,
...
Seja $\mathbf{A}\in\mathbb{R}^{p\times p}$ uma matriz simétrica e considere
...
O @thm-autovalores-otimizacao-restrita aplica-se diretamente a problemas em que o critério a otimizar é uma forma quadrática.
...
Pelo @thm-autovalores-otimizacao-restrita, a solução é um autovetor associado ao maior autovalor de $\boldsymbol{\Sigma}$. O maior autovalor é a maior variância possível entre combinações lineares unitárias, e o autovetor correspondente determina a direção de maior variabilidade.

A imposição sucessiva de restrições de ortogonalidade conduz aos autovetores associados aos autovalores seguintes.

Razões entre formas quadráticas aparecem, com critérios específicos, em outros problemas multivariados. Na análise discriminante, procura-se uma direção que produza grande separação entre grupos em relação à variabilidade dentro dos grupos. Na MANOVA, determinadas direções maximizam a variação associada à hipótese em relação à variação residual. Na correlação canônica, procuram-se combinações lineares de dois conjuntos de variáveis que apresentem associação máxima.
...
O problema de otimização anterior identifica uma direção associada ao maior autovalor de $\mathbf A$. As direções seguintes são obtidas maximizando a mesma forma quadrática sob restrições sucessivas de ortogonalidade em relação às direções já determinadas.
...
O máximo é atingido por $\mathbf q_k$ e, quando o autoespaço associado a $\lambda_k$ possui dimensão maior que $1$, por qualquer autovetor unitário desse autoespaço que satisfaça as restrições de ortogonalidade. [@horn2013matrix]
...
A caracterização variacional identifica os autovalores como valores extremos ou estacionários da forma quadrática sobre a esfera unitária e os autovetores como as direções em que esses valores são atingidos. Restrições sucessivas de ortogonalidade recuperam os autovalores ordenados e seus autoespaços.


## Funções espectrais de matrizes

A decomposição espectral define funções de matrizes simétricas pela aplicação de uma função escalar aos autovalores. A inversa, a raiz quadrada principal e a raiz quadrada inversa são casos dessa construção.
...
A raiz quadrada principal $\mathbf A^{1/2}$ é a única matriz simétrica semidefinida positiva cujo quadrado é $\mathbf A$.
...
Quando $\mathbf{A}=\boldsymbol{\Sigma}\succ0$ é uma matriz de covariâncias, a transformação espectral aplicada ao vetor centralizado é
...
No @sec-cov, essa transformação define o branqueamento simétrico; nas coordenadas branqueadas, a distância de Mahalanobis coincide com a distância euclidiana.

A mudança de métrica por uma raiz quadrada inversa também reduz problemas definidos por uma matriz positiva definida a problemas euclidianos equivalentes, princípio utilizado no problema de autovalores generalizados.

## Autovalores generalizados

Quando o critério de interesse compara duas formas quadráticas, em vez de uma única forma quadrática relativamente à norma euclidiana, obtém-se o problema de autovalores generalizados,
...
A equação generalizada reduz-se ao problema ordinário de autovalores quando

$$
\mathbf M=\mathbf I_p.
$$

Problemas desse tipo aparecem quando o critério é uma razão entre duas formas quadráticas; para $\mathbf A$ simétrica e $\mathbf M\succ0$, o problema generalizado pode ser reduzido a um problema espectral simétrico ordinário [@gentle2017matrix].
...
para uma matriz simétrica. Consequentemente, todos os autovalores generalizados do par $(\mathbf{A},\mathbf{M})$ são reais e o @thm-teorema-espectral aplica-se à matriz $\widetilde{\mathbf{A}}$.
...
Quando $\mathbf M$ é uma matriz de covariâncias positiva definida, a transformação $\mathbf M^{-1/2}$ coincide com a forma espectral do branqueamento apresentada no @sec-cov. No problema generalizado, $\mathbf M$ representa genericamente a matriz que define a geometria.
...
A @fig-multiplicidades compara o autoespaço bidimensional de $\mathbf A_1$ com o autoespaço unidimensional de $\mathbf A_2$.
...
### Núcleo e imagem na decomposição espectral

Para matrizes simétricas, o autoespaço associado ao autovalor zero descreve o núcleo, enquanto os autoespaços associados aos autovalores não nulos geram a imagem.
...
A pseudoinversa espectral estende a inversão às matrizes simétricas singulares sem inverter as direções pertencentes ao núcleo.
...
A pseudoinversa de Moore--Penrose para matrizes retangulares é construída por meio da decomposição em valores singulares no @sec-cap-svd.

### Diagonalização simultânea por congruência

A redução simultânea de duas formas quadráticas conecta o problema espectral generalizado às raízes características de uma matriz relativamente a outra [@anderson2003].
...
#### Continuação do exemplo: forma diagonal simultânea

Os autovetores generalizados normalizados do @exm-autovalores-generalizados formam as colunas da matriz da base
...
Subespaços invariantes aparecem na demonstração do Teorema Espectral para operadores autoadjuntos [@eaton2007multivariate].
...
O caso unidimensional estabelece a ligação entre subespaços invariantes e autovetores.
...
mostra que todo subespaço invariante unidimensional é gerado por um autovetor.

### Autovalores complexos de matrizes reais

Matrizes reais gerais podem possuir autovalores complexos; o caso simétrico real distingue-se por possuir espectro real [@gentle2017matrix].
...
Considerada como operador em $\mathbb C^2$, a mesma matriz possui autovalores e autovetores complexos. O espectro complexo explica a ausência de retas invariantes reais na rotação de $90^\circ$. Para matrizes simétricas reais, o @thm-teorema-espectral garante autovalores reais e uma base ortonormal de autovetores reais.

### Independência entre autovetores

Autovetores associados a autovalores distintos são linearmente independentes, propriedade que permite construir bases de autovetores quando existem autovalores suficientes [@searleKhuri2017matrix].
...
::: {.callout-note collapse="true" appearance="simple" title="Demonstração"}
A demonstração procede por indução sobre $r$.
...
### Critérios gerais de diagonalização

Para uma matriz geral, a existência de uma base completa de autovetores é equivalente à coincidência entre as multiplicidades algébrica e geométrica de cada autovalor, desde que o polinômio característico se decomponha no corpo considerado [@searleKhuri2017matrix].
...
A diagonalização permite calcular potências aplicando a potência correspondente aos autovalores [@gentle2017matrix].
...
Para matrizes quadradas arbitrárias, a diagonalização pode falhar. Pelo @thm-teorema-espectral, toda matriz simétrica real admite uma base ortonormal de autovetores; sua diagonalização é, portanto, ortogonal e preserva a geometria euclidiana.

### Potências inteiras de matrizes simétricas

No caso simétrico, a diagonalização ortogonal fornece a mesma construção com uma base ortonormal [@gentle2017matrix].
...
A potência $\mathbf A^2$ preserva os autoespaços de $\mathbf A$ e eleva ao quadrado os fatores de escala associados aos autovalores.
:::

### Exponencial e logaritmo matriciais

A definição geral de funções de matrizes diagonalizáveis permite aplicar uma função escalar ao espectro; em particular, a exponencial possui também representação por série de potências [@gentle2017matrix].
...
A @tbl-funcoes-espectrais resume a ação de cada função espectral sobre os autovalores e as condições adicionais de definição.
...
As funções espectrais preservam os autoespaços e substituem cada autovalor $\lambda$ pelo escalar $f(\lambda)$. Potências, inversas, raízes e pseudoinversas correspondem a escolhas específicas de $f$.

## Síntese do capítulo

Autovalores e autovetores identificam direções nas quais a ação de uma matriz quadrada reduz-se à multiplicação por um escalar. Os autoespaços reúnem todas as direções associadas ao mesmo autovalor.
...
Agrupando autovalores repetidos, a decomposição espectral pode ser escrita em termos dos projetores espectrais:
...
A decomposição por projetores mostra que uma matriz simétrica atua projetando o vetor sobre autoespaços ortogonais e aplicando a cada componente o autovalor correspondente.

Os autovalores determinam diretamente o traço, o determinante e o posto da matriz:
...
A definição espectral preserva os autoespaços e transforma cada autovalor $\lambda$ no escalar $f(\lambda)$. Potências, inversas, raízes quadradas, raízes quadradas inversas, pseudoinversas, exponenciais e logaritmos correspondem a escolhas específicas de $f$, sob as condições de definição de cada função.
...
A @tbl-sintese-espectral organiza os principais objetos espectrais, suas expressões e interpretações.
...
A decomposição espectral aplica-se diretamente a matrizes simétricas quadradas. Para matrizes retangulares ou não simétricas, a **Decomposição em Valores Singulares**, desenvolvida no @sec-cap-svd, relaciona a matriz aos espectros de $\mathbf{A}^{\top}\mathbf{A}$ e $\mathbf{A}\mathbf{A}^{\top}$ e conecta valores singulares, espaços fundamentais, posto e aproximações de baixa dimensão.