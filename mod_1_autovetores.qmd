...
Uma reta que passa pela origem e é gerada por um vetor não nulo $\mathbf{v}$ é o subespaço
...
Um mesmo autoespaço unidimensional contém infinitos autovetores associados ao mesmo autovalor, pois qualquer múltiplo não nulo de um autovetor permanece na mesma direção e satisfaz a mesma equação de autovalor.
...
Como a restrição impõe $\mathbf{a}\neq\mathbf{0}$, essa equação mostra que toda direção estacionária é um autovetor de $\mathbf{A}$ e que o multiplicador $\eta$ é o autovalor correspondente.
...
O máximo é atingido por qualquer autovetor unitário associado a $\lambda_k$ que satisfaça as restrições. [@horn2013matrix]
:::
...
Quando há autovalores repetidos, as direções sucessivas dentro do mesmo autoespaço não são individualmente determinadas. Qualquer base ortonormal desse autoespaço fornece o mesmo valor da função objetivo e descreve o mesmo subespaço ótimo. Nesse caso, o subespaço espectral é intrínseco, enquanto a escolha de vetores individuais dentro dele não é única.
...
Como $\mathbf{a}\neq\mathbf{0}$, o multiplicador $\eta$ é um autovalor generalizado e $\mathbf{a}$ é um autovetor generalizado correspondente.

Multiplicando a equação à esquerda por $\mathbf{a}^{\top}$,
...
Denotando o autovalor generalizado correspondente por $\lambda$, conclui-se que o valor da função objetivo em um autovetor generalizado normalizado é igual a $\lambda$.
