...
No Capítulo 8, a @prp-estrutura-matriz-covariancias estabeleceu que simetria e semidefinição positiva caracterizam as matrizes que podem ocorrer como matrizes de covariâncias. Na família normal multivariada, qualquer matriz com essa estrutura pode ser utilizada na construção de uma distribuição por meio de uma fatoração $\boldsymbol{\Sigma}=\mathbf A\mathbf A^\top$.
...
Como $\boldsymbol{\Sigma}$ é simétrica e positiva semidefinida, @thm-teorema-espectral e @prp-positividade-autovalores garantem que, se
...
sua decomposição espectral reduzida pode ser escrita como
...
de dimensão $r<p$. Nesse suporte, entretanto, a distribuição possui densidade relativamente à medida de volume $r$-dimensional. Sua expressão será apresentada após a densidade da normal multivariada não singular.
...
No caso singular de @def-normal-multivariada-singular, a distribuição não possui densidade em relação à medida de Lebesgue em $\mathbb R^p$, mas @prp-suporte-normal-singular mostra que seu suporte é
...
Pela representação espectral na @eq-elipsoide-coordenadas-espectrais, desenvolvida no Capítulo 8, essas superfícies são elipsoidais. Seus eixos e extensões são determinados pela estrutura espectral de $\boldsymbol{\Sigma}$. Não é necessário reconstruir aqui essa geometria; no modelo normal, o novo resultado é a correspondência
...
O Capítulo 8 definiu na @def-distancia-mahalanobis a distância de Mahalanobis como uma medida geométrica adaptada à estrutura de covariâncias. Quando o vetor segue uma distribuição normal multivariada não singular, a distância quadrática ao vetor de médias possui uma distribuição probabilística conhecida.
...
Os resultados desta seção mostram a passagem da geometria da distância de Mahalanobis e de seus elipsoides, desenvolvida na @def-distancia-mahalanobis e @eq-elipsoide-covariancia no Capítulo 8, para uma interpretação probabilística. Na família normal, a forma quadrática de Mahalanobis possui uma distribuição conhecida, e os elipsoides podem ser calibrados por probabilidades populacionais.
...
| Caso singular | o suporte é $\boldsymbol{\mu}+\mathcal C(\boldsymbol{\Sigma})$; a densidade é definida relativamente ao volume nesse subespaço |
...
possui distribuição $\chi_p^2$. Esse resultado atribui uma calibração probabilística aos elipsoides de Mahalanobis desenvolvidos geometricamente na @eq-elipsoide-covariancia no Capítulo 8. 

O Capítulo 8 já havia mostrado na @eq-esperanca-mahalanobis-quadratica, sem hipótese de normalidade, que a esperança dessa forma quadrática é $p$. A normalidade acrescenta a distribuição completa $\chi_p^2$, permitindo definir regiões de probabilidade por seus quantis.
