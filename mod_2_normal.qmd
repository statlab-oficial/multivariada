...
Na família normal multivariada, essa situação é diferente. O vetor de médias e a matriz de covariâncias determinam toda a distribuição. Além disso, combinações lineares, transformações afins, distribuições marginais e distribuições condicionais permanecem dentro da família normal.
...
A propriedade que caracteriza a distribuição normal multivariada é formulada por meio das combinações lineares das componentes.
...
Em uma distribuição geral, o vetor de médias e a matriz de covariâncias não determinam a distribuição conjunta. Antes de estabelecer o resultado correspondente para a família normal, é necessário apresentar o princípio que permite caracterizar uma distribuição multivariada por suas projeções unidimensionais.
...
A matriz $\boldsymbol{\Sigma}$ de uma distribuição normal multivariada pode ser apenas semidefinida positiva. Nesse caso, algumas combinações lineares possuem variância zero.
...
A definição por combinações lineares também abrange distribuições normais singulares. Uma função densidade em relação ao volume de dimensão $p$, entretanto, existe somente quando
...
A estabilidade da família normal sob transformações lineares e afins é uma de suas propriedades mais importantes. Ela permite obter distribuições de combinações lineares, subvectores, projeções e versões padronizadas sem realizar integrações diretamente na densidade conjunta.
...
Na família normal multivariada, a normalidade conjunta estabelece uma relação mais forte entre essas propriedades. A estrutura de covariâncias determina quando componentes ou blocos do vetor são independentes.
...
A distribuição condicional descreve o comportamento probabilístico de um bloco quando o valor do outro bloco é conhecido.
...
O Capítulo 8 apresentou a distância de Mahalanobis como medida geométrica adaptada à matriz de covariâncias. Para uma distribuição normal multivariada, essa distância também possui uma distribuição probabilística conhecida.
...
Quando as populações normais possuem uma matriz de covariâncias comum, as diferenças entre suas log-densidades são funções afins das observações. Quando as matrizes de covariâncias são distintas, essas diferenças são, em geral, funções quadráticas.

Essa distinção motiva regras lineares e quadráticas de classificação. A construção dessas regras exige ainda probabilidades dos grupos, possíveis custos de decisão e estimação dos parâmetros desconhecidos. Esses elementos serão desenvolvidos posteriormente na formulação da análise discriminante.
