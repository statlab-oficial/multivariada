# Vetores e representação dos dados multivariados {#sec-cap-vetores}
...
Adotam-se as seguintes convenções de notação:
...
Cada componente $x_j$ corresponde ao valor observado da variável $j$, para $j=1,\ldots,p$. [@anderson2003]
:::
...
Nesse exemplo, o vetor organiza as medidas observadas. A comparação geométrica de variáveis com unidades diferentes exige atenção às escalas.
:::
...
A representação direta de uma observação como vetor de $\mathbb{R}^p$ será utilizada para variáveis quantitativas, salvo indicação explícita em contrário. Variáveis categóricas e conjuntos de dados mistos requerem representações compatíveis com a natureza das variáveis e com a técnica utilizada.
...
As unidades observacionais são representadas como pontos de $\mathbb{R}^p$, com uma coordenada para cada uma das $p$ variáveis [@anderson2003]. A notação $\mathbb{R}^p$ identifica explicitamente o espaço ambiente dessa representação.
...
As linhas da matriz correspondem às observações, e as colunas correspondem às variáveis. Essa organização coincide com a estrutura usual de uma planilha ou de um banco de dados. [@gentle2017matrix]
...
### Representações por linhas e por colunas
...
Na representação por linhas, cada unidade observacional é representada por um ponto em $\mathbb{R}^p$, cujas coordenadas correspondem às $p$ variáveis.

Na representação por colunas, cada variável é representada por um vetor em $\mathbb{R}^n$, cujas componentes correspondem às $n$ unidades observacionais.
...
O produto interno associa dois vetores de $\mathbb{R}^p$ a um número real. A partir dessa operação, definem-se norma, ângulo, ortogonalidade e distância euclidiana. [@eaton2007multivariate]
...
A igualdade ocorre se, e somente se, um dos vetores é múltiplo escalar do outro. [@eaton2007multivariate]
:::
...
A distância euclidiana depende das unidades e das escalas das variáveis. Uma variável com valores numericamente maiores pode contribuir mais para a distância entre as observações. Por isso, comparações baseadas em distância podem requerer padronização ou outra geometria compatível com as escalas das variáveis.
:::
...
O resultado do produto externo é uma matriz. [@gentle2017matrix]
:::
...
Assim, o produto interno produz um escalar, enquanto o produto externo produz uma matriz.
...
Esses conceitos permitem interpretar os dados multivariados simultaneamente como vetores, matrizes e uma nuvem de pontos.
