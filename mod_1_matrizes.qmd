...
3. **Existência do elemento neutro**
   ...
   \mathbf{A}.
   $$
   O elemeno neuto de dimensão $n \times p$ é a a seguinte matriz nula:
...
5. Em particular, para vetores $\mathbf{x}\in\mathbb{R}^p$ e $\mathbf{y}\in\mathbb{R}^q$, $\left(
\mathbf{x}\mathbf{y}^{\top}\right)^{\top}=\mathbf{y}\mathbf{x}^{\top}$.
:::
...
Se $\mathbf{A}_{22}$ e $\mathbf{A}_{11\cdot2}$ são invertíveis, então

$$
\mathbf{A}^{-1}
=
\begin{bmatrix}
\mathbf{A}_{11\cdot2}^{-1}
&
-\mathbf{A}_{11\cdot2}^{-1}\mathbf{A}_{12}\mathbf{A}_{22}^{-1}\\[4pt]
-\mathbf{A}_{22}^{-1}\mathbf{A}_{21}\mathbf{A}_{11\cdot2}^{-1}
&
\mathbf{A}_{22}^{-1}
+
