# Álgebra matricial aplicada {#sec-cap-matrizes}
...
Este capítulo desenvolve as operações e propriedades necessárias para manipular matrizes. O produto matriz-vetor e o produto entre matrizes permitem combinar linhas e colunas de dimensões compatíveis, enquanto a transposição, a simetria e o traço descrevem propriedades estruturais importantes dessas operações. Nesse contexto, o produto externo é um caso particular do produto matricial.

Também serão estudadas a identidade, a inversa, o determinante, o posto e sua relação com sistemas lineares, além de classes particulares de matrizes e de formas quadráticas. Matrizes particionadas e o complemento de Schur fornecem a base algébrica para covariâncias por blocos e distribuições condicionais.
...
A adição de matrizes e a multiplicação por escalar preservam as dimensões das matrizes envolvidas. O produto matricial utiliza uma regra diferente e pode produzir uma matriz com dimensões distintas das matrizes multiplicadas.
...
cada linha de $\mathbf{A}$ pode ser associada a um produto interno com $\mathbf{x}$. O produto matriz-vetor reúne esses produtos internos em um vetor de $\mathbb{R}^n$.
...
Esses produtos aparecem em matrizes de Gram, produtos cruzados, covariâncias e na decomposição em valores singulares.
:::
...
Portanto, o traço permite representar a soma dos quadrados de todos os elementos de uma matriz. Essa expressão também aparece no estudo da norma de Frobenius, de matrizes de produtos cruzados e de decomposições matriciais.
...
O traço aparece na representação de somas de quadrados, produtos cruzados, variâncias totais, formas quadráticas e expressões de verossimilhança. Sua relação com os autovalores é estabelecida no capítulo sobre decomposição espectral.
...
O delta de Kronecker permite representar de forma compacta relações de ortogonalidade e ortonormalidade.
:::
...
O determinante associa uma matriz quadrada a um número real. Seu valor permite caracterizar a invertibilidade, e suas propriedades relacionam determinantes de produtos, transpostas e inversas.
...
O **determinante** de $\mathbf{A}$ é o escalar
...
em que $S_n$ é o conjunto das permutações de $\{1,\ldots,n\}$. Uma **inversão** de uma permutação $\sigma$ é um par de índices $i<j$ tal que $\sigma(i)>\sigma(j)$. Define-se $\operatorname{sgn}(\sigma)=1$ quando o número de inversões é par e $\operatorname{sgn}(\sigma)=-1$ quando é ímpar. [@gentle2017matrix]
...
Essa estrutura aparece nas decomposições matriciais, em que uma matriz pode ser representada como soma de parcelas de posto 1.
...
Algumas matrizes quadradas possuem estruturas que simplificam produtos e inversões. Entre elas estão as matrizes ortogonais e diagonais.
...
As matrizes diagonais aparecem naturalmente em decomposições matriciais.

## Formas quadráticas e positividade

As formas quadráticas associam um vetor a um escalar por meio de uma expressão do tipo $\mathbf{x}^{\top}\mathbf A\mathbf{x}$. Elas aparecem em somas de quadrados, variâncias, distâncias e densidades multivariadas e permitem relacionar propriedades de matrizes simétricas a propriedades escalares.
...
é denominada **forma quadrática** associada à matriz $\mathbf A$. [@searleKhuri2017matrix]
:::

A forma quadrática depende de um único vetor e, em geral, não é uma função linear desse vetor. Para qualquer $\alpha\in\mathbb R$,

$$
\begin{aligned}
Q(\alpha\mathbf{x})
&=
(\alpha\mathbf{x})^{\top}\mathbf A(\alpha\mathbf{x})\\
&=
\alpha^2\mathbf{x}^{\top}\mathbf A\mathbf{x}\\
&=
\alpha^2Q(\mathbf{x}).
\end{aligned}
$$

O produto interno usual corresponde ao caso $\mathbf A=\mathbf I_p$.

::: {#exm-produto-interno-forma-quadratica}
## Produto interno e norma ao quadrado

Considere $\mathbf A=\mathbf I_p$. Então,

$$
Q(\mathbf{x})
=
\mathbf{x}^{\top}\mathbf I_p\mathbf{x}
=
\mathbf{x}^{\top}\mathbf{x}
=
...
Portanto, a norma euclidiana ao quadrado é uma forma quadrática cuja matriz representante é $\mathbf I_p$.
:::
...
Os termos fora da diagonal aparecem combinados. No caso bidimensional, $b$ e $c$ contribuem somente por meio da soma
...
não é necessariamente única quando matrizes não simétricas são admitidas. Duas matrizes com a mesma parte simétrica produzem a mesma forma quadrática.
...
por satisfazer $\mathbf K^{\top}=-\mathbf K$, não altera o valor de $\mathbf{x}^{\top}\mathbf A\mathbf{x}$. Portanto, toda forma quadrática pode ser estudada por sua única matriz simétrica representante.
...
Com essas definições, toda matriz positiva definida é também semidefinida positiva, e toda matriz negativa definida é também semidefinida negativa. Quando for necessário excluir os casos definidos, essa restrição será indicada explicitamente.
...
As operações por blocos permitem manipular grupos de linhas e colunas como unidades matriciais. [@anderson2003]

O complemento de Schur utiliza essa estrutura para representar a parcela de um bloco ajustada pelo outro.
...
parte de $\mathbf{A}_{11}$ e retira a parcela associada às relações que passam pelo segundo bloco. A fatoração matricial que produz essa correção é apresentada na @sec-matrizes-eliminacao-blocos.
...
Para matrizes simétricas, o complemento de Schur permite verificar a definição positiva de uma matriz particionada por meio de matrizes de menor dimensão.
...
é positiva definida. [@gentle2017matrix]
:::
...
representa a covariância residual do primeiro bloco após o ajuste linear pelo segundo bloco, como desenvolvido na @sec-cov. Sob normalidade conjunta, a mesma matriz é a covariância condicional do primeiro bloco dado o segundo, como apresentado na @sec-normal. [@anderson2003]

## Aprofundamento teórico {#sec-matrizes-aprofundamento-teorico}
...
é antissimétrica. [@searleKhuri2017matrix]
:::
...
Para uma matriz $\mathbf A=(a_{ij})_{n\times n}$, a expansão em cofatores de uma linha fixa $i$ é
...
e a expansão em cofatores de uma coluna fixa $j$ é
...
Cada cofator envolve o determinante de uma matriz de ordem $n-1$, de modo que a expansão pode ser aplicada recursivamente. [@gentle2017matrix]
...
A expansão por cofatores é útil para compreender a definição do determinante e realizar cálculos de pequena ordem. Para matrizes maiores, a expansão direta se torna pouco eficiente. Nesses casos, o determinante costuma ser obtido por eliminação ou por decomposições matriciais.
:::
...
A pseudoinversa estende a resolução de sistemas lineares a matrizes retangulares ou singulares. As condições de Moore--Penrose a caracterizam; a decomposição em valores singulares fornece sua construção no @sec-cap-svd.
...
As matrizes triangulares aparecem na resolução de sistemas e em decomposições matriciais.

### Eliminação por blocos {#sec-matrizes-eliminacao-blocos}

Considere a matriz particionada de @def-complemento-schur e suponha que $\mathbf{A}_{22}$ seja invertível. A eliminação do bloco $\mathbf{A}_{12}$ produz uma matriz equivalente cuja posição superior direita é ocupada por uma matriz nula.
...
-   calcular e interpretar formas quadráticas;
