# Decomposição em Valores Singulares (EM REVISÃO -  NÃO LER AINDA)
...
A matriz $\mathbf D\in\mathbb R^{q\times p}$ é denominada **diagonal retangular** porque seus elementos fora da diagonal principal são nulos.
...
e $d_1,\ldots,d_s$ denotam seus elementos diagonais, então, utilizando a delta de Kronecker definida anteriormente,
...
enquanto os elementos que não pertencem ao bloco diagonal de ordem $s$ são iguais a zero.
...
Assim, a delta de Kronecker expressa de forma compacta que apenas os elementos da diagonal principal podem ser diferentes de zero.
...
então $s=p$ e $\mathbf D$ possui $q-p$ linhas nulas abaixo de sua parte diagonal. Se

$$
p>q,
$$

então $s=q$ e $\mathbf D$ possui $p-q$ colunas nulas à direita de sua parte diagonal. Se
...
a matriz $\mathbf D$ possui, portanto, $s$ posições diagonais. Se
...
## Construção da SVD e relação com a deomposição espectral
