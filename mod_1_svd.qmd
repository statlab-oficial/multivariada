# Decomposição em Valores Singulares {#sec-cap-svd}

Pelo @thm-teorema-espectral, uma matriz simétrica real admite a decomposição espectral ortogonal na forma
...
Para uma matriz de dados, os mesmos valores singulares conectam direções da representação por linhas em $\mathbb R^p$ e da representação por colunas em $\mathbb R^n$.
...
Esses elementos são os **valores singulares** de $\mathbf{A}$. [@eaton2007multivariate; @searleKhuri2017matrix]
:::

A construção da SVD a partir das decomposições espectrais de $\mathbf{A}^{\top}\mathbf{A}$ e $\mathbf{A}\mathbf{A}^{\top}$ fornece a demonstração do teorema.
...
Esses produtos são os projetores ortogonais formalizados em @prp-projetores-espacos-svd.
...
A forma completa acrescenta bases ortonormais para os complementos ortogonais dos subespaços gerados pelas colunas de $\mathbf U_r$ e $\mathbf V_r$.
...
A matriz $\mathbf A_k$ possui posto $k$ e constitui uma aproximação de $\mathbf A$. Os erros do truncamento são caracterizados em @prp-erros-svd-truncada, e sua propriedade de otimalidade em @thm-eckart-young-mirsky.

A literatura apresenta formas equivalentes da SVD com fatores de dimensões diferentes. Neste capítulo, adotam-se as três convenções a seguir para distinguir a decomposição exata completa, sua forma exata reduzida aos valores singulares positivos e o truncamento aproximado [@gentle2017matrix; @searleKhuri2017matrix]:
...
Os valores numéricos estão arredondados a quatro casas decimais; por isso, as igualdades que utilizam esses valores estão sujeitas ao erro de arredondamento.
...
A SVD completa apresenta bases ortonormais completas dos espaços de origem e de chegada. A SVD compacta mantém somente as direções associadas aos valores singulares positivos e reconstrói exatamente $\mathbf A$. A SVD truncada conserva apenas parte dessas direções e, por isso, produz uma aproximação de posto menor.
:::
...
Os espectros de $\mathbf{A}^{\top}\mathbf{A}$ e $\mathbf{A}\mathbf{A}^{\top}$ estão diretamente relacionados: seus autovalores positivos coincidem, e os autovetores correspondentes determinam as direções singulares da transformação
...
obtêm-se $r$ números positivos associados aos autovalores positivos comuns às duas matrizes de Gram. A @prp-construcao-pares-singulares identifica esses números como os valores singulares positivos de $\mathbf A$.
:::
...
A @prp-construcao-svd-compacta fornece a parte não nula da decomposição; a SVD completa é obtida completando as bases ortonormais dos espaços envolvidos.
...
A matriz $\mathbf A^{\top}\mathbf A$ coincide com a matriz analisada em @exm-reta-invariante. Seus autovalores são
...
O maior valor singular determina a norma espectral em @def-norma-espectral, e os valores singulares extremos determinam o número de condição em @def-numero-condicao-svd.
...
A SVD identifica bases ortonormais dos quatro espaços fundamentais associados à matriz. Essas bases determinam o posto, os núcleos e os projetores ortogonais, enquanto os valores singulares determinam as normas espectral e de Frobenius.
...
Como são projetores ortogonais, essas matrizes são simétricas e idempotentes pela @prp-propriedades-matriz-projecao.
...
As dimensões $p-r$ e $q-r$ dos núcleos não coincidem, em geral, com o número de posições diagonais nulas de $\mathbf D$. Como
...
Pela @prp-extremos-comprimentos-svd, a norma espectral é o maior fator de ampliação produzido pela transformação.
...
Em uma matriz de posto máximo, os valores singulares extremos descrevem a maior e a menor escala ativa da transformação. Uma grande disparidade entre essas escalas indica que a inversão das direções mais contraídas pode amplificar pequenas perturbações.
...
Para matrizes de posto máximo, o número de condição compara o maior e o menor fatores positivos de escala associados à transformação.
...
**Núcleo não trivial**, **deficiência de posto** e **mau condicionamento** são propriedades distintas.
...
Essa amplificação explica por que posto completo e invertibilidade não garantem, por si sós, estabilidade numérica.
...
A SVD relaciona as representações por linhas e por colunas da matriz de dados:

- as colunas de $\mathbf{V}_r$ pertencem a $\mathbb{R}^{p}$ e determinam direções na representação das observações por linhas;
- as colunas de $\mathbf{U}_r$ pertencem a $\mathbb{R}^{n}$ e determinam direções na representação das variáveis por colunas.
...
Assim, o mesmo valor singular $d_j$ relaciona uma direção $\mathbf v_j$ em $\mathbb R^p$, associada à representação das observações por linhas, a uma direção $\mathbf u_j$ em $\mathbb R^n$, associada à representação das variáveis por colunas.
:::
...
expressam, assim, a dualidade entre a representação por linhas e a representação por colunas da matriz de dados.
...
- $\mathbf v_j\in\mathbb R^p$ determina uma direção associada à representação das observações por linhas;
- $\mathbf u_j\in\mathbb R^n$ determina uma direção associada à representação das variáveis por colunas.

As matrizes $\mathbf X^{\top}\mathbf X$ e $\mathbf X\mathbf X^{\top}$ possuem ordens $p$ e $n$, respectivamente. Se
...
Em ambos os casos, os autovalores positivos são os mesmos e correspondem aos quadrados dos valores singulares de $\mathbf X$.
...
Para uma matriz de dados centralizada $\mathbf X_c$, com $n\geq2$, considere
...
A SVD compacta de $\mathbf X_c$ determina diretamente a decomposição espectral de $\mathbf S$.
...
Portanto, a SVD da matriz centralizada determina diretamente a estrutura espectral da matriz de covariâncias amostral.
...
Por isso, a transformação por $\mathbf{V}_2$ representa apenas uma mudança ortogonal de coordenadas na representação das observações por linhas. Não ocorre redução exata da dimensão.
...
Na forma expandida da SVD, a contribuição de cada componente singular e o erro produzido pelo truncamento podem ser identificados separadamente.
...
Pela @def-svd-truncada, a aproximação de posto $k$ pode ser expressa diretamente a partir da forma expandida da SVD.
...
O erro do truncamento é determinado pelas parcelas singulares descartadas.
...
Para uma matriz genérica, $\eta_k$ mede exclusivamente a proporção de sua magnitude quadrática, segundo a norma de Frobenius, representada pelos primeiros $k$ componentes singulares. A razão $\eta_k$ não deve ser interpretada, em geral, como proporção de variância explicada.
...
A SVD truncada fornece uma aproximação ótima entre as matrizes de posto limitado.
...
A formulação da melhor aproximação de posto $k$ na norma de Frobenius é apresentada em @gentle2017matrix.
:::
...
Se $r=0$, então $\mathbf{A}=\mathbf{0}$ e define-se sua pseudoinversa como a matriz nula de dimensão $p\times q$.
...
o problema de mínimos quadrados consiste em determinar $\mathbf x$ que minimize a distância entre $\mathbf b$ e $\mathbf A\mathbf x$:
...
não existe solução exata, e o problema consiste em minimizar a distância de $\mathbf b$ ao espaço coluna de $\mathbf A$.
...
A pseudoinversa fornece a solução de mínimos quadrados de menor norma e, juntamente com o núcleo de $\mathbf A$, permite caracterizar todos os minimizadores.
...
Considere um valor singular positivo $d$ com multiplicidade $m$. Se
...
Pela invariância ortogonal, as normas espectral e de Frobenius de $\mathbf A$ coincidem com as de $\mathbf D$:
...
### Unicidade da aproximação {#sec-svd-unicidade-aproximacao}
...
Quando todos os $r$ componentes singulares positivos são mantidos, não há truncamento e a reconstrução é exata:
...
A pseudoinversa espectral definida em @def-pseudoinversa-espectral é obtida a partir da decomposição espectral de uma matriz simétrica.
...
### Caracterização de Moore--Penrose
...
[@searleKhuri2017matrix; @gentle2017matrix]
:::
...
A forma expandida da pseudoinversa permite expressar diretamente a solução de mínimos quadrados de menor norma nas direções singulares.
...
o núcleo de $\mathbf A$ é não trivial e existem outros minimizadores, obtidos pela adição de componentes pertencentes a
...
| Número de condição | $\kappa_2(\mathbf A)=d_1/d_s$ para posto máximo | Compara os fatores extremos de escala e caracteriza sensibilidade a perturbações |
...
- distinguir deficiência de posto, invertibilidade e mau condicionamento por meio do número de condição espectral;
...
Em problemas de otimização, estimação e transformação de variáveis, as estruturas matriciais desenvolvidas neste módulo são combinadas com gradientes, Hessianas, multiplicadores de Lagrange, diferenciais matriciais e integrais, tratados no @sec-cap-calculo.
