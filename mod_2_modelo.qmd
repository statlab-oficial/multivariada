...
contém os efeitos do modelo sobre as $s$ combinações de respostas definidas por $\mathbf C_2$.
...
Portanto, $\mathbf C_1$ atua sobre os termos do modelo e $\mathbf C_2$ atua sobre as respostas.
...
As linhas de $\boldsymbol{\Theta}$ correspondem às combinações dos termos definidas por $\mathbf C_1$, enquanto suas colunas correspondem às combinações das respostas definidas por $\mathbf C_2$.
...
No caso escalar, com uma combinação dos termos $\mathbf c_1$ e uma combinação das respostas $\mathbf c_2$,
...
Quando $\mathbf C_1$ não contém combinações redundantes, $\mathbf G_{C_1}$ é positiva definida e permite ponderar esse afastamento segundo a variabilidade decorrente do delineamento.
...
A formulação por $\mathbf C_1\mathbf B\mathbf C_2$ identifica a função paramétrica examinada; a formulação por modelos encaixados mostra quanto a inclusão dos termos correspondentes reduz a variação residual.
...
Esses casos mostram a função das duas matrizes. A matriz $\mathbf C_1$ combina termos do modelo, enquanto $\mathbf C_2$ combina respostas.
...
A matriz $\mathbf C_1$ combina termos do modelo e $\mathbf C_2$ combina respostas. A hipótese produz duas matrizes construídas sobre as mesmas combinações de respostas:
...
A primeira representa a variação associada ao afastamento da hipótese; a segunda representa a variação residual correspondente. Quando $\mathbf C_2=\mathbf I_p$, obtém-se o par
...
| hipótese geral | $\mathbf C_1\mathbf B\mathbf C_2=\boldsymbol{\Theta}_0$ | combina termos do modelo e respostas |
