...
::: {#prp-transformacao-normal-matricial}
...
::: {#prp-equacoes-normais-modelo-multivariado}
...
::: {#prp-decomposicao-criterio-minimos-quadrados}
...
::: {#prp-propriedades-projetores-modelo-residual}
...
::: {#prp-ortogonalidade-ajuste-residuo}
...
::: {#prp-ortogonalidade-residuos-delineamento}
...
Sob a estrutura de segunda ordem adotada neste capítulo, erros de unidades distintas são não correlacionados. Os resíduos, entretanto, podem apresentar covariâncias não nulas entre unidades distintas, pois são obtidos pela projeção conjunta das respostas por meio de $\mathbf M_Z$.
:::
...
::: {#prp-momentos-valores-ajustados-residuos}
...
::: {#prp-relacao-estimadores-covariancia-residual}
## Relação entre os estimadores de covariância residual
...
Depois da maximização em relação à estrutura de médias, a matriz
...
é a estimativa de máxima verossimilhança no espaço paramétrico
...
Quando $\mathbf Z$ possui posto deficiente, a matriz $\mathbf B$ não é identificada de forma única. Permanece identificada a matriz de médias condicionais $\mathbf Z\mathbf B$, bem como as funções lineares dos parâmetros que são invariantes entre todas as parametrizações equivalentes [@christensen2011linear].
...
$$
rp.
$$ {#eq-dimensao-espaco-funcoes-estimaveis}
...
::: {#prp-unicidade-estimativa-funcao-estimavel}
...
::: {#prp-condicao-restricoes-identificacao}
...
::: {.proof}
Primeiro, considere a existência. Como
...
Esta seção distingue deficiência de posto exata de quase dependência linear e discute como condicionamento, tolerâncias numéricas e decomposições matriciais afetam o cálculo dos estimadores. Esses aspectos são importantes para a estabilidade computacional, mas não são necessários para acompanhar a formulação das funções estimáveis e das hipóteses lineares desenvolvida no núcleo do capítulo.
:::
...
=
\frac{s_1}{s_r}.
$$ {#eq-numero-condicao-efetivo-matriz-delineamento}
...
\min
\left\{
\operatorname{posto}(\mathbf E),
\operatorname{posto}(\mathbf M)
\right\}.
$$
...
#### Ausência de efeito associado a um coeficiente
...
\frac{\mathbf E}{n}.
$$
Se o modelo normal matricial é não singular e
...
\mathbf H
\qquad
\text{e, quando as respostas são transformadas,}\qquad
\mathbf E_M.
$$
