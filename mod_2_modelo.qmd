...
em que $\mathcal I_k$ representa o conjunto dos índices das unidades pertencentes ao grupo $k$. Assim,
...
Quando $\mathbf C_1$ não contém combinações redundantes, $\mathbf V_{C_1}$ é positiva definida e permite ponderar esse afastamento segundo a variabilidade decorrente do delineamento.
...
Várias linhas em $\mathbf C_1$ permitem representar simultaneamente diferentes contrastes entre grupos. Essa é a estrutura que será utilizada posteriormente na MANOVA.
...
examina o efeito definido por $\mathbf c_1$ sobre a combinação de respostas definida por $\mathbf c_2$.
