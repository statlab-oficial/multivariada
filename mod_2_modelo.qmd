...
No caso de grupos, essa estrutura relacionará a variação residual e a variação associada ao modelo às matrizes de dispersão intragrupos e entre grupos definidas em @def-sscp-intragrupos e @def-sscp-entre-grupos. Essa relação será reutilizada posteriormente na ANOVA/MANOVA e na Análise Discriminante.

As distribuições amostrais utilizadas na construção de testes e regiões de confiança serão desenvolvidas no Capítulo 12. Os critérios específicos da MANOVA e a formulação da Análise Discriminante pertencem ao Módulo 3.
...
- $\mathbf T$ continuará representando a SQPC total observada definida em @def-matriz-sscp-total; sua versão como estatística aleatória foi definida em @def-sscp-amostral-aleatoria;
...
Um termo pode representar, por exemplo, o intercepto, a pertença a um grupo ou tratamento, o valor de uma covariável quantitativa ou outro efeito especificado no modelo. A matriz de delineamento reúne essas informações para todas as unidades.
...
Sob o modelo linear normal multivariado, a log-verossimilhança para uma matriz observada de respostas $\mathbf Y$, a menos de termos que não dependem dos parâmetros, é
