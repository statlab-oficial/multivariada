...
O modelo reúne, em uma mesma formulação:
...
A primeira parcela representa os vetores de médias condicionais determinados pelo delineamento e pelos parâmetros. A segunda reúne os afastamentos aleatórios em relação a essas médias.
...
A distribuição normal matricial permite expressar diretamente uma distribuição normal para uma matriz aleatória.
...
Na parametrização geral, $\mathbf U$ e $\mathbf V$ não possuem escalas separadamente identificáveis, pois, para qualquer $c>0$,
...
se aproxima da matriz observada de respostas $\mathbf Y$.

Como todas as respostas utilizam a mesma matriz de delineamento, a estimação pode ser realizada simultaneamente. Cada coluna de $\mathbf B$ poderia ser obtida por um modelo linear separado, mas a formulação matricial preserva a dependência entre as respostas e prepara a estimação da matriz de covariâncias residual.
...
No modelo linear normal multivariado, a log-verossimilhança é
...
Os valores ajustados e os resíduos são obtidos pela decomposição de cada coluna da matriz de respostas em duas componentes ortogonais:

- uma componente pertencente ao espaço de médias representado pela matriz de delineamento;
- uma componente pertencente ao complemento ortogonal desse espaço.

A construção depende do espaço coluna de $\mathbf Z$, e não da parametrização particular utilizada para representar esse espaço. Por isso, os valores ajustados e os resíduos permanecem únicos mesmo quando a matriz de parâmetros não possui uma estimativa única.
...
O valor esperado da SSCP residual depende do posto do espaço de médias removido pelo ajuste.
...
O resultado anterior fornece diretamente um estimador não viesado da matriz de covariâncias residual.
...
Sob o modelo normal matricial, a função log-verossimilhança é
...
Nessa situação, o objeto diretamente identificado pelo modelo é o espaço de médias. A interpretação estatística deve concentrar-se nas combinações dos parâmetros que permanecem determinadas por esse espaço.
...
Quando $\mathbf B$ não é identificada integralmente, a análise deve ser formulada por meio de funções dos parâmetros que permanecem determinadas pela matriz de médias.
...
Essa igualdade exige que as linhas de $\mathbf C$ pertençam ao espaço linha de $\mathbf Z$.
...
A estrutura geral é construída por multiplicações à esquerda e à direita da matriz de parâmetros:

$$
\mathbf C\mathbf B\mathbf M.
$$

A matriz $\mathbf C$ combina os termos do modelo. A matriz $\mathbf M$ combina as respostas. Essa formulação inclui comparações entre grupos, testes de efeitos, tendências, diferenças entre respostas e hipóteses envolvendo várias combinações simultaneamente.
...
Considere o desvio observado em relação à hipótese:
...
Uma hipótese linear também pode ser representada pela comparação entre dois espaços de médias.
