# Cálculo Diferencial, Integral e Matricial Aplicado à Estatística Multivariada {#sec-cap-calculo}
...
O cálculo integral representa probabilidades, esperanças, distribuições marginais e transformações de vetores aleatórios. Nessas transformações, o determinante Jacobiano descreve a alteração local de volume.
...
Neste capítulo, salvo indicação em contrário, $\|\cdot\|$ aplicada a vetores denota a norma euclidiana definida em @def-norma-euclidiana.
...
\|\mathbf{X}\|_F^2,
$$
em que $\|\cdot\|_F$ denota a norma de Frobenius definida em @def-norma-frobenius.
...
O gradiente matricial é caracterizado pela relação
...
<<<<<<< HEAD
A expressão $2\mathbf{A}^{\top}(\mathbf{A}\mathbf{x}-\mathbf{b})$ é o gradiente do critério de mínimos quadrados desenvolvido em @prp-equacoes-normais-calculo.
=======
A expressão $2\mathbf{A}^{\top}(\mathbf{A}\mathbf{x}-\mathbf{b})$
é o gradiente do critério de mínimos quadrados desenvolvido em
@prp-equacoes-normais-calculo.
>>>>>>> 8fc317f2d585b3cda4f4a6d2c003bd1bdaf48a9e
:::
...
A curvatura da função é determinada pela matriz $\mathbf{A}$. A Hessiana intervém tanto na aproximação de segunda ordem quanto nos critérios de convexidade e de classificação local de pontos estacionários.
...
A inexistência de mínimos locais não globais diferencia a otimização convexa da otimização geral.
...
A @prp-condicao-primeira-ordem-extremo-local estabelece a condição necessária de primeira ordem para extremos locais interiores, enquanto o @thm-classificacao-hessiana utiliza a Hessiana para classificar pontos estacionários quando o teste de segunda ordem é conclusivo.

No caso convexo, a @prp-convexidade-minimizacao mostra que um ponto estacionário é minimizador global. A condição de primeira ordem, o teste da Hessiana e a convexidade fornecem critérios distintos de otimalidade para problemas sem restrições.
...
Pelo @thm-ortogonalidade-espacos-fundamentais,
...
O resultado também coincide com o caso de igualdade no @thm-cauchy-schwarz, pois
...
As formas quadráticas foram definidas em @def-forma-quadratica, e o @prp-parte-simetrica-forma-quadratica mostrou que seu valor depende apenas da parte simétrica da matriz. Suas derivadas dependem dessa mesma parte simétrica.
...
Utilizando a classificação de positividade apresentada em @def-classificacao-positividade, se
...
Retomando a norma euclidiana definida em @def-norma-euclidiana,
...
As equações normais já foram introduzidas geometricamente em @def-equacoes-normais, a partir da projeção sobre o espaço coluna. No Capítulo 6, @prp-minimos-quadrados-pseudoinversa apresentou a solução por meio da SVD e da pseudoinversa. A condição de primeira ordem do cálculo diferencial produz a mesma caracterização dos mínimos quadrados.

### Equações normais

::: {#prp-equacoes-normais-calculo}
## Caracterização dos mínimos quadrados pelas equações normais
...
[@gentle2017matrix; @eaton2007multivariate]
:::
...
Logo, $f$ é convexa. Pelo @prp-convexidade-minimizacao, um ponto é minimizador global se, e somente se, seu gradiente é nulo. Portanto,
...
O caso de posto deficiente, o conjunto de todos os minimizadores e a solução de menor norma foram estabelecidos em @prp-minimos-quadrados-pseudoinversa. As equações normais também surgem diretamente da condição de primeira ordem do problema de otimização.
...
Uma função escalar pode depender de todos os elementos de uma matriz. A diferenciação elemento a elemento é possível, mas se torna extensa quando a dimensão aumenta. Os diferenciais matriciais e as identidades de traço permitem organizar essas derivadas preservando a estrutura dos produtos. [@gentle2017matrix; @searleKhuri2017matrix]
...
Utilizando o produto interno de Frobenius definido em @def-produto-interno-frobenius,
...
A identidade permite reconhecer o gradiente após a reorganização do diferencial. 
...
O traço foi definido em @def-traco, e suas propriedades algébricas foram estabelecidas em @prp-propriedades-basicas-traco e @prp-traco-produtos-matriciais. No cálculo matricial, as permutações cíclicas dos fatores são especialmente úteis:
...
A reorganização de diferenciais também utiliza a relação
...
Essas transformações reescrevem o diferencial na forma
...
A matriz inversa e o determinante foram definidos em @def-matriz-inversa e @def-determinante, enquanto a relação entre determinante e invertibilidade foi estabelecida em @thm-determinante-invertibilidade. A classificação das matrizes simétricas por positividade foi apresentada em @def-classificacao-positividade.

Quando a matriz inversa e o determinante são tratados como funções de uma matriz variável, suas derivadas exigem condições de invertibilidade e, em aplicações com matrizes de covariâncias, definição positiva.
...
[@magnus2019matrix; @searleKhuri2017matrix]
:::
...
\mathbf{X}^{-\top}.
$$
O gradiente possui a mesma dimensão de $\mathbf{X}$,
...
A vetorização transforma uma matriz em um vetor. Essa operação permite representar objetos matriciais por vetores e relacionar transformações matriciais com operações usuais da álgebra linear.
=======
## Vetorização e produto de Kronecker

A vetorização transforma uma matriz em um vetor. O produto de Kronecker permite representar estruturas matriciais por meio de operações entre vetores e matrizes.
>>>>>>> 8fc317f2d585b3cda4f4a6d2c003bd1bdaf48a9e
...
[@searleKhuri2017matrix; @fujikoshi2010multivariate]
:::
...
[@eaton2007multivariate; @fujikoshi2010multivariate]
:::
...
Para vetores aleatórios contínuos, as integrais são realizadas sobre subconjuntos de $\mathbb{R}^{p}$.
=======
## Integração múltipla e mudança de variáveis

O cálculo integral permite obter probabilidades, esperanças, momentos e distribuições marginais de vetores aleatórios contínuos. A mudança de variáveis permite, adicionalmente, transformar regiões de integração e determinar densidades após transformações do vetor aleatório. [@apostol1969calculus2; @eaton2007multivariate]

Para vetores aleatórios contínuos, as integrais são realizadas sobre subconjuntos de $\mathbb{R}^{p}$. Integrais múltiplas, marginalização, esperanças e mudança de variáveis descrevem probabilidades e transformações de distribuições contínuas.
>>>>>>> 8fc317f2d585b3cda4f4a6d2c003bd1bdaf48a9e
...
O vetor aleatório $\boldsymbol{\mathsf X}$ é absolutamente contínuo em relação à medida de Lebesgue em $\mathbb{R}^p$ quando existe uma função mensurável
...
::: {.callout-note collapse="true" appearance="simple" title="Demonstração"}
Como a densidade conjunta é não negativa, o @thm-tonelli permite integrar sucessivamente suas componentes.
...
Considere um vetor aleatório absolutamente contínuo $\boldsymbol{\mathsf X}$ com densidade $f_{\boldsymbol{\mathsf X}}$, tal que
...
satisfazendo as condições do @thm-mudanca-variaveis-integral.
...
A formulação considera apenas transformações que admitem uma inversa única na região considerada.
...
A transformação converte esferas no espaço de $\mathbf{z}$ em elipsoides no espaço de $\mathbf{x}$, relacionando a mudança de variáveis às formas quadráticas que aparecem em densidades multivariadas.


Integrais múltiplas calculam probabilidades e esperanças, a marginalização produz densidades marginais e a mudança de variáveis determina como integrais e densidades se transformam. A diferenciação de integrais parametrizadas exige condições adicionais de regularidade.
...
A identidade geral de vetorização, o vetor escore, a informação observada e a informação de Fisher aparecem em desenvolvimentos inferenciais baseados em representações vetorizadas e funções de verossimilhança.

A interpretação inferencial desses objetos requer condições de regularidade e resultados probabilísticos sobre estimadores e distribuições assintóticas.
:::
...
Suponha que o vetor escore possua segundo momento finito. A informação de Fisher da amostra é definida por
...
Derivadas de funções vetoriais e matriciais, condições de otimização, diferenciais matriciais, integrais múltiplas e mudanças de variáveis compõem as ferramentas de cálculo utilizadas na Estatística Multivariada.
...
Na otimização sem restrições, o gradiente identifica pontos estacionários e a definitude da Hessiana caracteriza extremos locais, enquanto a convexidade estabelece propriedades globais. Em problemas com restrições de igualdade, os multiplicadores de Lagrange incorporam as restrições às condições de estacionariedade. Para funções de resíduos, a diferenciação conduz às equações normais e aos problemas de mínimos quadrados.

No cálculo matricial, os diferenciais e as identidades de traço organizam derivadas de funções de parâmetros matriciais. Derivadas de inversas, determinantes e log-determinantes aparecem em critérios definidos sobre matrizes de covariâncias. A vetorização e o produto de Kronecker representam transformações entre matrizes por meio de operações vetoriais.

O cálculo integral produz probabilidades, esperanças e densidades marginais. O Teorema da Mudança de Variáveis mostra como o determinante Jacobiano modifica elementos de volume e densidades sob transformações. Transformações afins e elipsoidais relacionam a mudança de variáveis às formas quadráticas e às matrizes positivas definidas.

As principais relações estabelecidas no capítulo são:

| Estrutura matemática | Aplicação principal |
|:---|:---|
...
: Relações entre ferramentas do cálculo e aplicações multivariadas. {#tbl-relacoes-calculo}
...
A identidade geral de vetorização conecta produtos matriciais a representações vetoriais, enquanto o vetor escore, a informação observada e a informação de Fisher conectam derivadas da log-verossimilhança a quantidades inferenciais.
...
- calcular probabilidades, esperanças e densidades marginais por integração;
- aplicar mudanças de variáveis e interpretar o determinante Jacobiano.

## Apêndice de cálculo para Estatística Multivariada {#sec-apendice-calculo-multivariado}

O apêndice reúne extensões de otimização, mínimos quadrados, cálculo matricial, integração e aproximação inferencial que não integram o núcleo do capítulo.
...
As condições de primeira ordem obtidas para restrições lineares também se aplicam a critérios quadráticos e a funções de resíduos.
...
retoma-se o produto interno induzido definido em @def-produto-interno-induzido:
...
o critério pode ser escrito utilizando a norma induzida de @def-norma-induzida:
...
Essa aproximação conduz ao método de Gauss--Newton.

Para funções cujos argumentos são matrizes, os diferenciais e as identidades de traço organizam as derivadas sem decompor a matriz em todos os seus elementos.
...
Os diferenciais matriciais e as identidades de traço também tratam funções em que a matriz variável aparece no interior de uma inversa ou de um determinante. Essas funções surgem naturalmente quando o parâmetro pertence ao conjunto das matrizes positivas definidas.
...
Os operadores de vetorização e meia vetorização fornecem uma parametrização explícita dos elementos distintos.
### Derivadas vetorizadas e parâmetros matriciais

::: {.callout-note appearance="simple" icon="false"}
## Leitura complementar

Quando parâmetros matriciais são tratados explicitamente como vetores, $\operatorname{vec}$, o produto de Kronecker e matrizes especiais representam Hessianas, transposições, parâmetros simétricos e Jacobianas matriciais.
:::
...
::: {#prp-convexidade-forma-quadratica-matricial}
## Convexidade da forma quadrática matricial
...
::: {#prp-gradiente-parametro-simetrico}
## Gradiente em relação aos elementos distintos de uma matriz simétrica
...
A vetorização e o produto de Kronecker representam derivadas matriciais por matrizes usuais. A forma vetorizada também descreve covariâncias de vetores matriciais, Hessianas e aproximações de funções de parâmetros matriciais.

Os operadores $\operatorname{vec}$, $\operatorname{vech}$ e as matrizes de comutação, duplicação e eliminação representam derivadas de ordem superior e parâmetros matriciais com restrições estruturais.
...
[@apostol1969calculus2; @eaton2007multivariate]
:::
...
Integrais múltiplas calculam probabilidades e esperanças, a marginalização produz densidades marginais e a mudança de variáveis determina como integrais e densidades se transformam.
...
A interpretação inferencial desses objetos requer condições de regularidade e resultados probabilísticos sobre estimadores.
:::
...
[@casella2002inference; @anderson2003]
:::
...
: Relações entre as ferramentas do cálculo e seus usos posteriores. {#tbl-relacoes-calculo}
...
A identidade geral de vetorização, o vetor escore, a informação observada e a informação de Fisher formam o aprofundamento teórico. O apêndice reúne extensões de otimização, cálculo matricial, integração e aproximação inferencial.
...
- utilizar diferenciais e identidades de traço em funções matriciais;
- diferenciar expressões envolvendo inversas, determinantes e log-determinantes;
- utilizar $\operatorname{vec}$ e o produto de Kronecker;
...
O método de Newton utiliza a aproximação quadrática local de $f$. [@gentle2017matrix]
...
Essa aproximação conduz ao método de Gauss--Newton. [@gentle2017matrix]
...
A ponderação das observações por $\boldsymbol{\Omega}$, por outro lado, permanece nas equações normais e pode modificar o estimador. [@anderson2003]
...
A parametrização polar não é uma transformação globalmente bijetiva em todas as escolhas possíveis de domínio: a origem possui representação angular não única e o ângulo é periódico. Para a aplicação da fórmula de mudança de variáveis, trabalha-se em regiões adequadas de injetividade; as fronteiras envolvidas possuem medida nula e não alteram o valor das integrais usuais. [@apostol1969calculus2]
...
A troca entre derivação e integração exige condições de regularidade, como suporte comum e dominação por uma função integrável; sem essas condições, a derivação sob o sinal de integração não é automaticamente válida.
:::
...
O @thm-convergencia-dominada permite trocar um limite e uma integral quando todas as funções da sequência são dominadas por uma mesma função integrável. A diferenciação sob o sinal de integração aplica esse teorema aos quocientes de diferenças.
...
O @thm-diferenciacao-integral pressupõe que o domínio $\mathcal{D}$ não depende de $\boldsymbol{\theta}$. Quando o domínio ou a fronteira da região de integração varia com o parâmetro, a derivação pode produzir termos adicionais que não estão cobertos por suas hipóteses.
...
A resolução desse sistema linear evita calcular explicitamente a inversa da informação observada.

O procedimento corresponde à aplicação do método de Newton à equação
...
Quando a identidade de informação é válida, a primeira representa a curvatura observada e a segunda sua correspondente curvatura esperada. [@anderson2003]
...
A expressão obtida é uma aproximação local de primeira ordem. Uma representação assintótica rigorosa exige controlar o termo restante da expansão e substituir a informação observada por um limite apropriado.

A aproximação anterior expressa o erro do estimador como o produto entre:
...
<<<<<<< HEAD
A demonstração formal combina a aproximação de Taylor de primeira ordem com resultados probabilísticos sobre convergência em distribuição. A expansão diferencial produz a matriz
=======
A demonstração formal utiliza a aproximação de Taylor de primeira ordem juntamente com resultados probabilísticos sobre convergência em distribuição. [@fujikoshi2010multivariate] Esses resultados serão desenvolvidos no módulo inferencial; neste capítulo interessa principalmente a origem diferencial da matriz
>>>>>>> 8fc317f2d585b3cda4f4a6d2c003bd1bdaf48a9e
...
Portanto, o vetor de médias amostrais é o único minimizador do critério. [@anderson2003]
:::

::: {#exm-criterio-modelo-linear-multivariado}
...
Para $\boldsymbol{\Sigma}$ fixa e positiva definida, o minimizador em relação a $\mathbf{B}$ não depende de $\boldsymbol{\Sigma}$, pois $\boldsymbol{\Sigma}^{-1}$ é invertível e desaparece da condição de primeira ordem.
...
O critério do modelo combina gradientes matriciais, log-determinante e diferenciação de funções envolvendo matrizes inversas.

A análise probabilística do critério exige propriedades dos estimadores, distribuições amostrais e formulações de hipóteses que não são estabelecidas neste apêndice.
:::

As principais correspondências entre objetos de cálculo e aplicações estatísticas são:
...
: Relações entre as ferramentas de cálculo e suas aplicações estatísticas. {#tbl-calculo-inferencia}

<<<<<<< HEAD
A interpretação estatística dessas correspondências exige hipóteses probabilísticas e resultados sobre estimação e distribuição amostral.
=======
...
Antes do início do Módulo II, os **Exercícios de fixação do Módulo I** integram os principais conceitos desenvolvidos ao longo dos sete capítulos. Os exercícios articulam representação matricial dos dados, projeções, transformações lineares, formas quadráticas, autovalores e autovetores, SVD, pseudoinversa e cálculo vetorial e matricial em problemas da Estatística Multivariada.

O capítulo seguinte inicia o **Módulo II — Fundamentos Probabilísticos e Inferenciais da Análise Multivariada** com o estudo das matrizes de covariâncias e correlações. Gradientes, formas quadráticas, transformações lineares, integrais, projeções e produtos matriciais compõem a base matemática para descrever a variabilidade conjunta de vetores aleatórios e desenvolver os resultados probabilísticos e inferenciais do módulo.