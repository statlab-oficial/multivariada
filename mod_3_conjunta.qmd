...
A @tbl-atributos-niveis-conjunta apresenta um exemplo abstrato de atributos e níveis utilizados na formação de perfis.
...
A equação @eq-modelo-linear-conjunta é a especialização, para uma única resposta, do modelo linear definido em @def-modelo-linear-multivariado.
...
A @tbl-codificacao-efeitos-conjunta apresenta uma possível codificação por efeitos para um atributo com três níveis.
...
o Teorema Posto--Nulidade de @thm-posto-nulidade mostra que essa condição equivale a
...
A pseudoinversa de Moore--Penrose, definida em @def-pseudoinversa-svd, permite selecionar uma solução de mínimos quadrados mesmo quando
...
Entretanto, @prp-solucoes-minimos-quadrados-posto-deficiente estabelece que, nesse caso, os coeficientes podem permanecer não únicos mesmo quando os valores ajustados são únicos. Portanto, a pseudoinversa não transforma uma parametrização não identificada em identificada.
...
Então, pela definição de @def-funcao-estimavel-modelo-multivariado e pela caracterização de @prp-caracterizacao-estimabilidade-modelo-multivariado, a função
...
O critério é a especialização para uma única resposta daquele definido em @def-criterio-minimos-quadrados-multivariado:
...
Sob as condições de @prp-momentos-estimador-matriz-parametros, sua especialização para uma única resposta fornece
...
Os valores ajustados foram definidos no modelo linear em @def-valores-ajustados-modelo-multivariado. No caso de uma única resposta,
...
é a especialização de @eq-valores-ajustados-observados-modelo.

Sob posto coluna completo, o projetor sobre o espaço do delineamento é dado por @eq-projetor-modelo-posto-completo. Portanto,
...
Pela propriedade de projeção sobre o espaço coluna estabelecida em @prp-projecao-espaco-coluna,
...
R_j
=
\max_{\ell\in\mathcal L_j}
u_j(\ell)
-
\min_{\ell\in\mathcal L_j}
u_j(\ell),
$$
...
A @tbl-sintese-modulo3 reúne essas estruturas.
