# Formulação da Análise Discriminante

A Análise Discriminante estuda problemas em que os grupos de interesse são previamente conhecidos e cada unidade é descrita por um vetor de características quantitativas. Diferentemente da Análise de Agrupamentos, os rótulos das observações utilizadas para caracterizar os grupos são conhecidos.

O problema possui objetivos relacionados, mas distintos:

- **discriminação:** identificar características ou combinações das variáveis que diferenciem os grupos;
- **representação discriminante:** construir coordenadas de menor dimensão nas quais a separação entre grupos possa ser descrita;
- **classificação:** construir uma regra capaz de atribuir uma nova observação a uma das classes conhecidas.

Esses objetivos conduzem a duas formulações complementares.

Na **formulação probabilística**, cada classe é descrita por uma distribuição condicional e a classificação é tratada como um problema de decisão. Probabilidades *a priori*, densidades condicionais e perdas determinam a regra ótima segundo o critério considerado.

Na **formulação geométrica de Fisher**, procuram-se combinações lineares das variáveis que apresentem grande separação entre os grupos relativamente à dispersão existente dentro deles [@anderson2003; @johnson2007; @rencher2012; @fisher1936].

A distinção entre as duas formulações deve ser preservada. Uma direção que separa grupos conhecidos não constitui, por si só, uma regra de classificação, e uma regra de classificação não precisa ser obtida por uma projeção de Fisher. Sob condições específicas, entretanto, as duas construções apresentam relações matemáticas diretas.

O objetivo deste capítulo é mostrar como:

- a teoria de decisão e o teorema de Bayes produzem regras de classificação;
- a distribuição normal multivariada e a distância de Mahalanobis conduzem às regras discriminantes linear e quadrática;
- a decomposição da dispersão entre e dentro dos grupos conduz ao critério de Fisher;
- formas quadráticas e autovalores generalizados determinam as direções discriminantes;
- a mesma estrutura matricial utilizada na MANOVA reaparece com outro objetivo estatístico.

Neste capítulo, os parâmetros populacionais, as quantidades amostrais e as regras estimadas serão distinguidos explicitamente. Algoritmos computacionais, regularização, seleção de variáveis, validação cruzada, taxas de erro e demais procedimentos de avaliação preditiva pertencem à etapa metodológica posterior.

Os elementos centrais são sintetizados na @tbl-estrutura-discriminante.

| Formulação | Objeto de partida | Problema | Resultado |
|---|---|---|---|
| decisão estatística | densidades condicionais, probabilidades *a priori* e perdas | escolher uma ação para uma nova observação | regra de Bayes |
| normal com covariância comum | $\boldsymbol{\mu}_g$, $\boldsymbol{\Sigma}$ e $\pi_g$ | minimizar risco sob perda zero--um | regra discriminante linear |
| normal com covariâncias distintas | $\boldsymbol{\mu}_g$, $\boldsymbol{\Sigma}_g$ e $\pi_g$ | minimizar risco sob perda zero--um | regra discriminante quadrática |
| geométrica de Fisher | dispersões intragrupos e entre grupos | maximizar separação relativa | direções e coordenadas discriminantes |

: Formulações complementares da Análise Discriminante. {#tbl-estrutura-discriminante}

## O problema de discriminação e classificação

Considere uma variável aleatória de classe

$$
\mathsf G
\in
\{1,\ldots,G\},
\qquad
G\geq2,
$$

e um vetor aleatório de características

$$
\boldsymbol{\mathsf X}
=
\begin{pmatrix}
\mathsf X_1\\
\vdots\\
\mathsf X_p
\end{pmatrix}
\in
\mathbb R^p.
$$

Para cada classe

$$
g\in\{1,\ldots,G\},
$$

existe uma distribuição condicional de

$$
\boldsymbol{\mathsf X}
\mid
\mathsf G=g.
$$

Uma realização

$$
\mathbf x
\in
\mathbb R^p
$$

representa os valores observados das características de uma unidade.

Durante a construção da técnica, os rótulos das unidades utilizadas para caracterizar os grupos são conhecidos. Por isso, a Análise Discriminante pertence aos problemas supervisionados.

Na classificação, procura-se uma regra

$$
\psi:
\mathbb R^p
\longrightarrow
\{1,\ldots,G\}
$$

que associe uma nova observação

$$
\mathbf x
$$

a uma das classes.

A qualidade preditiva de uma regra estimada constitui um problema adicional. Taxas de erro observadas, matrizes de confusão, validação cruzada e outros procedimentos de avaliação não serão confundidos com a formulação matemática da regra.

## Classificação como problema de decisão

A classificação pode ser formulada como um problema de decisão. No Módulo 2, funções de perda e risco foram introduzidas no contexto da estimação; aqui, esses conceitos são aplicados a ações de classificação e complementados por probabilidades *a priori*, probabilidades posteriores e pela regra de Bayes.

Para cada

$$
\mathbf x
\in
\mathbb R^p,
$$

deve-se escolher uma ação

$$
a
\in
\mathcal A,
\qquad
\mathcal A
=
\{1,\ldots,G\},
$$

em que a ação

$$
a=g
$$

significa classificar a observação na classe $g$.

A decisão depende de três objetos:

1. probabilidades *a priori* das classes;
2. distribuições condicionais das características;
3. perdas associadas às possíveis decisões.

### Probabilidades *a priori*

::: {#def-probabilidades-priori-discriminante}
## Probabilidades a priori das classes

Seja

$$
\mathsf G
\in
\{1,\ldots,G\}
$$

uma variável aleatória de classe.

As **probabilidades a priori** são

$$
\pi_g
=
P
\left(
\mathsf G=g
\right),
\qquad
g=1,\ldots,G,
$$

tais que

$$
\pi_g>0
$$

e

$$
\sum_{g=1}^{G}
\pi_g
=
1.
$$
:::

As probabilidades *a priori* descrevem a distribuição das classes antes da observação das características da nova unidade.

Elas são parâmetros do problema de decisão. Não devem ser confundidas automaticamente com proporções de grupos observadas em uma amostra. A relação entre probabilidades populacionais e estimativas amostrais será discutida posteriormente.

### Densidades condicionais e probabilidades posteriores

Suponha que, para cada classe $g$,

$$
\boldsymbol{\mathsf X}
\mid
\mathsf G=g
$$

possua densidade

$$
f_g(\mathbf x)
=
f
\left(
\mathbf x
\mid
\mathsf G=g
\right).
$$

Para uma observação $\mathbf x$ tal que

$$
\sum_{h=1}^{G}
\pi_h
f_h(\mathbf x)
>
0,
$$

o teorema de Bayes fornece

$$
P
\left(
\mathsf G=g
\mid
\boldsymbol{\mathsf X}=\mathbf x
\right)
=
\frac{
\pi_g f_g(\mathbf x)
}{
\displaystyle
\sum_{h=1}^{G}
\pi_h f_h(\mathbf x)
}.
$$ {#eq-posterior-classe-discriminante}

A probabilidade posterior combina dois tipos de informação.

O fator

$$
\pi_g
$$

representa a frequência probabilística da classe antes de observar $\mathbf x$.

O fator

$$
f_g(\mathbf x)
$$

mede a densidade atribuída à observação $\mathbf x$ pela distribuição condicional da classe.

A classificação, entretanto, não depende necessariamente apenas da classe de maior probabilidade posterior. As consequências das decisões também podem ser diferentes.

## Perda e risco de classificação

::: {#def-perda-classificacao-discriminante}
## Perda de classificação

Considere o conjunto de ações

$$
\mathcal A
=
\{1,\ldots,G\}
$$

e as classes verdadeiras

$$
g
\in
\{1,\ldots,G\}.
$$

Uma **função de perda de classificação** é uma função

$$
L:
\mathcal A
\times
\{1,\ldots,G\}
\longrightarrow
[0,\infty)
$$

tal que

$$
L(a,g)
$$

quantifica a consequência de escolher a ação $a$ quando a classe verdadeira é $g$.
:::

Os valores

$$
L(g,g)
$$

correspondem às decisões corretas, enquanto

$$
L(a,g),
\qquad
a\neq g,
$$

correspondem aos diferentes tipos de erro.

A teoria não exige que todos os erros possuam a mesma consequência.

::: {#def-risco-condicional-classificacao-discriminante}
## Risco condicional de classificação

Considere

$$
\mathsf G
\in
\{1,\ldots,G\},
$$

o conjunto de ações

$$
\mathcal A
=
\{1,\ldots,G\},
$$

uma função de perda

$$
L:
\mathcal A
\times
\{1,\ldots,G\}
\longrightarrow
[0,\infty)
$$

e uma observação

$$
\mathbf x
\in
\mathbb R^p
$$

para a qual as probabilidades posteriores

$$
P
\left(
\mathsf G=g
\mid
\boldsymbol{\mathsf X}=\mathbf x
\right)
$$

estão definidas.

O **risco condicional** de escolher

$$
a\in\mathcal A
$$

é

$$
R
\left(
a\mid\mathbf x
\right)
=
\sum_{g=1}^{G}
L(a,g)
P
\left(
\mathsf G=g
\mid
\boldsymbol{\mathsf X}=\mathbf x
\right).
$$ {#eq-risco-condicional-classificacao-discriminante}
:::

O risco é uma esperança condicional da perda: para uma observação fixa $\mathbf x$, cada possível consequência é ponderada pela probabilidade posterior da respectiva classe verdadeira.

### Regra de Bayes

::: {#def-regra-bayes-classificacao-discriminante}
## Regra de Bayes para classificação

Considere o conjunto de ações

$$
\mathcal A
=
\{1,\ldots,G\}
$$

e, para cada

$$
\mathbf x
\in
\mathbb R^p,
$$

riscos condicionais

$$
R
\left(
a\mid\mathbf x
\right),
\qquad
a\in\mathcal A.
$$

O conjunto das ações de Bayes é

$$
\mathcal A_{\mathrm B}
(\mathbf x)
=
\underset{
a\in\mathcal A
}{
\operatorname{arg\,min}
}
\;
R
\left(
a\mid\mathbf x
\right).
$$ {#eq-regra-bayes-risco-discriminante}

Uma **regra de Bayes** é qualquer função

$$
\psi_{\mathrm B}:
\mathbb R^p
\longrightarrow
\mathcal A
$$

tal que

$$
\psi_{\mathrm B}(\mathbf x)
\in
\mathcal A_{\mathrm B}(\mathbf x)
$$

para todo $\mathbf x$ em que os riscos estejam definidos.

Quando existem várias ações minimizadoras, uma convenção de desempate determina uma regra unívoca.
:::

A regra ótima depende conjuntamente:

- das distribuições condicionais;
- das probabilidades *a priori*;
- da função de perda.

Portanto, o problema de classificação não é determinado somente pela geometria dos grupos no espaço das características.

### Perda zero--um

Um caso particular é

$$
L(a,g)
=
\begin{cases}
0,
&
a=g,
\\[4pt]
1,
&
a\neq g.
\end{cases}
$$ {#eq-perda-zero-um-discriminante}

Nesse caso,

$$
\begin{aligned}
R
\left(
a\mid\mathbf x
\right)
&=
\sum_{g\neq a}
P
\left(
\mathsf G=g
\mid
\boldsymbol{\mathsf X}=\mathbf x
\right)
\\
&=
1
-
P
\left(
\mathsf G=a
\mid
\boldsymbol{\mathsf X}=\mathbf x
\right).
\end{aligned}
$$

Assim, minimizar o risco equivale a maximizar a probabilidade posterior.

O conjunto das classes ótimas é

$$
\underset{
g\in\{1,\ldots,G\}
}{
\operatorname{arg\,max}
}
\;
P
\left(
\mathsf G=g
\mid
\boldsymbol{\mathsf X}=\mathbf x
\right).
$$

Por @eq-posterior-classe-discriminante, o denominador é comum a todas as classes. Portanto,

$$
\psi_{\mathrm B}(\mathbf x)
\in
\underset{
g\in\{1,\ldots,G\}
}{
\operatorname{arg\,max}
}
\;
\pi_g f_g(\mathbf x).
$$ {#eq-regra-bayes-classificacao-discriminante}

Quando

$$
f_g(\mathbf x)>0
$$

para todas as classes, a mesma comparação pode ser realizada por

$$
\log\pi_g
+
\log f_g(\mathbf x),
$$ {#eq-escore-log-bayes-discriminante}

pois o logaritmo é estritamente crescente em

$$
(0,\infty).
$$

A regra de máxima probabilidade posterior é, portanto, um **caso particular** da regra de Bayes associado à perda zero--um.

Com perdas assimétricas, maximizar a posterior não é necessariamente ótimo. Nesse caso, a decisão continua sendo determinada por @eq-regra-bayes-risco-discriminante.

Essa distinção será importante posteriormente: probabilidades *a priori* e perdas pertencem ao problema de decisão, enquanto o critério geométrico de Fisher é construído a partir das dispersões entre e dentro dos grupos.

## Discriminação linear sob normalidade

Considere agora a especialização probabilística em que

$$
\boldsymbol{\mathsf X}
\mid
\mathsf G=g
\sim
N_p
\left(
\boldsymbol{\mu}_g,
\boldsymbol{\Sigma}
\right),
\qquad
g=1,\ldots,G,
$$ {#eq-modelo-normal-lda}

com

$$
\boldsymbol{\mu}_g
\in
\mathbb R^p
$$

e matriz de covariâncias comum

$$
\boldsymbol{\Sigma}
\in
\mathbb R^{p\times p},
\qquad
\boldsymbol{\Sigma}
\succ
\mathbf0.
$$

Além disso, considere probabilidades *a priori*

$$
\pi_g>0,
\qquad
\sum_{g=1}^{G}\pi_g=1.
$$

A normalidade não foi necessária para definir a regra de Bayes. Ela passa a ser utilizada agora para especificar explicitamente as densidades

$$
f_g(\mathbf x).
$$

O capítulo sobre a distribuição normal multivariada mostrou em @eq-log-razao-normal-covariancia-comum que a comparação entre duas log-densidades normais com a mesma matriz de covariâncias é afim em $\mathbf x$.

A Análise Discriminante utiliza esse resultado dentro do problema de decisão.

Sob perda zero--um, deve-se comparar

$$
\log\pi_g
+
\log f_g(\mathbf x).
$$

A log-densidade normal da classe $g$ contém

$$
-\frac12
\log
\left|
\boldsymbol{\Sigma}
\right|
-
\frac12
(\mathbf x-\boldsymbol{\mu}_g)^\top
\boldsymbol{\Sigma}^{-1}
(\mathbf x-\boldsymbol{\mu}_g),
$$

além do termo

$$
-\frac p2\log(2\pi).
$$

Como

$$
\boldsymbol{\Sigma}
$$

é comum a todas as classes,

$$
-\frac p2\log(2\pi)
$$

e

$$
-\frac12
\log
\left|
\boldsymbol{\Sigma}
\right|
$$

não participam da comparação entre grupos.

Expandindo a forma quadrática,

$$
\begin{aligned}
&
-\frac12
(\mathbf x-\boldsymbol{\mu}_g)^\top
\boldsymbol{\Sigma}^{-1}
(\mathbf x-\boldsymbol{\mu}_g)
\\
&=
-\frac12
\mathbf x^\top
\boldsymbol{\Sigma}^{-1}
\mathbf x
+
\mathbf x^\top
\boldsymbol{\Sigma}^{-1}
\boldsymbol{\mu}_g
-
\frac12
\boldsymbol{\mu}_g^\top
\boldsymbol{\Sigma}^{-1}
\boldsymbol{\mu}_g.
\end{aligned}
$$

O termo

$$
-\frac12
\mathbf x^\top
\boldsymbol{\Sigma}^{-1}
\mathbf x
$$

também é comum a todas as classes.

Restam apenas os termos que dependem de $g$.

::: {#def-funcao-discriminante-linear}
## Função discriminante linear

Considere

$$
G\geq2
$$

classes com

$$
\boldsymbol{\mathsf X}
\mid
\mathsf G=g
\sim
N_p
\left(
\boldsymbol{\mu}_g,
\boldsymbol{\Sigma}
\right),
\qquad
g=1,\ldots,G,
$$

em que

$$
\boldsymbol{\mu}_g
\in
\mathbb R^p,
\qquad
\boldsymbol{\Sigma}
\in
\mathbb R^{p\times p},
\qquad
\boldsymbol{\Sigma}\succ\mathbf0,
$$

e probabilidades *a priori*

$$
\pi_g>0,
\qquad
\sum_{g=1}^{G}\pi_g=1.
$$

Sob perda zero--um, a **função discriminante linear** da classe $g$ é

$$
\delta_g(\mathbf x)
=
\mathbf x^\top
\boldsymbol{\Sigma}^{-1}
\boldsymbol{\mu}_g
-
\frac12
\boldsymbol{\mu}_g^\top
\boldsymbol{\Sigma}^{-1}
\boldsymbol{\mu}_g
+
\log\pi_g.
$$ {#eq-funcao-discriminante-linear}
:::

A regra discriminante linear escolhe qualquer classe que maximize essa função:

$$
\psi_{\mathrm{LDA}}(\mathbf x)
\in
\underset{
g\in\{1,\ldots,G\}
}{
\operatorname{arg\,max}
}
\;
\delta_g(\mathbf x).
$$ {#eq-regra-classificacao-lda}

Quando o máximo é único, a classificação é determinada sem necessidade de convenção adicional de desempate.

A sigla LDA, de *Linear Discriminant Analysis*, é tradicional. Entretanto,

$$
\delta_g(\mathbf x)
$$

é, matematicamente, uma **função afim** de $\mathbf x$:

$$
\delta_g(\mathbf x)
=
\mathbf x^\top
\mathbf b_g
+
c_g,
$$

com

$$
\mathbf b_g
=
\boldsymbol{\Sigma}^{-1}
\boldsymbol{\mu}_g
$$

e

$$
c_g
=
-
\frac12
\boldsymbol{\mu}_g^\top
\boldsymbol{\Sigma}^{-1}
\boldsymbol{\mu}_g
+
\log\pi_g.
$$

A denominação “linear” refere-se convencionalmente à estrutura das fronteiras de decisão, que serão hiperplanos.

### Fronteiras entre classes

Considere duas classes distintas

$$
g\neq h.
$$

A fronteira de decisão entre elas é o conjunto de pontos para os quais

$$
\delta_g(\mathbf x)
=
\delta_h(\mathbf x).
$$

Substituindo @eq-funcao-discriminante-linear,

$$
\begin{aligned}
0
&=
\mathbf x^\top
\boldsymbol{\Sigma}^{-1}
\left(
\boldsymbol{\mu}_g
-
\boldsymbol{\mu}_h
\right)
\\
&\quad
-
\frac12
\left(
\boldsymbol{\mu}_g^\top
\boldsymbol{\Sigma}^{-1}
\boldsymbol{\mu}_g
-
\boldsymbol{\mu}_h^\top
\boldsymbol{\Sigma}^{-1}
\boldsymbol{\mu}_h
\right)
+
\log
\frac{\pi_g}{\pi_h}.
\end{aligned}
$$ {#eq-fronteira-linear-discriminante}

A equação possui a forma

$$
\mathbf x^\top\mathbf v
+
b
=
0,
$$

e define um hiperplano sempre que

$$
\mathbf v
=
\boldsymbol{\Sigma}^{-1}
\left(
\boldsymbol{\mu}_g
-
\boldsymbol{\mu}_h
\right)
\neq
\mathbf0.
$$

Se

$$
\boldsymbol{\mu}_g
\neq
\boldsymbol{\mu}_h,
$$

a positividade definida de

$$
\boldsymbol{\Sigma}
$$

garante

$$
\mathbf v\neq\mathbf0.
$$

O vetor normal ao hiperplano é, portanto,

$$
\boldsymbol{\Sigma}^{-1}
\left(
\boldsymbol{\mu}_g
-
\boldsymbol{\mu}_h
\right).
$$ {#eq-normal-fronteira-lda}

A diferença bruta entre os vetores de médias,

$$
\boldsymbol{\mu}_g
-
\boldsymbol{\mu}_h,
$$

não determina sozinha a orientação da fronteira.

Ela é transformada por

$$
\boldsymbol{\Sigma}^{-1},
$$

de modo que variâncias e covariâncias alteram a direção relevante para discriminar as classes.

As probabilidades *a priori* entram apenas pelo termo

$$
\log
\frac{\pi_g}{\pi_h}.
$$

Mantidos os vetores de médias e a matriz de covariâncias, alterar as probabilidades *a priori* desloca o hiperplano, mas não modifica seu vetor normal.

Há um caso degenerado importante. Se

$$
\boldsymbol{\mu}_g
=
\boldsymbol{\mu}_h,
$$

as duas classes possuem a mesma densidade condicional sob o modelo de covariância comum. A comparação entre elas é determinada somente pelas probabilidades *a priori*; nesse caso, não existe uma fronteira hiperplana não trivial produzida pelas características.

### Relação com a distância de Mahalanobis

A distância de Mahalanobis foi definida em @def-distancia-mahalanobis. Para a classe $g$,

$$
d_M^2
\left(
\mathbf x,
\boldsymbol{\mu}_g;
\boldsymbol{\Sigma}
\right)
=
(\mathbf x-\boldsymbol{\mu}_g)^\top
\boldsymbol{\Sigma}^{-1}
(\mathbf x-\boldsymbol{\mu}_g).
$$

Essa forma quadrática é exatamente a quantidade que aparece no expoente da densidade normal multivariada.

Sob o modelo em @eq-modelo-normal-lda e perda zero--um, a comparação entre classes pode ser escrita como

$$
-\frac12
d_M^2
\left(
\mathbf x,
\boldsymbol{\mu}_g;
\boldsymbol{\Sigma}
\right)
+
\log\pi_g.
$$ {#eq-lda-mahalanobis-priori}

Consequentemente,

$$
\psi_{\mathrm{LDA}}(\mathbf x)
\in
\underset{
g\in\{1,\ldots,G\}
}{
\operatorname{arg\,min}
}
\left\{
d_M^2
\left(
\mathbf x,
\boldsymbol{\mu}_g;
\boldsymbol{\Sigma}
\right)
-
2\log\pi_g
\right\}.
$$ {#eq-regra-lda-mahalanobis}

Essa forma evidencia dois componentes distintos da decisão:

- a posição de $\mathbf x$ relativamente ao centro da classe segundo a geometria de Mahalanobis;
- o peso probabilístico *a priori* atribuído à classe.

Se

$$
\pi_1
=
\cdots
=
\pi_G
=
\frac1G,
$$

o termo

$$
-2\log\pi_g
$$

é comum a todas as classes e pode ser retirado da comparação.

Assim,

$$
\psi_{\mathrm{LDA}}(\mathbf x)
\in
\underset{
g\in\{1,\ldots,G\}
}{
\operatorname{arg\,min}
}
\;
d_M^2
\left(
\mathbf x,
\boldsymbol{\mu}_g;
\boldsymbol{\Sigma}
\right).
$$ {#eq-lda-centroide-mahalanobis}

Sob normalidade, covariância comum, probabilidades *a priori* iguais e perda zero--um, a regra de Bayes classifica, portanto, pela menor distância de Mahalanobis ao centro populacional da classe.

A conexão com os fundamentos do Módulo 2 é direta. A mesma matriz

$$
\boldsymbol{\Sigma}
$$

que determina a forma dos elipsoides da distribuição normal determina também a métrica utilizada para comparar a nova observação com os centroides.

A classificação não utiliza uma proximidade euclidiana bruta. Ela utiliza uma geometria que incorpora simultaneamente:

- as escalas das variáveis;
- suas variâncias;
- suas covariâncias.

Essa interpretação não exige reconstruir a teoria da distância de Mahalanobis: ela mostra como uma estrutura geométrica já estabelecida passa a determinar uma regra probabilística de classificação.

A LDA apresentada até aqui é uma **regra populacional**: utiliza

$$
\boldsymbol{\mu}_1,\ldots,\boldsymbol{\mu}_G,
\qquad
\boldsymbol{\Sigma},
\qquad
\pi_1,\ldots,\pi_G,
$$

que são parâmetros populacionais.

Na prática, esses parâmetros geralmente não são conhecidos e precisam ser estimados a partir de observações com classes conhecidas. A distinção entre a regra populacional e sua versão estimada será estabelecida após a formulação da discriminação quadrática.

## Discriminação quadrática

A estrutura linear da seção anterior decorreu da hipótese de que todas as classes compartilham a mesma matriz de covariâncias.

Considere agora

$$
\boldsymbol{\mathsf X}
\mid
\mathsf G=g
\sim
N_p
\left(
\boldsymbol{\mu}_g,
\boldsymbol{\Sigma}_g
\right),
\qquad
g=1,\ldots,G,
$$ {#eq-modelo-normal-qda}

em que

$$
\boldsymbol{\mu}_g
\in
\mathbb R^p
$$

e

$$
\boldsymbol{\Sigma}_g
\in
\mathbb R^{p\times p},
\qquad
\boldsymbol{\Sigma}_g
\succ
\mathbf0.
$$

Considere também probabilidades *a priori*

$$
\pi_g>0,
\qquad
\sum_{g=1}^{G}\pi_g=1.
$$

A regra de Bayes continua sendo determinada pelas probabilidades posteriores e pela função de perda. A diferença em relação à LDA está no modelo utilizado para as distribuições condicionais.

Sob perda zero--um, deve-se comparar

$$
\log\pi_g
+
\log f_g(\mathbf x).
$$

Para a distribuição normal da classe $g$,

$$
\begin{aligned}
\log f_g(\mathbf x)
&=
-\frac p2
\log(2\pi)
-
\frac12
\log
\left|
\boldsymbol{\Sigma}_g
\right|
\\
&\quad
-
\frac12
(\mathbf x-\boldsymbol{\mu}_g)^\top
\boldsymbol{\Sigma}_g^{-1}
(\mathbf x-\boldsymbol{\mu}_g).
\end{aligned}
$$

O primeiro termo é comum a todas as classes, mas os demais dependem de $g$.

Diferentemente do que ocorreu em @eq-modelo-normal-lda, não existe agora uma única matriz de covariâncias que permita cancelar os termos quadráticos em $\mathbf x$.

Esse resultado já foi antecipado no capítulo da distribuição normal multivariada pela comparação de log-densidades com matrizes de covariâncias distintas.

::: {#def-funcao-discriminante-quadratica}
## Função discriminante quadrática

Considere

$$
G\geq2
$$

classes com

$$
\boldsymbol{\mathsf X}
\mid
\mathsf G=g
\sim
N_p
\left(
\boldsymbol{\mu}_g,
\boldsymbol{\Sigma}_g
\right),
\qquad
g=1,\ldots,G,
$$

em que

$$
\boldsymbol{\mu}_g
\in
\mathbb R^p,
$$

$$
\boldsymbol{\Sigma}_g
\in
\mathbb R^{p\times p},
\qquad
\boldsymbol{\Sigma}_g
\succ
\mathbf0,
$$

e probabilidades *a priori*

$$
\pi_g>0,
\qquad
\sum_{g=1}^{G}\pi_g=1.
$$

Sob perda zero--um, a **função discriminante quadrática** da classe $g$ é

$$
\delta_g^{Q}(\mathbf x)
=
-\frac12
\log
\left|
\boldsymbol{\Sigma}_g
\right|
-
\frac12
(\mathbf x-\boldsymbol{\mu}_g)^\top
\boldsymbol{\Sigma}_g^{-1}
(\mathbf x-\boldsymbol{\mu}_g)
+
\log\pi_g.
$$ {#eq-funcao-discriminante-quadratica}
:::

A regra discriminante quadrática escolhe qualquer classe que maximize o escore:

$$
\psi_{\mathrm{QDA}}(\mathbf x)
\in
\underset{
g\in\{1,\ldots,G\}
}{
\operatorname{arg\,max}
}
\;
\delta_g^{Q}(\mathbf x).
$$ {#eq-regra-classificacao-qda}

Quando o máximo é único, a classe fica determinada sem convenção adicional de desempate.

A denominação *Quadratic Discriminant Analysis* decorre da presença de uma forma quadrática específica para cada classe.

Expandindo-a,

$$
\begin{aligned}
&
-\frac12
(\mathbf x-\boldsymbol{\mu}_g)^\top
\boldsymbol{\Sigma}_g^{-1}
(\mathbf x-\boldsymbol{\mu}_g)
\\
&=
-\frac12
\mathbf x^\top
\boldsymbol{\Sigma}_g^{-1}
\mathbf x
+
\mathbf x^\top
\boldsymbol{\Sigma}_g^{-1}
\boldsymbol{\mu}_g
-
\frac12
\boldsymbol{\mu}_g^\top
\boldsymbol{\Sigma}_g^{-1}
\boldsymbol{\mu}_g.
\end{aligned}
$$

Como

$$
\boldsymbol{\Sigma}_g^{-1}
$$

depende da classe, o coeficiente do termo quadrático

$$
\mathbf x^\top
\boldsymbol{\Sigma}_g^{-1}
\mathbf x
$$

também depende de $g$.

Duas partes da função possuem papéis distintos.

A forma

$$
(\mathbf x-\boldsymbol{\mu}_g)^\top
\boldsymbol{\Sigma}_g^{-1}
(\mathbf x-\boldsymbol{\mu}_g)
$$

é a distância de Mahalanobis quadrática entre a observação e o centro da classe segundo a geometria própria daquela população.

Já o termo

$$
\log
\left|
\boldsymbol{\Sigma}_g
\right|
$$

é parte do fator de normalização da densidade normal e penaliza classes segundo sua dispersão multivariada.

Geometricamente,

$$
\left|
\boldsymbol{\Sigma}_g
\right|^{1/2}
$$

é proporcional ao fator de volume dos elipsoides definidos por uma mesma distância de Mahalanobis. Portanto, duas classes podem atribuir a um ponto a mesma distância de Mahalanobis e ainda produzir funções discriminantes distintas porque suas distribuições possuem diferentes concentrações no espaço.

### Fronteiras quadráticas

Considere duas classes

$$
g\neq h.
$$

Sua fronteira de decisão é o conjunto de pontos que satisfazem

$$
\delta_g^{Q}(\mathbf x)
=
\delta_h^{Q}(\mathbf x).
$$

Subtraindo as duas funções, obtém-se uma equação que contém o termo

$$
\frac12
\mathbf x^\top
\left(
\boldsymbol{\Sigma}_h^{-1}
-
\boldsymbol{\Sigma}_g^{-1}
\right)
\mathbf x.
$$

Além dele, aparecem termos lineares em $\mathbf x$ e termos constantes determinados pelos vetores de médias, determinantes das covariâncias e probabilidades *a priori*.

Assim, a fronteira pode ser escrita na forma geral

$$
\mathbf x^\top
\mathbf A_{gh}
\mathbf x
+
\mathbf b_{gh}^\top
\mathbf x
+
c_{gh}
=
0,
$$ {#eq-fronteira-quadratica-discriminante}

em que

$$
\mathbf A_{gh}
=
\frac12
\left(
\boldsymbol{\Sigma}_h^{-1}
-
\boldsymbol{\Sigma}_g^{-1}
\right).
$$

Em geral,

$$
\mathbf A_{gh}
\neq
\mathbf0,
$$

e a fronteira pertence à família das superfícies quadráticas.

Se, porém,

$$
\boldsymbol{\Sigma}_g
=
\boldsymbol{\Sigma}_h,
$$

então

$$
\mathbf A_{gh}
=
\mathbf0.
$$

Quando todas as classes compartilham

$$
\boldsymbol{\Sigma}_1
=
\cdots
=
\boldsymbol{\Sigma}_G
=
\boldsymbol{\Sigma},
$$

os termos quadráticos e os log-determinantes comuns cancelam-se para todas as comparações entre classes. A QDA reduz-se então à estrutura da LDA em @eq-funcao-discriminante-linear.

Portanto, a linearidade ou quadraticidade da fronteira não é uma escolha gráfica posterior. Ela decorre diretamente da estrutura probabilística atribuída às matrizes de covariâncias condicionais.

A @fig-fronteiras-lda-qda ilustra essa diferença para duas classes normais. No primeiro painel, a mesma matriz de covariâncias é utilizada para ambas as populações e a fronteira é uma reta. No segundo, cada população possui sua própria matriz de covariâncias e a fronteira pode ser curva.

```{r}
#| label: fig-fronteiras-lda-qda
#| echo: false
#| warning: false
#| message: false
#| fig-cap: "Fronteiras de decisão para duas populações normais. As elipses delimitam regiões de concentração de 75%. Com covariância comum, a LDA produz uma fronteira linear; com covariâncias distintas, a QDA pode produzir uma fronteira quadrática."
#| fig-width: 10
#| fig-height: 5
#| fig-align: center

library(ggplot2)

delta_lda <- function(X, mu, Sigma, prior = 0.5) {
  Sigma_inv <- solve(Sigma)

  as.vector(
    X %*% Sigma_inv %*% mu
  ) -
    0.5 *
    as.numeric(
      t(mu) %*% Sigma_inv %*% mu
    ) +
    log(prior)
}

delta_qda <- function(X, mu, Sigma, prior = 0.5) {
  Sigma_inv <- solve(Sigma)

  D <- sweep(
    X,
    2,
    mu,
    "-"
  )

  quadratica <- rowSums(
    (D %*% Sigma_inv) * D
  )

  -0.5 * log(det(Sigma)) -
    0.5 * quadratica +
    log(prior)
}

elipse_normal <- function(
  mu,
  Sigma,
  classe,
  cenario,
  prob = 0.75,
  n = 250
) {
  decomposicao <- eigen(
    Sigma,
    symmetric = TRUE
  )

  angulo <- seq(
    0,
    2 * pi,
    length.out = n
  )

  circulo <- cbind(
    cos(angulo),
    sin(angulo)
  )

  raio <- sqrt(
    qchisq(
      prob,
      df = 2
    )
  )

  transformacao <- decomposicao$vectors %*%
    diag(
      sqrt(
        decomposicao$values
      )
    )

  pontos <- raio *
    circulo %*%
    t(transformacao)

  pontos <- sweep(
    pontos,
    2,
    mu,
    "+"
  )

  data.frame(
    x1 = pontos[, 1],
    x2 = pontos[, 2],
    classe = classe,
    cenario = cenario
  )
}

mu_1 <- c(
  -1.4,
  -0.3
)

mu_2 <- c(
  1.4,
  0.3
)

Sigma_comum <- matrix(
  c(
    1.0, 0.55,
    0.55, 0.8
  ),
  nrow = 2,
  byrow = TRUE
)

Sigma_1 <- matrix(
  c(
    1.5, 0.7,
    0.7, 0.6
  ),
  nrow = 2,
  byrow = TRUE
)

Sigma_2 <- matrix(
  c(
    0.5, -0.15,
    -0.15, 1.3
  ),
  nrow = 2,
  byrow = TRUE
)

grade_base <- expand.grid(
  x1 = seq(
    -4,
    4,
    length.out = 220
  ),
  x2 = seq(
    -3.5,
    3.5,
    length.out = 220
  )
)

X_grade <- as.matrix(
  grade_base[
    c(
      "x1",
      "x2"
    )
  ]
)

grade_lda <- grade_base

grade_lda$diferenca <-
  delta_lda(
    X_grade,
    mu_1,
    Sigma_comum
  ) -
  delta_lda(
    X_grade,
    mu_2,
    Sigma_comum
  )

grade_lda$cenario <-
  "LDA: covariância comum"

grade_qda <- grade_base

grade_qda$diferenca <-
  delta_qda(
    X_grade,
    mu_1,
    Sigma_1
  ) -
  delta_qda(
    X_grade,
    mu_2,
    Sigma_2
  )

grade_qda$cenario <-
  "QDA: covariâncias distintas"

grade <- rbind(
  grade_lda,
  grade_qda
)

elipses <- rbind(
  elipse_normal(
    mu_1,
    Sigma_comum,
    "Classe 1",
    "LDA: covariância comum"
  ),
  elipse_normal(
    mu_2,
    Sigma_comum,
    "Classe 2",
    "LDA: covariância comum"
  ),
  elipse_normal(
    mu_1,
    Sigma_1,
    "Classe 1",
    "QDA: covariâncias distintas"
  ),
  elipse_normal(
    mu_2,
    Sigma_2,
    "Classe 2",
    "QDA: covariâncias distintas"
  )
)

centros <- rbind(
  data.frame(
    x1 = c(
      mu_1[1],
      mu_2[1]
    ),
    x2 = c(
      mu_1[2],
      mu_2[2]
    ),
    classe = c(
      "Classe 1",
      "Classe 2"
    ),
    cenario =
      "LDA: covariância comum"
  ),
  data.frame(
    x1 = c(
      mu_1[1],
      mu_2[1]
    ),
    x2 = c(
      mu_1[2],
      mu_2[2]
    ),
    classe = c(
      "Classe 1",
      "Classe 2"
    ),
    cenario =
      "QDA: covariâncias distintas"
  )
)

ggplot() +
  geom_path(
    data = elipses,
    aes(
      x = x1,
      y = x2,
      linetype = classe,
      group = interaction(
        classe,
        cenario
      )
    ),
    linewidth = 0.9
  ) +
  geom_point(
    data = centros,
    aes(
      x = x1,
      y = x2,
      shape = classe
    ),
    size = 3
  ) +
  geom_contour(
    data = grade,
    aes(
      x = x1,
      y = x2,
      z = diferenca
    ),
    breaks = 0,
    linewidth = 1.1,
    show.legend = FALSE
  ) +
  facet_wrap(
    ~ cenario
  ) +
  coord_equal() +
  labs(
    x = expression(X[1]),
    y = expression(X[2]),
    linetype = "Classe",
    shape = "Classe"
  ) +
  theme_minimal(
    base_size = 12
  ) +
  theme(
    legend.position = "bottom"
  )
```

A diferença visual entre as fronteiras é uma consequência das hipóteses probabilísticas. Na LDA, os termos quadráticos comuns cancelam-se. Na QDA, esse cancelamento não ocorre quando as matrizes de covariâncias diferem.

## Da regra populacional à regra estimada

As funções discriminantes formuladas até aqui utilizam parâmetros populacionais.

Na LDA, são necessários

$$
\boldsymbol{\mu}_1,\ldots,\boldsymbol{\mu}_G,
\qquad
\boldsymbol{\Sigma},
\qquad
\pi_1,\ldots,\pi_G.
$$

Na QDA, são necessários

$$
\boldsymbol{\mu}_1,\ldots,\boldsymbol{\mu}_G,
\qquad
\boldsymbol{\Sigma}_1,\ldots,\boldsymbol{\Sigma}_G,
\qquad
\pi_1,\ldots,\pi_G.
$$

Essas quantidades são parâmetros fixos da distribuição populacional.

Em uma aplicação, elas geralmente são desconhecidas. Os grupos conhecidos na amostra fornecem informação para estimá-las.

Essa passagem deve ser distinguida cuidadosamente:

- **parâmetro:** característica fixa e desconhecida da população;
- **estimador:** estatística aleatória calculada a partir de uma amostra ainda não observada;
- **estimativa:** valor assumido pelo estimador após a observação dos dados;
- **regra estimada:** regra obtida substituindo os parâmetros desconhecidos por suas estimativas.

Essa distinção utiliza diretamente os fundamentos de estatísticas amostrais e estimação desenvolvidos no Módulo 2.

### Amostra com classes conhecidas

Considere uma amostra de treinamento com

$$
n
=
\sum_{g=1}^{G}
n_g
$$

observações, em que

$$
n_g\geq1
$$

é o número de unidades observadas da classe $g$.

Para a classe $g$, sejam

$$
\boldsymbol{\mathsf X}_{1g},
\ldots,
\boldsymbol{\mathsf X}_{n_g g}
\in
\mathbb R^p
$$

os vetores aleatórios da amostra.

O vetor de médias amostral da classe é o estimador

$$
\overline{\boldsymbol{\mathsf X}}_g
=
\frac1{n_g}
\sum_{i=1}^{n_g}
\boldsymbol{\mathsf X}_{ig}
\in
\mathbb R^p.
$$ {#eq-estimador-media-grupo-discriminante}

Após a observação dos dados, sua realização é

$$
\overline{\mathbf x}_g
=
\frac1{n_g}
\sum_{i=1}^{n_g}
\mathbf x_{ig}.
$$ {#eq-estimativa-media-grupo-discriminante}

Quando

$$
n_g\geq2,
$$

a matriz de covariâncias amostral da classe é

$$
\boldsymbol{\mathsf S}_g
=
\frac1{n_g-1}
\sum_{i=1}^{n_g}
\left(
\boldsymbol{\mathsf X}_{ig}
-
\overline{\boldsymbol{\mathsf X}}_g
\right)
\left(
\boldsymbol{\mathsf X}_{ig}
-
\overline{\boldsymbol{\mathsf X}}_g
\right)^\top.
$$ {#eq-estimador-covariancia-grupo-discriminante}

Sua estimativa observada é

$$
\mathbf S_g
=
\frac1{n_g-1}
\sum_{i=1}^{n_g}
\left(
\mathbf x_{ig}
-
\overline{\mathbf x}_g
\right)
\left(
\mathbf x_{ig}
-
\overline{\mathbf x}_g
\right)^\top.
$$ {#eq-estimativa-covariancia-grupo-discriminante}

Essas estatísticas não dependem da hipótese de normalidade para serem definidas.

Sob o modelo normal, elas passam a ser utilizadas como estimadores dos parâmetros das distribuições condicionais.

### Covariância comum na LDA

Se o modelo populacional supõe

$$
\boldsymbol{\Sigma}_1
=
\cdots
=
\boldsymbol{\Sigma}_G
=
\boldsymbol{\Sigma},
$$

as informações de dispersão dentro das classes podem ser reunidas.

Para

$$
n>G,
$$

defina a matriz aleatória de covariâncias agrupada por

$$
\boldsymbol{\mathsf S}_p
=
\frac{
1
}{
n-G
}
\boldsymbol{\mathsf T}_{\mathrm{intra}},
$$ {#eq-estimador-covariancia-agrupada-discriminante}

em que

$$
\boldsymbol{\mathsf T}_{\mathrm{intra}}
$$

é a matriz aleatória de dispersão intragrupos.

Após a observação da amostra,

$$
\mathbf S_p
=
\frac{
1
}{
n-G
}
\mathbf T_{\mathrm{intra}}.
$$ {#eq-estimativa-covariancia-agrupada-discriminante}

Essa é a estimativa agrupada da matriz de covariâncias comum utilizada na formulação amostral clássica da LDA.

Quando

$$
n_g\geq2,
\qquad
g=1,\ldots,G,
$$

a relação também pode ser escrita como

$$
\mathbf S_p
=
\frac{
\displaystyle
\sum_{g=1}^{G}
(n_g-1)
\mathbf S_g
}{
n-G
}.
$$

Assim, a mesma matriz de dispersão intragrupos que aparece na formulação de Fisher fornece a estimativa de covariância comum utilizada na LDA.


### Probabilidades *a priori*: especificação ou estimação

As probabilidades

$$
\pi_1,\ldots,\pi_G
$$

também precisam ser conhecidas ou especificadas para construir a regra populacional de Bayes.

Quando a amostra constitui uma amostra aleatória representativa da mistura populacional das classes, uma possibilidade é o estimador empírico

$$
\widehat{\pi}_g
=
\frac{n_g}{n}.
$$ {#eq-estimador-priori-empirico-discriminante}

Entretanto,

$$
\frac{n_g}{n}
$$

não deve ser utilizado automaticamente como probabilidade *a priori*.

Em delineamentos que fixam artificialmente os tamanhos dos grupos, estudos caso-controle, amostragens desbalanceadas por planejamento ou situações em que prevalências externas são conhecidas, as proporções amostrais podem não representar

$$
P(\mathsf G=g).
$$

As probabilidades *a priori* podem, portanto:

- ser conhecidas por informação populacional;
- ser especificadas como parte do problema de decisão;
- ser estimadas a partir da amostra quando o mecanismo amostral permite essa interpretação.

Esse ponto é independente da estimação dos vetores de médias e das matrizes de covariâncias.

## Regra estimada da LDA

Suponha que, após observar a amostra de treinamento, estejam disponíveis

$$
\overline{\mathbf x}_1,\ldots,\overline{\mathbf x}_G,
$$

uma matriz agrupada

$$
\mathbf S_p
\succ
\mathbf0
$$

e valores positivos

$$
\widehat{\pi}_1,\ldots,\widehat{\pi}_G
$$

tais que

$$
\sum_{g=1}^{G}
\widehat{\pi}_g
=
1.
$$

::: {#def-funcao-discriminante-linear-estimada}
## Função discriminante linear estimada

Considere

$$
G\geq2,
$$

vetores

$$
\overline{\mathbf x}_g
\in
\mathbb R^p,
\qquad
g=1,\ldots,G,
$$

uma matriz

$$
\mathbf S_p
\in
\mathbb R^{p\times p},
\qquad
\mathbf S_p\succ\mathbf0,
$$

e valores

$$
\widehat{\pi}_g>0,
\qquad
\sum_{g=1}^{G}
\widehat{\pi}_g=1.
$$

A **função discriminante linear estimada** da classe $g$ é

$$
\widehat{\delta}_g(\mathbf x)
=
\mathbf x^\top
\mathbf S_p^{-1}
\overline{\mathbf x}_g
-
\frac12
\overline{\mathbf x}_g^\top
\mathbf S_p^{-1}
\overline{\mathbf x}_g
+
\log
\widehat{\pi}_g,
\qquad
\mathbf x\in\mathbb R^p.
$$ {#eq-funcao-discriminante-linear-estimada}
:::

A regra *plug-in* correspondente é

$$
\widehat{\psi}_{\mathrm{LDA}}(\mathbf x)
\in
\underset{
g\in\{1,\ldots,G\}
}{
\operatorname{arg\,max}
}
\;
\widehat{\delta}_g(\mathbf x).
$$ {#eq-regra-estimada-lda}

A expressão possui a mesma forma algébrica da regra populacional, mas seus elementos têm natureza estatística diferente.

Na regra populacional,

$$
\boldsymbol{\mu}_g,
\qquad
\boldsymbol{\Sigma},
\qquad
\pi_g
$$

são parâmetros.

Na regra estimada,

$$
\overline{\mathbf x}_g,
\qquad
\mathbf S_p,
\qquad
\widehat{\pi}_g
$$

são valores obtidos a partir de uma amostra de treinamento ou especificados a partir de informação externa.

Antes de observar a amostra, os estimadores correspondentes são aleatórios. Portanto, a própria regra estimada varia de uma amostra de treinamento para outra.

Essa variabilidade é distinta da aleatoriedade de uma nova observação a ser classificada.

## Regra estimada da QDA

Na QDA, cada classe possui sua própria matriz de covariâncias.

Suponha que, após a observação da amostra,

$$
\mathbf S_g
\succ
\mathbf0,
\qquad
g=1,\ldots,G,
$$

e que estejam disponíveis

$$
\overline{\mathbf x}_g
$$

e

$$
\widehat{\pi}_g>0.
$$

::: {#def-funcao-discriminante-quadratica-estimada}
## Função discriminante quadrática estimada

Sob as condições anteriores, a **função discriminante quadrática estimada** da classe $g$ é

$$
\widehat{\delta}_g^{Q}(\mathbf x)
=
-\frac12
\log
\left|
\mathbf S_g
\right|
-
\frac12
(\mathbf x-\overline{\mathbf x}_g)^\top
\mathbf S_g^{-1}
(\mathbf x-\overline{\mathbf x}_g)
+
\log
\widehat{\pi}_g.
$$ {#eq-funcao-discriminante-quadratica-estimada}
:::

A regra *plug-in* é

$$
\widehat{\psi}_{\mathrm{QDA}}(\mathbf x)
\in
\underset{
g\in\{1,\ldots,G\}
}{
\operatorname{arg\,max}
}
\;
\widehat{\delta}_g^{Q}(\mathbf x).
$$ {#eq-regra-estimada-qda}

A QDA exige estimar

$$
G
$$

matrizes de covariâncias distintas, enquanto a LDA utiliza uma única matriz agrupada.

Essa diferença possui consequências de posto e quantidade de informação necessária por grupo, que serão examinadas posteriormente na seção sobre existência e singularidade.

O ponto importante nesta etapa é distinguir a **forma do modelo** da **estimação de seus parâmetros**.

A hipótese

$$
\boldsymbol{\Sigma}_1
=
\cdots
=
\boldsymbol{\Sigma}_G
$$

define a estrutura populacional da LDA.

A utilização de

$$
\mathbf S_p
$$

é uma etapa de estimação dessa estrutura.

Analogamente, a hipótese de matrizes populacionais específicas

$$
\boldsymbol{\Sigma}_g
$$

define a QDA, enquanto

$$
\mathbf S_g
$$

são suas estimativas amostrais usuais.

## Regra populacional e regra estimada

A @tbl-populacional-estimada-discriminante sintetiza os níveis que não devem ser confundidos.

| Papel | LDA | QDA |
|---|---|---|
| médias populacionais | $\boldsymbol{\mu}_g$ | $\boldsymbol{\mu}_g$ |
| covariância populacional | $\boldsymbol{\Sigma}$ comum | $\boldsymbol{\Sigma}_g$ específica |
| probabilidades *a priori* | $\pi_g$ | $\pi_g$ |
| estimativa da média | $\overline{\mathbf x}_g$ | $\overline{\mathbf x}_g$ |
| estimativa de covariância | $\mathbf S_p$ | $\mathbf S_g$ |
| probabilidade *a priori* utilizada na regra estimada | $\widehat{\pi}_g$ ou valor especificado | $\widehat{\pi}_g$ ou valor especificado |
| função populacional | $\delta_g(\mathbf x)$ | $\delta_g^Q(\mathbf x)$ |
| função estimada | $\widehat{\delta}_g(\mathbf x)$ | $\widehat{\delta}_g^Q(\mathbf x)$ |

: Distinção entre parâmetros populacionais, estimativas e funções discriminantes estimadas. {#tbl-populacional-estimada-discriminante}

A substituição de parâmetros por estimativas não altera apenas a notação.

A regra populacional de Bayes é definida em relação à distribuição verdadeira e à função de perda especificada.

A regra *plug-in* utiliza uma aproximação dessa regra construída a partir da informação amostral.

Mesmo que o modelo normal esteja corretamente especificado, a regra estimada não coincide necessariamente com a regra populacional porque

$$
\overline{\mathbf x}_g
\neq
\boldsymbol{\mu}_g,
$$

$$
\mathbf S_p
\neq
\boldsymbol{\Sigma}
$$

ou

$$
\mathbf S_g
\neq
\boldsymbol{\Sigma}_g
$$

em uma amostra finita.

Esse é um uso direto do princípio desenvolvido no Módulo 2: estimadores são variáveis aleatórias e diferentes amostras produzem diferentes estimativas.

A avaliação de quanto essa variabilidade afeta a classificação exige estudar o desempenho da regra estimada em novas observações. Esse problema pertence à etapa metodológica posterior e não será desenvolvido aqui.

## O que as regras probabilísticas resolvem

As funções em @eq-funcao-discriminante-linear e @eq-funcao-discriminante-quadratica surgem de um problema de decisão.

Seu objetivo é determinar, para uma observação

$$
\mathbf x,
$$

qual ação minimiza o risco condicional.

Sob perda zero--um, isso equivale a selecionar a classe de maior probabilidade posterior.

A estrutura populacional das duas regras é sintetizada na @tbl-lda-qda.

| Aspecto | LDA | QDA |
|---|---|---|
| modelo condicional | $N_p(\boldsymbol{\mu}_g,\boldsymbol{\Sigma})$ | $N_p(\boldsymbol{\mu}_g,\boldsymbol{\Sigma}_g)$ |
| covariância | comum a todas as classes | específica de cada classe |
| função discriminante | afim em $\mathbf x$ | quadrática em $\mathbf x$ |
| fronteira entre duas classes | hiperplano | superfície quadrática em geral |
| geometria de distância | uma única métrica de Mahalanobis | uma métrica de Mahalanobis para cada classe |
| probabilidades *a priori* | entram por $\log\pi_g$ | entram por $\log\pi_g$ |
| perda considerada nas fórmulas apresentadas | zero--um | zero--um |
| resultado imediato | regra populacional de classificação | regra populacional de classificação |

: Comparação entre as formulações discriminantes linear e quadrática sob normalidade. {#tbl-lda-qda}

A construção probabilística não procura uma direção única que maximize a separação entre os grupos.

Ela compara diretamente os modelos condicionais das classes no espaço original das características e produz uma regra de decisão.

A formulação geométrica de Fisher parte de outro problema.

Nela, os rótulos conhecidos serão utilizados para decompor a dispersão em componentes dentro e entre os grupos e procurar combinações lineares nas quais a separação entre eles seja grande relativamente à dispersão intragrupos.

Essa nova formulação utilizará diretamente os fundamentos sobre esperança, covariância, formas quadráticas e autovalores generalizados desenvolvidos nos Módulos 1 e 2.

## Formulação geométrica de Fisher

A formulação probabilística desenvolvida nas seções anteriores produz uma regra para decidir a classe de uma nova observação. A formulação de Fisher parte de outro problema: construir combinações lineares das características que evidenciem a separação entre grupos previamente conhecidos.

Esse problema pode ser formulado tanto em nível populacional quanto em nível amostral.

A formulação populacional permite identificar qual estrutura de dispersão se pretende representar. A formulação amostral substitui essa estrutura pelas dispersões calculadas a partir das observações com rótulos conhecidos.

Nenhuma dessas formulações exige, para ser definida, que as distribuições condicionais sejam normais.

### Dispersões populacionais dentro e entre grupos

Considere novamente

$$
\mathsf G
\in
\{1,\ldots,G\},
$$

com

$$
P(\mathsf G=g)
=
\pi_g,
\qquad
\pi_g>0,
\qquad
\sum_{g=1}^{G}\pi_g=1,
$$

e um vetor aleatório

$$
\boldsymbol{\mathsf X}
\in
\mathbb R^p
$$

com segundos momentos finitos.

Para cada classe, defina

$$
\boldsymbol{\mu}_g
=
E
\left(
\boldsymbol{\mathsf X}
\mid
\mathsf G=g
\right)
\in
\mathbb R^p
$$

e

$$
\boldsymbol{\Sigma}_g
=
\operatorname{Cov}
\left(
\boldsymbol{\mathsf X}
\mid
\mathsf G=g
\right)
\in
\mathbb R^{p\times p}.
$$

A média marginal de

$$
\boldsymbol{\mathsf X}
$$

é

$$
\boldsymbol{\mu}
=
E
\left(
\boldsymbol{\mathsf X}
\right)
=
\sum_{g=1}^{G}
\pi_g
\boldsymbol{\mu}_g.
$$ {#eq-media-populacional-fisher}

::: {#def-dispersoes-populacionais-fisher}
## Dispersões populacionais de Fisher

Considere

$$
\mathsf G
\in
\{1,\ldots,G\},
$$

com

$$
P(\mathsf G=g)=\pi_g>0,
\qquad
\sum_{g=1}^{G}\pi_g=1,
$$

e

$$
E
\left(
\|
\boldsymbol{\mathsf X}
\|^2
\right)
<
\infty.
$$

Se

$$
\boldsymbol{\mu}_g
=
E
\left(
\boldsymbol{\mathsf X}
\mid
\mathsf G=g
\right)
$$

e

$$
\boldsymbol{\Sigma}_g
=
\operatorname{Cov}
\left(
\boldsymbol{\mathsf X}
\mid
\mathsf G=g
\right),
$$

defina a **dispersão populacional intragrupos** por

$$
\boldsymbol{\Sigma}_W
=
\sum_{g=1}^{G}
\pi_g
\boldsymbol{\Sigma}_g
\in
\mathbb R^{p\times p},
$$ {#eq-dispersao-populacional-intra-fisher}

e a **dispersão populacional entre grupos** por

$$
\boldsymbol{\Sigma}_B
=
\sum_{g=1}^{G}
\pi_g
\left(
\boldsymbol{\mu}_g
-
\boldsymbol{\mu}
\right)
\left(
\boldsymbol{\mu}_g
-
\boldsymbol{\mu}
\right)^\top
\in
\mathbb R^{p\times p},
$$ {#eq-dispersao-populacional-entre-fisher}

em que

$$
\boldsymbol{\mu}
=
\sum_{g=1}^{G}
\pi_g
\boldsymbol{\mu}_g.
$$
:::

As duas matrizes são simétricas e semidefinidas positivas:

$$
\boldsymbol{\Sigma}_W
\succeq
\mathbf0
$$

e

$$
\boldsymbol{\Sigma}_B
\succeq
\mathbf0.
$$

A primeira resume a variabilidade existente **dentro das classes**, enquanto a segunda resume a variabilidade dos vetores de médias das classes em torno da média marginal.

Essas duas componentes recuperam a matriz de covariâncias marginal de

$$
\boldsymbol{\mathsf X}.
$$

::: {#prp-decomposicao-covariancia-fisher}
## Decomposição populacional da covariância por grupos

Considere

$$
\mathsf G
\in
\{1,\ldots,G\},
$$

com

$$
P(\mathsf G=g)=\pi_g>0,
\qquad
\sum_{g=1}^{G}\pi_g=1,
$$

e

$$
E
\left(
\|
\boldsymbol{\mathsf X}
\|^2
\right)
<
\infty.
$$

Defina

$$
\boldsymbol{\mu}_g
=
E
\left(
\boldsymbol{\mathsf X}
\mid
\mathsf G=g
\right),
\qquad
\boldsymbol{\mu}
=
\sum_{g=1}^{G}
\pi_g\boldsymbol{\mu}_g,
$$

$$
\boldsymbol{\Sigma}_W
=
\sum_{g=1}^{G}
\pi_g
\operatorname{Cov}
\left(
\boldsymbol{\mathsf X}
\mid
\mathsf G=g
\right)
$$

e

$$
\boldsymbol{\Sigma}_B
=
\sum_{g=1}^{G}
\pi_g
(\boldsymbol{\mu}_g-\boldsymbol{\mu})
(\boldsymbol{\mu}_g-\boldsymbol{\mu})^\top.
$$

Então,

$$
\operatorname{Cov}
\left(
\boldsymbol{\mathsf X}
\right)
=
\boldsymbol{\Sigma}_W
+
\boldsymbol{\Sigma}_B.
$$ {#eq-decomposicao-covariancia-fisher}
:::

::: {.proof}
Escreva

$$
\boldsymbol{\mathsf X}
-
\boldsymbol{\mu}
=
\left(
\boldsymbol{\mathsf X}
-
\boldsymbol{\mu}_{\mathsf G}
\right)
+
\left(
\boldsymbol{\mu}_{\mathsf G}
-
\boldsymbol{\mu}
\right),
$$

em que

$$
\boldsymbol{\mu}_{\mathsf G}
=
E
\left(
\boldsymbol{\mathsf X}
\mid
\mathsf G
\right).
$$

Ao desenvolver o produto externo e tomar esperança, os termos cruzados são nulos porque

$$
E
\left[
\boldsymbol{\mathsf X}
-
\boldsymbol{\mu}_{\mathsf G}
\mid
\mathsf G
\right]
=
\mathbf0.
$$

Assim,

$$
\begin{aligned}
\operatorname{Cov}
\left(
\boldsymbol{\mathsf X}
\right)
&=
E
\left[
\operatorname{Cov}
\left(
\boldsymbol{\mathsf X}
\mid
\mathsf G
\right)
\right]
\\
&\quad+
\operatorname{Cov}
\left[
E
\left(
\boldsymbol{\mathsf X}
\mid
\mathsf G
\right)
\right]
\\
&=
\boldsymbol{\Sigma}_W
+
\boldsymbol{\Sigma}_B.
\end{aligned}
$$
:::

Essa é a versão populacional da mesma ideia de decomposição da dispersão que, no Módulo 2, apareceu para os dados observados como

$$
\mathbf T
=
\mathbf T_{\mathrm{intra}}
+
\mathbf T_{\mathrm{entre}}.
$$

As duas decomposições não devem ser confundidas.

Em

$$
\boldsymbol{\Sigma}_W
\qquad\text{e}\qquad
\boldsymbol{\Sigma}_B,
$$

os objetos são parâmetros populacionais.

Em

$$
\mathbf T_{\mathrm{intra}}
\qquad\text{e}\qquad
\mathbf T_{\mathrm{entre}},
$$

os objetos são matrizes calculadas a partir de uma amostra observada.

### Critério populacional de Fisher

Considere uma direção

$$
\mathbf a
\in
\mathbb R^p,
\qquad
\mathbf a\neq\mathbf0,
$$

e a combinação linear

$$
\mathsf Z
=
\mathbf a^\top
\boldsymbol{\mathsf X}.
$$

Dentro da classe $g$,

$$
\operatorname{Var}
\left(
\mathsf Z
\mid
\mathsf G=g
\right)
=
\mathbf a^\top
\boldsymbol{\Sigma}_g
\mathbf a.
$$

A média ponderada dessas variâncias é

$$
\sum_{g=1}^{G}
\pi_g
\operatorname{Var}
\left(
\mathsf Z
\mid
\mathsf G=g
\right)
=
\mathbf a^\top
\boldsymbol{\Sigma}_W
\mathbf a.
$$ {#eq-dispersao-populacional-intra-projetada-fisher}

Por outro lado, a variabilidade das médias condicionais projetadas é

$$
\sum_{g=1}^{G}
\pi_g
\left[
\mathbf a^\top
\left(
\boldsymbol{\mu}_g-\boldsymbol{\mu}
\right)
\right]^2
=
\mathbf a^\top
\boldsymbol{\Sigma}_B
\mathbf a.
$$ {#eq-dispersao-populacional-entre-projetada-fisher}

::: {#def-criterio-fisher-populacional-discriminante}
## Critério populacional de Fisher

Sejam

$$
\boldsymbol{\Sigma}_B,
\boldsymbol{\Sigma}_W
\in
\mathbb R^{p\times p}
$$

matrizes simétricas tais que

$$
\boldsymbol{\Sigma}_B
\succeq
\mathbf0
\qquad
\text{e}
\qquad
\boldsymbol{\Sigma}_W
\succ
\mathbf0.
$$

Para

$$
\mathbf a
\in
\mathbb R^p,
\qquad
\mathbf a\neq\mathbf0,
$$

o **critério populacional de Fisher** é

$$
J_F^{(P)}
(\mathbf a)
=
\frac{
\mathbf a^\top
\boldsymbol{\Sigma}_B
\mathbf a
}{
\mathbf a^\top
\boldsymbol{\Sigma}_W
\mathbf a
}.
$$ {#eq-criterio-fisher-populacional-discriminante}
:::

O numerador mede a separação dos vetores de médias das classes depois da projeção.

O denominador mede a dispersão média existente dentro das classes nessa mesma direção.

Portanto, Fisher não procura simplesmente uma direção em que os vetores de médias estejam distantes. A separação é avaliada relativamente à variabilidade interna das populações.

Essa formulação requer segundos momentos finitos e, na forma clássica acima, positividade definida de

$$
\boldsymbol{\Sigma}_W.
$$

Ela **não exige normalidade multivariada**.

Também não exige

$$
\boldsymbol{\Sigma}_1
=
\cdots
=
\boldsymbol{\Sigma}_G.
$$

Se as covariâncias forem comuns,

$$
\boldsymbol{\Sigma}_g
=
\boldsymbol{\Sigma},
\qquad
g=1,\ldots,G,
$$

então

$$
\boldsymbol{\Sigma}_W
=
\boldsymbol{\Sigma},
$$

situação que posteriormente estabelecerá uma ligação direta entre Fisher e a LDA.

## Formulação amostral de Fisher

Considere agora uma amostra com grupos conhecidos, contendo

$$
n_g
$$

observações no grupo $g$ e

$$
n
=
\sum_{g=1}^{G}
n_g
$$

observações no total.

As matrizes de dispersão intragrupos e entre grupos foram definidas no Módulo 2 em @def-sscp-intragrupos e @def-sscp-entre-grupos:

$$
\mathbf T_{\mathrm{intra}}
=
\sum_{g=1}^{G}
\sum_{i=1}^{n_g}
\left(
\mathbf x_{ig}
-
\overline{\mathbf x}_g
\right)
\left(
\mathbf x_{ig}
-
\overline{\mathbf x}_g
\right)^\top
$$

e

$$
\mathbf T_{\mathrm{entre}}
=
\sum_{g=1}^{G}
n_g
\left(
\overline{\mathbf x}_g
-
\overline{\mathbf x}
\right)
\left(
\overline{\mathbf x}_g
-
\overline{\mathbf x}
\right)^\top.
$$

Pela propriedade estabelecida em @prp-decomposicao-dispersao-grupos,

$$
\mathbf T
=
\mathbf T_{\mathrm{intra}}
+
\mathbf T_{\mathrm{entre}}.
$$

Esses resultados não precisam ser novamente demonstrados.

Para relacioná-los à formulação populacional, observe que

$$
\frac1n
\mathbf T_{\mathrm{intra}}
$$

e

$$
\frac1n
\mathbf T_{\mathrm{entre}}
$$

são os análogos empíricos das duas parcelas de dispersão quando os pesos dos grupos são

$$
\frac{n_g}{n}.
$$

A multiplicação de ambas as matrizes por um mesmo escalar positivo não altera o quociente de Fisher. Portanto,

$$
\frac{
\mathbf a^\top
(\mathbf T_{\mathrm{entre}}/n)
\mathbf a
}{
\mathbf a^\top
(\mathbf T_{\mathrm{intra}}/n)
\mathbf a
}
=
\frac{
\mathbf a^\top
\mathbf T_{\mathrm{entre}}
\mathbf a
}{
\mathbf a^\top
\mathbf T_{\mathrm{intra}}
\mathbf a
}.
$$

Assim, a formulação amostral pode ser escrita diretamente em termos das matrizes de dispersão já construídas no Módulo 2.

Os pesos

$$
n_g
$$

presentes em

$$
\mathbf T_{\mathrm{entre}}
$$

são pesos da decomposição da **amostra observada**. Eles não devem ser confundidos automaticamente com as probabilidades *a priori*

$$
\pi_g
$$

utilizadas na regra de Bayes.

Em particular, quando os tamanhos dos grupos são fixados pelo delineamento, as proporções

$$
\frac{n_g}{n}
$$

podem não representar as prevalências populacionais.

## Dispersão depois de uma projeção

Considere

$$
\mathbf a
\in
\mathbb R^p,
\qquad
\mathbf a\neq\mathbf0,
$$

e a combinação linear

$$
Z
=
\mathbf a^\top
\mathbf x.
$$ {#eq-combinacao-fisher-discriminante}

Depois da projeção, a dispersão amostral dentro dos grupos é

$$
\mathbf a^\top
\mathbf T_{\mathrm{intra}}
\mathbf a,
$$ {#eq-dispersao-intra-projetada-discriminante}

enquanto a dispersão amostral entre os centroides é

$$
\mathbf a^\top
\mathbf T_{\mathrm{entre}}
\mathbf a.
$$ {#eq-dispersao-entre-projetada-discriminante}

A primeira quantidade aumenta quando as observações permanecem muito dispersas em torno de seus respectivos centroides depois da projeção.

A segunda aumenta quando os centroides permanecem separados na direção escolhida.

::: {#def-criterio-fisher-discriminante}
## Critério amostral de Fisher

Sejam

$$
\mathbf T_{\mathrm{entre}},
\mathbf T_{\mathrm{intra}}
\in
\mathbb R^{p\times p}
$$

matrizes simétricas tais que

$$
\mathbf T_{\mathrm{entre}}
\succeq
\mathbf0
$$

e

$$
\mathbf T_{\mathrm{intra}}
\succ
\mathbf0.
$$

Para

$$
\mathbf a
\in
\mathbb R^p,
\qquad
\mathbf a\neq\mathbf0,
$$

o **critério amostral de Fisher** é

$$
J_F(\mathbf a)
=
\frac{
\mathbf a^\top
\mathbf T_{\mathrm{entre}}
\mathbf a
}{
\mathbf a^\top
\mathbf T_{\mathrm{intra}}
\mathbf a
}.
$$ {#eq-criterio-fisher-discriminante}
:::

O quociente é invariante à escala de $\mathbf a$. Para todo

$$
c\neq0,
$$

$$
J_F(c\mathbf a)
=
J_F(\mathbf a).
$$

Portanto, Fisher escolhe uma **direção**, e não um vetor com comprimento determinado.

Maximizar

$$
J_F(\mathbf a)
$$

significa encontrar uma combinação das características em que a separação entre os grupos seja grande relativamente à dispersão que permanece dentro deles.

Essa é uma projeção supervisionada: os rótulos entram diretamente na construção de

$$
\mathbf T_{\mathrm{intra}}
$$

e

$$
\mathbf T_{\mathrm{entre}}.
$$

A @fig-projecoes-fisher compara duas projeções das mesmas observações. A primeira utiliza a direção que une os centroides. A segunda incorpora a dispersão intragrupos por meio do critério de Fisher. Para tornar comparáveis as projeções, seus escores são expressos relativamente à dispersão intragrupos.

```{r}
#| label: fig-projecoes-fisher
#| echo: false
#| warning: false
#| message: false
#| fig-cap: "Projeção das mesmas duas classes na direção que une os centroides e na direção de Fisher. A direção discriminante aumenta a separação entre os grupos relativamente à dispersão intragrupos."
#| fig-width: 10
#| fig-height: 5
#| fig-align: center

library(ggplot2)

rmvnorm_base <- function(
  n,
  mu,
  Sigma
) {
  p <- length(mu)

  Z <- matrix(
    rnorm(
      n * p
    ),
    nrow = n,
    ncol = p
  )

  X <- Z %*%
    chol(Sigma)

  sweep(
    X,
    2,
    mu,
    "+"
  )
}

padroniza_intra <- function(
  z,
  grupo
) {
  medias_grupo <- ave(
    z,
    grupo
  )

  variancia_intra <-
    sum(
      (z - medias_grupo)^2
    ) /
    (
      length(z) -
        length(
          unique(grupo)
        )
    )

  (
    z - mean(z)
  ) /
    sqrt(
      variancia_intra
    )
}

set.seed(2026)

mu_1 <- c(
  -1.5,
  -0.5
)

mu_2 <- c(
  1.5,
  0.5
)

Sigma <- matrix(
  c(
    2.0, 1.4,
    1.4, 1.2
  ),
  nrow = 2,
  byrow = TRUE
)

n <- 180

X_1 <- rmvnorm_base(
  n,
  mu_1,
  Sigma
)

X_2 <- rmvnorm_base(
  n,
  mu_2,
  Sigma
)

X <- rbind(
  X_1,
  X_2
)

grupo <- factor(
  rep(
    c(
      "Classe 1",
      "Classe 2"
    ),
    each = n
  )
)

media_1 <- colMeans(
  X_1
)

media_2 <- colMeans(
  X_2
)

d <- media_2 - media_1

D_1 <- sweep(
  X_1,
  2,
  media_1,
  "-"
)

D_2 <- sweep(
  X_2,
  2,
  media_2,
  "-"
)

T_intra <- crossprod(
  D_1
) +
  crossprod(
    D_2
  )

a_centroides <- d /
  sqrt(
    sum(
      d^2
    )
  )

a_fisher <- solve(
  T_intra,
  d
)

a_fisher <- a_fisher /
  sqrt(
    as.numeric(
      t(a_fisher) %*%
        T_intra %*%
        a_fisher
    )
  )

z_centroides <- as.vector(
  X %*%
    a_centroides
)

z_fisher <- as.vector(
  X %*%
    a_fisher
)

dados_projecao <- rbind(
  data.frame(
    escore = padroniza_intra(
      z_centroides,
      grupo
    ),
    grupo = grupo,
    direcao =
      "Direção entre os centroides"
  ),
  data.frame(
    escore = padroniza_intra(
      z_fisher,
      grupo
    ),
    grupo = grupo,
    direcao =
      "Direção de Fisher"
  )
)

ggplot(
  dados_projecao,
  aes(
    x = escore,
    linetype = grupo
  )
) +
  geom_density(
    linewidth = 1.1,
    adjust = 1.05
  ) +
  facet_wrap(
    ~ direcao,
    nrow = 1
  ) +
  labs(
    x =
      "Coordenada projetada / dispersão intragrupos",
    y =
      "Densidade",
    linetype =
      "Classe"
  ) +
  theme_minimal(
    base_size = 12
  ) +
  theme(
    legend.position = "bottom"
  )
```

A figura ilustra o papel do denominador de @eq-criterio-fisher-discriminante: uma diferença entre centroides é mais informativa quando ocorre em uma direção na qual a dispersão intragrupos é relativamente pequena.

## Direções discriminantes e autovalores generalizados

O critério de Fisher é exatamente o quociente de Rayleigh generalizado definido em @def-quociente-rayleigh-generalizado, aplicado ao par

$$
\left(
\mathbf T_{\mathrm{entre}},
\mathbf T_{\mathrm{intra}}
\right).
$$

Pelo resultado geral de @thm-extremos-rayleigh-generalizado, a maximização de

$$
J_F(\mathbf a)
$$

conduz ao problema

$$
\mathbf T_{\mathrm{entre}}
\mathbf a
=
\lambda
\mathbf T_{\mathrm{intra}}
\mathbf a.
$$ {#eq-autovalor-generalizado-fisher}

Esse é o mesmo tipo de problema matemático utilizado no capítulo de MANOVA.

A técnica, entretanto, atribui outro significado às soluções.

Na MANOVA, as raízes são agregadas para construir critérios de teste.

Na formulação de Fisher, os autovetores são utilizados como direções para representar a separação entre grupos.

### Geometria pela dispersão intragrupos

Como

$$
\mathbf T_{\mathrm{intra}}
\succ
\mathbf0,
$$

sua raiz quadrada principal e sua inversa existem.

Defina

$$
\mathbf u
=
\mathbf T_{\mathrm{intra}}^{1/2}
\mathbf a.
$$

Então,

$$
\mathbf a
=
\mathbf T_{\mathrm{intra}}^{-1/2}
\mathbf u,
$$

e @eq-autovalor-generalizado-fisher torna-se

$$
\mathbf T_{\mathrm{intra}}^{-1/2}
\mathbf T_{\mathrm{entre}}
\mathbf T_{\mathrm{intra}}^{-1/2}
\mathbf u
=
\lambda
\mathbf u.
$$ {#eq-fisher-branqueado}

Defina

$$
\mathbf K_F
=
\mathbf T_{\mathrm{intra}}^{-1/2}
\mathbf T_{\mathrm{entre}}
\mathbf T_{\mathrm{intra}}^{-1/2}
\in
\mathbb R^{p\times p}.
$$ {#eq-matriz-fisher-branqueada}

A matriz

$$
\mathbf K_F
$$

é simétrica e semidefinida positiva.

Além disso, como

$$
\mathbf T_{\mathrm{intra}}^{-1/2}
$$

é invertível,

$$
\operatorname{posto}
\left(
\mathbf K_F
\right)
=
\operatorname{posto}
\left(
\mathbf T_{\mathrm{entre}}
\right).
$$ {#eq-posto-matriz-fisher-branqueada}

Defina

$$
r_F
=
\operatorname{posto}
\left(
\mathbf T_{\mathrm{entre}}
\right).
$$ {#eq-posto-entre-fisher}

Como

$$
\mathbf K_F
\succeq
\mathbf0,
$$

ela possui exatamente

$$
r_F
$$

autovalores positivos.

Esses autovalores são exatamente as raízes positivas do problema generalizado de Fisher.

::: {#def-direcoes-discriminantes-fisher}
## Direções discriminantes de Fisher

Sejam

$$
\mathbf T_{\mathrm{entre}},
\mathbf T_{\mathrm{intra}}
\in
\mathbb R^{p\times p}
$$

matrizes simétricas tais que

$$
\mathbf T_{\mathrm{entre}}
\succeq
\mathbf0
$$

e

$$
\mathbf T_{\mathrm{intra}}
\succ
\mathbf0.
$$

Defina

$$
r_F
=
\operatorname{posto}
\left(
\mathbf T_{\mathrm{entre}}
\right).
$$

Se

$$
r_F\geq1,
$$

as **direções discriminantes de Fisher** são autovetores generalizados

$$
\mathbf a_1,\ldots,\mathbf a_{r_F}
\in
\mathbb R^p
$$

associados às raízes positivas

$$
\lambda_1
\geq
\lambda_2
\geq
\cdots
\geq
\lambda_{r_F}
>
0
$$

do problema

$$
\mathbf T_{\mathrm{entre}}
\mathbf a_j
=
\lambda_j
\mathbf T_{\mathrm{intra}}
\mathbf a_j.
$$
:::

Se

$$
r_F=0,
$$

então

$$
\mathbf T_{\mathrm{entre}}
=
\mathbf0,
$$

e não existe direção com separação amostral positiva entre os centroides.

A primeira direção satisfaz

$$
J_F(\mathbf a_1)
=
\lambda_1
=
\max_{\mathbf a\neq\mathbf0}
J_F(\mathbf a).
$$

Pela ortogonalidade dos autovetores generalizados estabelecida em @prp-ortogonalidade-autovetores-generalizados, as direções podem ser escolhidas de modo que

$$
\mathbf a_j^\top
\mathbf T_{\mathrm{intra}}
\mathbf a_\ell
=
0,
\qquad
j\neq\ell.
$$ {#eq-ortogonalidade-fisher}

Essa ortogonalidade é definida pela geometria intragrupos.

Em geral,

$$
\mathbf a_j^\top
\mathbf a_\ell
$$

não precisa ser zero.

Uma normalização conveniente é

$$
\mathbf a_j^\top
\mathbf T_{\mathrm{intra}}
\mathbf a_j
=
1.
$$ {#eq-normalizacao-fisher}

Sob essa normalização,

$$
\begin{aligned}
\mathbf a_j^\top
\mathbf T_{\mathrm{entre}}
\mathbf a_j
&=
\lambda_j
\mathbf a_j^\top
\mathbf T_{\mathrm{intra}}
\mathbf a_j
\\
&=
\lambda_j.
\end{aligned}
$$ {#eq-raiz-separacao-fisher}

Assim, cada raiz mede a separação entre grupos na correspondente direção depois que a dispersão intragrupos foi normalizada para $1$.

### Número de direções discriminantes

Pelo resultado estabelecido no Módulo 2,

$$
\operatorname{posto}
\left(
\mathbf T_{\mathrm{entre}}
\right)
\leq
\min
\left(
p,G-1
\right).
$$

Como

$$
r_F
=
\operatorname{posto}
\left(
\mathbf T_{\mathrm{entre}}
\right),
$$

segue que

$$
r_F
\leq
\min
\left(
p,G-1
\right).
$$ {#eq-numero-direcoes-discriminantes}

Esse limite possui uma interpretação direta.

Os $G$ centroides centralizados em torno da média global satisfazem uma relação linear ponderada e, portanto, podem gerar no máximo

$$
G-1
$$

direções independentes de separação.

Assim:

- dois grupos produzem no máximo uma direção discriminante;
- três grupos produzem no máximo duas;
- em geral, $G$ grupos produzem no máximo $G-1$ direções.

O limite também não pode ultrapassar a dimensão original

$$
p.
$$

Por isso,

$$
\min(p,G-1)
$$

é o número máximo possível de dimensões discriminantes não nulas.

## Coordenadas discriminantes

As direções

$$
\mathbf a_1,\ldots,\mathbf a_{r_F}
$$

definem um subespaço supervisionado para representar as observações.

::: {#def-coordenadas-discriminantes}
## Coordenadas discriminantes

Considere

$$
r_F\geq1
$$

direções discriminantes

$$
\mathbf a_1,\ldots,\mathbf a_{r_F}
\in
\mathbb R^p
$$

e uma média global amostral

$$
\overline{\mathbf x}
\in
\mathbb R^p.
$$

A **coordenada discriminante $j$** de uma observação

$$
\mathbf x
\in
\mathbb R^p
$$

é

$$
z_j(\mathbf x)
=
\mathbf a_j^\top
\left(
\mathbf x
-
\overline{\mathbf x}
\right),
\qquad
j=1,\ldots,r_F.
$$ {#eq-coordenada-discriminante}

O vetor de coordenadas discriminantes é

$$
\mathbf z(\mathbf x)
=
\begin{pmatrix}
z_1(\mathbf x)\\
\vdots\\
z_{r_F}(\mathbf x)
\end{pmatrix}
\in
\mathbb R^{r_F}.
$$ {#eq-vetor-coordenadas-discriminantes}
:::

A centralização em torno de

$$
\overline{\mathbf x}
$$

coloca a média global na origem da representação.

O uso de

$$
\mathbf a_j^\top\mathbf x
$$

sem centralização preservaria todas as diferenças entre observações e centroides; modificaria apenas a origem das coordenadas.

O centroide discriminante da classe $g$ na dimensão $j$ é

$$
\overline z_{gj}
=
\mathbf a_j^\top
\left(
\overline{\mathbf x}_g
-
\overline{\mathbf x}
\right).
$$ {#eq-centroide-discriminante}

A representação discriminante possui dimensão

$$
r_F
\leq
\min
\left(
p,G-1
\right).
$$

Ela é uma redução de dimensionalidade **supervisionada**, pois depende dos rótulos conhecidos.

Alterar os grupos modifica

$$
\mathbf T_{\mathrm{intra}},
\qquad
\mathbf T_{\mathrm{entre}}
$$

e, consequentemente, pode modificar tanto o subespaço discriminante quanto suas direções.

A magnitude isolada dos coeficientes de uma direção deve ser interpretada com cautela. Esses coeficientes dependem da escala das características e de suas relações de dispersão e covariância. Procedimentos operacionais para interpretar coeficientes, correlações estruturais e contribuições das variáveis serão tratados posteriormente.

A @fig-coordenadas-discriminantes mostra três grupos observados em três características. O primeiro painel utiliza apenas duas características originais. O segundo utiliza as duas direções discriminantes disponíveis para os três grupos e incorpora informação das três características.

```{r}
#| label: fig-coordenadas-discriminantes
#| echo: false
#| warning: false
#| message: false
#| fig-cap: "Representação de três grupos em duas características originais e nas duas coordenadas discriminantes de Fisher. As coordenadas são construídas especificamente para representar a separação entre os grupos."
#| fig-width: 10
#| fig-height: 5
#| fig-align: center

library(ggplot2)

rmvnorm_base <- function(
  n,
  mu,
  Sigma
) {
  p <- length(mu)

  Z <- matrix(
    rnorm(
      n * p
    ),
    nrow = n,
    ncol = p
  )

  X <- Z %*%
    chol(Sigma)

  sweep(
    X,
    2,
    mu,
    "+"
  )
}

set.seed(2027)

Sigma <- matrix(
  c(
    1.2, 0.7, 0.2,
    0.7, 1.8, 0.1,
    0.2, 0.1, 0.7
  ),
  nrow = 3,
  byrow = TRUE
)

medias <- list(
  c(
    -1.6,
    0.0,
    -0.8
  ),
  c(
    1.6,
    0.0,
    -0.8
  ),
  c(
    0.0,
    0.3,
    1.8
  )
)

n_grupo <- 55

X_lista <- lapply(
  medias,
  function(mu) {
    rmvnorm_base(
      n_grupo,
      mu,
      Sigma
    )
  }
)

X <- do.call(
  rbind,
  X_lista
)

grupo <- factor(
  rep(
    c(
      "Grupo 1",
      "Grupo 2",
      "Grupo 3"
    ),
    each = n_grupo
  )
)

media_global <- colMeans(
  X
)

T_intra <- matrix(
  0,
  nrow = 3,
  ncol = 3
)

T_entre <- matrix(
  0,
  nrow = 3,
  ncol = 3
)

for (
  g in levels(grupo)
) {
  X_g <- X[
    grupo == g,
    ,
    drop = FALSE
  ]

  media_g <- colMeans(
    X_g
  )

  D_g <- sweep(
    X_g,
    2,
    media_g,
    "-"
  )

  T_intra <- T_intra +
    crossprod(
      D_g
    )

  d_g <- matrix(
    media_g -
      media_global,
    ncol = 1
  )

  T_entre <- T_entre +
    nrow(X_g) *
    (
      d_g %*%
        t(d_g)
    )
}

decomp_intra <- eigen(
  T_intra,
  symmetric = TRUE
)

T_intra_menos_meio <-
  decomp_intra$vectors %*%
  diag(
    1 /
      sqrt(
        decomp_intra$values
      )
  ) %*%
  t(
    decomp_intra$vectors
  )

K_F <-
  T_intra_menos_meio %*%
  T_entre %*%
  T_intra_menos_meio

decomp_fisher <- eigen(
  K_F,
  symmetric = TRUE
)

r_F <- sum(
  decomp_fisher$values >
    1e-10
)

U_F <- decomp_fisher$vectors[
  ,
  seq_len(r_F),
  drop = FALSE
]

A <- T_intra_menos_meio %*%
  U_F

X_c <- sweep(
  X,
  2,
  media_global,
  "-"
)

Z <- X_c %*%
  A

dados_original <- data.frame(
  eixo_1 = X[, 1],
  eixo_2 = X[, 2],
  grupo = grupo,
  painel =
    "Características originais: X1 × X2"
)

dados_discriminante <- data.frame(
  eixo_1 = Z[, 1],
  eixo_2 = Z[, 2],
  grupo = grupo,
  painel =
    "Coordenadas discriminantes: z1 × z2"
)

dados_figura <- rbind(
  dados_original,
  dados_discriminante
)

centroides <- aggregate(
  cbind(
    eixo_1,
    eixo_2
  ) ~
    grupo +
    painel,
  data = dados_figura,
  FUN = mean
)

ggplot(
  dados_figura,
  aes(
    x = eixo_1,
    y = eixo_2,
    color = grupo,
    shape = grupo
  )
) +
  geom_point(
    alpha = 0.6,
    size = 1.8
  ) +
  geom_point(
    data = centroides,
    aes(
      x = eixo_1,
      y = eixo_2,
      color = grupo
    ),
    inherit.aes = FALSE,
    shape = 4,
    size = 4,
    stroke = 1.3
  ) +
  facet_wrap(
    ~ painel,
    scales = "free"
  ) +
  labs(
    x = NULL,
    y = NULL,
    color = "Grupo",
    shape = "Grupo"
  ) +
  theme_minimal(
    base_size = 12
  ) +
  theme(
    legend.position = "bottom"
  )
```

No segundo painel, as direções utilizadas satisfazem a geometria do problema generalizado. A matriz

$$
\mathbf T_{\mathrm{intra}}^{-1/2}
$$

transforma inicialmente a dispersão intragrupos, e os autovetores de

$$
\mathbf K_F
$$

determinam as direções de maior separação entre os centroides nesse espaço transformado.

A diferença em relação à ACP é estrutural.

A ACP escolhe direções em função da variabilidade total da nuvem.

Fisher escolhe direções em função da razão entre duas fontes de dispersão definidas pelos grupos.

## Relação espectral com a MANOVA

A semelhança entre Fisher e MANOVA não é apenas formal.

Na comparação de grupos,

$$
\mathbf E
=
\mathbf T_{\mathrm{intra}}
$$

e

$$
\mathbf H
=
\mathbf T_{\mathrm{entre}}.
$$ {#eq-correspondencia-manova-fisher}

Logo, os problemas

$$
\mathbf H
\mathbf a
=
\lambda
\mathbf E
\mathbf a
$$

e

$$
\mathbf T_{\mathrm{entre}}
\mathbf a
=
\lambda
\mathbf T_{\mathrm{intra}}
\mathbf a
$$

são o mesmo problema espectral quando representam a mesma estrutura de grupos.

Isso significa que as raízes e os subespaços característicos coincidem.

O objetivo estatístico, porém, muda.

Na MANOVA, as raízes são agregadas em critérios como Wilks, Pillai, Hotelling--Lawley e Roy para avaliar uma hipótese sobre diferenças entre grupos.

Na formulação de Fisher, os autovetores associados às raízes positivas são utilizados para construir direções e coordenadas de separação.

A mesma estrutura de autovalores generalizados sustenta, portanto, duas técnicas porque cada uma faz uma pergunta estatística diferente.

Essa distinção prepara o caso de dois grupos, no qual a direção de Fisher pode ser escrita explicitamente e sua relação com a regra discriminante linear normal torna-se particularmente transparente.

## O caso de dois grupos

Quando

$$
G=2,
$$

a dispersão entre grupos possui posto no máximo $1$. Portanto, existe no máximo uma direção discriminante com raiz positiva.

Defina a diferença entre os centroides amostrais por

$$
\mathbf d
=
\overline{\mathbf x}_1
-
\overline{\mathbf x}_2
\in
\mathbb R^p.
$$ {#eq-diferenca-medias-fisher}

Para dois grupos,

$$
\mathbf T_{\mathrm{entre}}
=
\frac{
n_1n_2
}{
n_1+n_2
}
\mathbf d\mathbf d^\top.
$$ {#eq-tentre-dois-grupos-fisher}

Como

$$
\mathbf d\mathbf d^\top
$$

possui posto no máximo $1$, segue imediatamente que

$$
\operatorname{posto}
\left(
\mathbf T_{\mathrm{entre}}
\right)
\leq1.
$$

Se

$$
\mathbf d=\mathbf0,
$$

então

$$
\mathbf T_{\mathrm{entre}}
=
\mathbf0,
$$

e não existe direção com separação positiva entre os centroides amostrais.

Considere, portanto,

$$
\mathbf d\neq\mathbf0
$$

e

$$
\mathbf T_{\mathrm{intra}}
\succ
\mathbf0.
$$

Substituindo @eq-tentre-dois-grupos-fisher em @eq-autovalor-generalizado-fisher,

$$
\frac{
n_1n_2
}{
n_1+n_2
}
\mathbf d
\mathbf d^\top
\mathbf a
=
\lambda
\mathbf T_{\mathrm{intra}}
\mathbf a.
$$

Para uma raiz positiva,

$$
\mathbf d^\top\mathbf a
\neq0,
$$

pois, caso contrário, o lado esquerdo seria nulo e, como

$$
\mathbf T_{\mathrm{intra}}
\succ\mathbf0,
$$

teríamos

$$
\lambda=0.
$$

Assim, para a direção associada à raiz positiva,

$$
\mathbf T_{\mathrm{intra}}
\mathbf a_1
\propto
\mathbf d.
$$

Como

$$
\mathbf T_{\mathrm{intra}}
$$

é invertível,

$$
\mathbf a_1
\propto
\mathbf T_{\mathrm{intra}}^{-1}
\left(
\overline{\mathbf x}_1
-
\overline{\mathbf x}_2
\right).
$$ {#eq-direcao-fisher-dois-grupos}

Essa expressão mostra por que a direção de Fisher não corresponde, em geral, simplesmente à reta que une os centroides.

O vetor

$$
\overline{\mathbf x}_1
-
\overline{\mathbf x}_2
$$

representa a separação bruta entre os grupos.

A multiplicação por

$$
\mathbf T_{\mathrm{intra}}^{-1}
$$

modifica essa direção de acordo com a dispersão e as associações existentes dentro dos grupos.

Em direções com grande dispersão intragrupos, uma determinada diferença entre os centroides recebe menor importância relativa. Em direções com pequena dispersão intragrupos, uma diferença de mesma magnitude pode assumir maior importância discriminante.

Esse é exatamente o significado geométrico do denominador do critério de Fisher.

### Formulação populacional para dois grupos

A mesma estrutura aparece no problema populacional.

Considere

$$
P(\mathsf G=1)=\pi_1,
\qquad
P(\mathsf G=2)=\pi_2,
$$

com

$$
\pi_1+\pi_2=1,
$$

e defina

$$
\mathbf d_\mu
=
\boldsymbol{\mu}_1
-
\boldsymbol{\mu}_2.
$$

A dispersão entre grupos de @eq-dispersao-populacional-entre-fisher reduz-se a

$$
\boldsymbol{\Sigma}_B
=
\pi_1\pi_2
\mathbf d_\mu
\mathbf d_\mu^\top.
$$

Se

$$
\boldsymbol{\Sigma}_W
\succ
\mathbf0
$$

e

$$
\boldsymbol{\mu}_1
\neq
\boldsymbol{\mu}_2,
$$

a direção populacional de Fisher satisfaz

$$
\mathbf a_F
\propto
\boldsymbol{\Sigma}_W^{-1}
\left(
\boldsymbol{\mu}_1
-
\boldsymbol{\mu}_2
\right).
$$

Essa expressão não exige normalidade. Ela decorre da estrutura de segundos momentos e do problema de Rayleigh generalizado.

## Relação entre Fisher e a discriminação linear normal

A formulação probabilística da LDA e a formulação geométrica de Fisher partem de problemas estatísticos distintos.

A relação entre elas torna-se particularmente simples no caso de dois grupos com covariância populacional comum.

Considere

$$
\boldsymbol{\mathsf X}
\mid
\mathsf G=g
\sim
N_p
\left(
\boldsymbol{\mu}_g,
\boldsymbol{\Sigma}
\right),
\qquad
g=1,2,
$$

com

$$
\boldsymbol{\Sigma}
\succ
\mathbf0.
$$

Na formulação probabilística, a fronteira da LDA sob perda zero--um possui vetor normal

$$
\boldsymbol{\Sigma}^{-1}
\left(
\boldsymbol{\mu}_1
-
\boldsymbol{\mu}_2
\right),
$$ {#eq-direcao-populacional-lda}

conforme @eq-normal-fronteira-lda.

Na formulação populacional de Fisher, a igualdade das covariâncias condicionais implica

$$
\boldsymbol{\Sigma}_W
=
\pi_1\boldsymbol{\Sigma}
+
\pi_2\boldsymbol{\Sigma}
=
\boldsymbol{\Sigma}.
$$

Portanto,

$$
\mathbf a_F
\propto
\boldsymbol{\Sigma}^{-1}
\left(
\boldsymbol{\mu}_1
-
\boldsymbol{\mu}_2
\right).
$$ {#eq-direcao-populacional-fisher}

Assim, sob covariância comum, a direção de Fisher e o vetor normal à fronteira da LDA são paralelos [@fisher1936; @anderson2003; @johnson2007].

Essa coincidência é uma equivalência de **direção**.

Ela não torna os dois problemas estatísticos idênticos.

Fisher resolve

$$
\max_{\mathbf a\neq\mathbf0}
\frac{
\mathbf a^\top
\boldsymbol{\Sigma}_B
\mathbf a
}{
\mathbf a^\top
\boldsymbol{\Sigma}_W
\mathbf a
},
$$

e obtém uma direção que maximiza a separação relativa entre as populações.

A LDA, por sua vez, resolve um problema de decisão. Sob perda zero--um, ela compara

$$
\delta_1(\mathbf x)
$$

e

$$
\delta_2(\mathbf x)
$$

para determinar em qual lado da fronteira uma nova observação será classificada.

A posição dessa fronteira depende também dos termos

$$
-\frac12
\boldsymbol{\mu}_g^\top
\boldsymbol{\Sigma}^{-1}
\boldsymbol{\mu}_g
$$

e

$$
\log\pi_g.
$$

As probabilidades *a priori* podem, portanto, deslocar a fronteira sem alterar sua orientação.

Se a função de perda não for zero--um, os custos associados às decisões também podem alterar a regra ótima, enquanto o critério geométrico de Fisher permanece inalterado.

Por isso,

> **direção discriminante e regra de classificação são objetos distintos.**

### Relação amostral

Na amostra, a matriz de covariâncias agrupada definida em @eq-estimativa-covariancia-agrupada-discriminante satisfaz

$$
\mathbf S_p
=
\frac{
1
}{
n-G
}
\mathbf T_{\mathrm{intra}}.
$$ {#eq-covariancia-agrupada-fisher}

Consequentemente,

$$
\mathbf S_p^{-1}
=
(n-G)
\mathbf T_{\mathrm{intra}}^{-1}.
$$

Para dois grupos,

$$
\mathbf S_p^{-1}
\left(
\overline{\mathbf x}_1
-
\overline{\mathbf x}_2
\right)
\propto
\mathbf T_{\mathrm{intra}}^{-1}
\left(
\overline{\mathbf x}_1
-
\overline{\mathbf x}_2
\right).
$$

Assim, quando a mesma matriz agrupada de covariâncias é utilizada, a orientação estimada da fronteira da LDA coincide, até escala, com a direção amostral de Fisher.

Novamente, a equivalência é da direção. A função discriminante estimada ainda contém

$$
\log\widehat{\pi}_g
$$

e os termos constantes necessários para definir a posição da fronteira e realizar a classificação.

### Mais de dois grupos

Quando

$$
G>2,
$$

a matriz

$$
\mathbf T_{\mathrm{entre}}
$$

pode possuir posto maior que $1$.

A formulação de Fisher fornece

$$
r_F
=
\operatorname{posto}
\left(
\mathbf T_{\mathrm{entre}}
\right)
\leq
\min(p,G-1)
$$

direções com raízes positivas.

A LDA, entretanto, não precisa projetar uma observação nessas direções para classificá-la. A regra populacional compara diretamente

$$
\delta_1(\mathbf x),
\ldots,
\delta_G(\mathbf x)
$$

no espaço original.

As direções de Fisher podem ser utilizadas para representação discriminante de baixa dimensão, enquanto a LDA define uma regra probabilística de decisão.

As duas construções compartilham a estrutura de médias e dispersão intragrupos, mas continuam respondendo a perguntas diferentes.

## Existência e singularidade

As formulações clássicas de Fisher, LDA e QDA utilizam inversas de matrizes de dispersão ou covariância.

A existência dessas inversas é uma questão algébrica que deve ser distinguida das hipóteses probabilísticas utilizadas para justificar os modelos de classificação.

### Formulação populacional de Fisher

O critério populacional em @eq-criterio-fisher-populacional-discriminante foi formulado supondo

$$
\boldsymbol{\Sigma}_W
\succ
\mathbf0.
$$

Essa condição garante

$$
\mathbf a^\top
\boldsymbol{\Sigma}_W
\mathbf a
>
0
$$

para todo

$$
\mathbf a\neq\mathbf0
$$

e permite utilizar

$$
\boldsymbol{\Sigma}_W^{-1}
\qquad\text{e}\qquad
\boldsymbol{\Sigma}_W^{-1/2}.
$$

A positividade definida de

$$
\boldsymbol{\Sigma}_W
$$

não exige normalidade.

Ela é uma condição sobre a estrutura de segundo momento dentro das classes.

### Formulação amostral de Fisher

Na formulação amostral clássica foi assumido

$$
\mathbf T_{\mathrm{intra}}
\succ
\mathbf0.
$$

Como

$$
\mathbf T_{\mathrm{intra}}
$$

é formada a partir de dados centralizados separadamente em cada um dos $G$ grupos, seu posto satisfaz

$$
\operatorname{posto}
\left(
\mathbf T_{\mathrm{intra}}
\right)
\leq
\min
\left(
p,
n-G
\right).
$$ {#eq-posto-tintra-discriminante}

Consequentemente, uma condição necessária para

$$
\mathbf T_{\mathrm{intra}}
$$

possuir posto completo é

$$
n-G
\geq
p.
$$ {#eq-condicao-dimensao-fisher}

Essa condição não é suficiente para um conjunto de dados arbitrário.

Mesmo quando

$$
n-G\geq p,
$$

relações lineares exatas entre as características dentro dos grupos podem tornar

$$
\mathbf T_{\mathrm{intra}}
$$

singular.

Quando

$$
p>n-G,
$$

a singularidade é inevitável.

Nesse caso, a formulação clássica baseada em

$$
\mathbf T_{\mathrm{intra}}^{-1}
$$

e

$$
\mathbf T_{\mathrm{intra}}^{-1/2}
$$

não está definida diretamente.

Pseudoinversas, regularização ou redução preliminar de dimensão constituem outras formulações e serão tratadas posteriormente.

### LDA estimada

Na regra populacional da LDA foi assumido

$$
\boldsymbol{\Sigma}
\succ
\mathbf0.
$$

Na regra *plug-in*, essa matriz é substituída por

$$
\mathbf S_p.
$$

Como

$$
\mathbf S_p
=
\frac1{n-G}
\mathbf T_{\mathrm{intra}},
$$

as duas matrizes possuem o mesmo posto.

Portanto,

$$
\mathbf S_p
$$

é invertível se, e somente se,

$$
\mathbf T_{\mathrm{intra}}
$$

é invertível.

Assim, a LDA estimada e a formulação amostral clássica de Fisher compartilham o mesmo problema de singularidade da dispersão intragrupos.

### QDA estimada

Na QDA, cada classe possui uma matriz de covariâncias própria.

A regra populacional clássica supõe

$$
\boldsymbol{\Sigma}_g
\succ
\mathbf0,
\qquad
g=1,\ldots,G.
$$

Na regra estimada, cada matriz é substituída por

$$
\mathbf S_g.
$$

Como

$$
\mathbf S_g
$$

é construída a partir de

$$
n_g
$$

observações centralizadas pelo próprio centroide,

$$
\operatorname{posto}
\left(
\mathbf S_g
\right)
\leq
\min
\left(
p,
n_g-1
\right).
$$

Logo,

$$
n_g-1
\geq
p
$$ {#eq-condicao-dimensao-qda}

é condição necessária para que a covariância amostral usual da classe $g$ possa possuir posto completo.

A QDA precisa satisfazer essa condição separadamente em cada grupo.

Por isso, sua formulação *plug-in* clássica exige mais informação por classe do que a LDA quando

$$
p
$$

é elevado.

## Identificação das direções discriminantes

As direções de Fisher apresentam as indeterminações próprias de um problema de autovalores generalizados.

Considere uma raiz positiva simples

$$
\lambda_j
$$

e um autovetor

$$
\mathbf a_j
$$

satisfazendo

$$
\mathbf T_{\mathrm{entre}}
\mathbf a_j
=
\lambda_j
\mathbf T_{\mathrm{intra}}
\mathbf a_j.
$$

Para qualquer

$$
c\neq0,
$$

o vetor

$$
c\mathbf a_j
$$

satisfaz a mesma equação.

Portanto, o comprimento do autovetor não é identificado pelo problema.

Com a normalização de @eq-normalizacao-fisher,

$$
\mathbf a_j^\top
\mathbf T_{\mathrm{intra}}
\mathbf a_j
=
1,
$$

a indeterminação de escala é eliminada, mas permanece a indeterminação de sinal:

$$
\mathbf a_j
\qquad\text{e}\qquad
-\mathbf a_j
$$

representam o mesmo eixo discriminante.

A mudança de sinal produz

$$
z_j(\mathbf x)
\longmapsto
-z_j(\mathbf x),
$$

mas não altera distâncias ao longo do eixo, separação dos centroides ou o subespaço representado.

Se uma raiz positiva possui multiplicidade maior que $1$, os autovetores individuais associados não são únicos.

Diferentes bases ortonormais segundo a métrica

$$
\mathbf T_{\mathrm{intra}}
$$

podem representar o mesmo autoespaço.

Nesse caso, o objeto identificado é o **subespaço discriminante associado à raiz repetida**, e não uma base particular.

Essa distinção deve ser preservada quando coordenadas discriminantes são interpretadas individualmente.

## Pressupostos e limites

A Análise Discriminante reúne três construções que não possuem exatamente os mesmos requisitos:

1. o critério geométrico de Fisher;
2. a regra normal linear;
3. a regra normal quadrática.

A @tbl-pressupostos-discriminante separa as principais condições.

| Condição | Fisher | LDA normal | QDA normal |
|---|---|---|---|
| grupos previamente definidos | necessária | necessária | necessária |
| segundos momentos finitos | necessários para a formulação populacional | satisfeitos sob normalidade | satisfeitos sob normalidade |
| normalidade multivariada | não necessária para definir Fisher | necessária para a derivação normal da regra apresentada | necessária para a derivação normal da regra apresentada |
| covariância comum entre grupos | não necessária | assumida | não assumida |
| matriz não singular na formulação clássica | $\boldsymbol{\Sigma}_W\succ0$ ou $\mathbf T_{\mathrm{intra}}\succ0$ | $\boldsymbol{\Sigma}\succ0$ e, na regra estimada, $\mathbf S_p$ invertível | $\boldsymbol{\Sigma}_g\succ0$ e, na regra estimada, $\mathbf S_g$ invertível |
| probabilidades *a priori* | não entram no critério | entram na regra de decisão | entram na regra de decisão |
| função de perda | não entra no critério | as funções apresentadas correspondem à perda zero--um | as funções apresentadas correspondem à perda zero--um |

: Condições associadas às formulações de Fisher, LDA e QDA. {#tbl-pressupostos-discriminante}

### Grupos previamente definidos

A Análise Discriminante pressupõe que os rótulos utilizados na construção da técnica possuem significado previamente estabelecido.

Na formulação de Fisher, esses rótulos determinam

$$
\mathbf T_{\mathrm{intra}}
$$

e

$$
\mathbf T_{\mathrm{entre}}.
$$

Na formulação probabilística, eles determinam quais observações caracterizam cada distribuição condicional.

A técnica não descobre automaticamente a existência de classes naturais.

Se os rótulos forem inadequados ao problema substantivo, uma solução matemática bem definida continuará descrevendo a separação ou classificação relativamente a esses rótulos.

### Normalidade e estrutura das covariâncias

A normalidade multivariada não é necessária para definir as dispersões populacionais de Fisher, o critério

$$
J_F^{(P)}(\mathbf a)
$$

ou sua versão amostral.

A normalidade entra nas formulações LDA e QDA apresentadas neste capítulo porque permite escrever explicitamente as densidades condicionais e derivar as regras de Bayes correspondentes.

Na LDA,

$$
\boldsymbol{\Sigma}_1
=
\cdots
=
\boldsymbol{\Sigma}_G
=
\boldsymbol{\Sigma}.
$$

É essa igualdade que faz com que os termos quadráticos em

$$
\mathbf x
$$

sejam comuns às classes e se cancelem na comparação.

Na QDA, as matrizes

$$
\boldsymbol{\Sigma}_g
$$

podem diferir e esse cancelamento não ocorre.

A distinção entre fronteiras lineares e quadráticas é, portanto, consequência direta da estrutura probabilística assumida.

### Independência das unidades da amostra de treinamento

Os estimadores usuais de médias e covariâncias apresentados neste capítulo pressupõem o esquema amostral clássico em que as unidades de treinamento fornecem informação independente sobre as populações consideradas.

Dependência temporal, espacial, longitudinal ou por agrupamentos pode modificar a estrutura amostral dos estimadores e não é incorporada automaticamente pelas fórmulas usuais de LDA e QDA.

Essa é uma questão da estrutura probabilística dos dados, e não do problema algébrico de Fisher isoladamente.

### Probabilidades *a priori* e perdas

As probabilidades

$$
\pi_g
$$

pertencem ao modelo de decisão.

Elas representam a ocorrência ou o peso prévio atribuído às classes antes da observação de

$$
\mathbf x.
$$

Sob perda zero--um, aparecem nas funções discriminantes por meio de

$$
\log\pi_g.
$$

A função de perda possui papel distinto.

Quando decisões erradas possuem consequências diferentes, a regra ótima é aquela que minimiza

$$
R(a\mid\mathbf x),
$$

e não necessariamente a classe de maior posterior.

Portanto:

- probabilidades *a priori*;
- probabilidades posteriores;
- função de perda;

pertencem ao problema de classificação.

Nenhum desses objetos participa do critério geométrico de Fisher.

### Sobreposição entre as populações

Mesmo com os parâmetros populacionais conhecidos e o modelo corretamente especificado, pode existir erro inevitável de classificação.

Se duas classes atribuem densidade positiva às mesmas regiões do espaço das características, então uma observação localizada nessas regiões não revela determinísticamente sua classe de origem.

A regra de Bayes escolhe a ação de menor risco condicional, mas o risco mínimo pode ser positivo.

Assim, erro de classificação não é necessariamente evidência de erro de estimação ou inadequação da técnica.

Parte dele pode resultar da própria sobreposição das distribuições populacionais.

A avaliação empírica do desempenho de uma regra estimada — incluindo validação cruzada, amostras de teste, matriz de confusão e medidas de desempenho — pertence à etapa metodológica posterior.

## Relações com outras técnicas

### MANOVA

A relação mais direta é com a MANOVA.

Na comparação de grupos,

$$
\mathbf H
=
\mathbf T_{\mathrm{entre}}
$$

e

$$
\mathbf E
=
\mathbf T_{\mathrm{intra}},
$$

conforme @eq-correspondencia-manova-fisher.

Portanto, o problema

$$
\mathbf H\mathbf a
=
\lambda
\mathbf E\mathbf a
$$

da MANOVA e o problema

$$
\mathbf T_{\mathrm{entre}}
\mathbf a
=
\lambda
\mathbf T_{\mathrm{intra}}
\mathbf a
$$

de Fisher são o mesmo problema de autovalores generalizados quando representam a mesma estrutura de grupos.

As raízes positivas são as mesmas, e os subespaços característicos são os mesmos.

O objetivo estatístico é diferente.

Na MANOVA, as raízes alimentam critérios globais destinados à inferência sobre diferenças entre grupos.

Em Fisher, os autovetores associados às raízes positivas são utilizados para construir direções de separação.

A formulação probabilística da Análise Discriminante acrescenta ainda um objeto que não é produzido pela MANOVA: uma regra de decisão para classificar novas observações.

A identidade da estrutura matricial não implica identidade entre as técnicas.

### Análise de Componentes Principais

ACP e Fisher constroem combinações lineares das características, mas otimizam quantidades diferentes.

Na ACP, uma direção é escolhida de acordo com a variabilidade total:

$$
\mathbf v^\top
\mathbf S
\mathbf v.
$$

Em Fisher, a direção é escolhida segundo

$$
\frac{
\mathbf a^\top
\mathbf T_{\mathrm{entre}}
\mathbf a
}{
\mathbf a^\top
\mathbf T_{\mathrm{intra}}
\mathbf a
}.
$$

A ACP não utiliza os rótulos dos grupos na construção das direções.

Fisher utiliza os rótulos para decompor a dispersão em componentes entre e dentro dos grupos.

Por isso, uma direção de grande variância total não precisa ser uma direção de boa separação entre classes.

A @tbl-acp-fisher sintetiza a diferença.

| Aspecto | ACP | Fisher |
|---|---|---|
| informação sobre grupos | não utilizada | utilizada |
| objeto matricial principal | covariância ou correlação total | dispersões entre e dentro dos grupos |
| critério | variância total | separação entre grupos relativa à dispersão intragrupos |
| tipo de representação | não supervisionada em relação aos grupos | supervisionada |
| dimensão máxima não nula | limitada pelo posto da matriz de covariâncias/dados | $\min(p,G-1)$ |
| interpretação das direções | variabilidade | separação relativa |

: Diferença entre as projeções da ACP e de Fisher. {#tbl-acp-fisher}

As duas técnicas utilizam projeções e decomposição espectral, mas a função objetivo determina o significado estatístico das direções.

### Análise de Agrupamentos

Análise Discriminante e Análise de Agrupamentos trabalham com estruturas de grupos, mas partem de informações diferentes.

Na Análise de Agrupamentos, os rótulos não são conhecidos e precisam ser construídos a partir das características observadas.

Na Análise Discriminante, os grupos já estão definidos e seus rótulos são utilizados para caracterizar as populações, construir dispersões e, quando apropriado, classificar novas observações.

Assim:

- Agrupamentos procura uma estrutura de partição;
- Discriminante estuda separação e classificação relativamente a classes conhecidas.

Centroides, distâncias e dispersões podem aparecer em ambas as técnicas, mas possuem funções diferentes dentro de cada problema.

### Correlação Canônica

Há também uma relação entre a Análise Discriminante e a Correlação Canônica.

Se a informação de classe for representada por um conjunto apropriado de variáveis indicadoras ou contrastes, a associação entre o bloco quantitativo

$$
\boldsymbol{\mathsf X}
$$

e o bloco que representa os grupos pode ser estudada por combinações lineares dos dois conjuntos.

Sob codificações e ponderações apropriadas, o subespaço obtido relaciona-se ao subespaço discriminante de Fisher.

Essa conexão não será derivada aqui porque a Correlação Canônica é o objeto do próximo capítulo.

Ela antecipa, entretanto, uma ideia importante: técnicas com objetivos aparentemente distintos podem conduzir a subespaços relacionados quando são formuladas a partir das mesmas estruturas de covariância e projeção.

## Síntese

A Análise Discriminante reúne uma formulação de decisão e uma formulação geométrica.

Na formulação probabilística, cada classe possui probabilidade *a priori*

$$
\pi_g
$$

e densidade condicional

$$
f_g(\mathbf x).
$$

A regra de Bayes minimiza o risco condicional

$$
R(a\mid\mathbf x).
$$

Sob perda zero--um, essa decisão equivale a maximizar a probabilidade posterior e, portanto,

$$
\pi_gf_g(\mathbf x).
$$

Para populações normais com covariância comum,

$$
\boldsymbol{\mathsf X}
\mid
\mathsf G=g
\sim
N_p
\left(
\boldsymbol{\mu}_g,
\boldsymbol{\Sigma}
\right),
$$

a comparação produz a função

$$
\delta_g(\mathbf x)
=
\mathbf x^\top
\boldsymbol{\Sigma}^{-1}
\boldsymbol{\mu}_g
-
\frac12
\boldsymbol{\mu}_g^\top
\boldsymbol{\Sigma}^{-1}
\boldsymbol{\mu}_g
+
\log\pi_g.
$$

As fronteiras são lineares porque os termos quadráticos comuns às classes se cancelam.

Quando as covariâncias são específicas,

$$
\boldsymbol{\Sigma}_g,
$$

esse cancelamento não ocorre e a função discriminante torna-se quadrática.

As regras populacionais utilizam parâmetros desconhecidos.

As regras *plug-in* substituem

$$
\boldsymbol{\mu}_g,
\qquad
\boldsymbol{\Sigma},
\qquad
\boldsymbol{\Sigma}_g
$$

por estimativas como

$$
\overline{\mathbf x}_g,
\qquad
\mathbf S_p,
\qquad
\mathbf S_g.
$$

Essa passagem distingue regra populacional, estimadores, estimativas e regra estimada.

Na formulação geométrica de Fisher, a dispersão populacional é decomposta em

$$
\operatorname{Cov}
\left(
\boldsymbol{\mathsf X}
\right)
=
\boldsymbol{\Sigma}_W
+
\boldsymbol{\Sigma}_B.
$$

A separação numa direção

$$
\mathbf a
$$

é medida relativamente à dispersão intragrupos por

$$
J_F^{(P)}
(\mathbf a)
=
\frac{
\mathbf a^\top
\boldsymbol{\Sigma}_B
\mathbf a
}{
\mathbf a^\top
\boldsymbol{\Sigma}_W
\mathbf a
}.
$$

Na amostra, o critério correspondente é

$$
J_F(\mathbf a)
=
\frac{
\mathbf a^\top
\mathbf T_{\mathrm{entre}}
\mathbf a
}{
\mathbf a^\top
\mathbf T_{\mathrm{intra}}
\mathbf a
}.
$$

Sua maximização conduz ao problema

$$
\mathbf T_{\mathrm{entre}}
\mathbf a
=
\lambda
\mathbf T_{\mathrm{intra}}
\mathbf a.
$$

Quando

$$
\mathbf T_{\mathrm{intra}}
\succ
\mathbf0,
$$

o problema é equivalente à decomposição espectral da matriz simétrica

$$
\mathbf T_{\mathrm{intra}}^{-1/2}
\mathbf T_{\mathrm{entre}}
\mathbf T_{\mathrm{intra}}^{-1/2}.
$$

Se

$$
r_F
=
\operatorname{posto}
\left(
\mathbf T_{\mathrm{entre}}
\right),
$$

existem exatamente

$$
r_F
$$

raízes positivas e

$$
r_F
\leq
\min(p,G-1).
$$

Os autovetores correspondentes definem as direções discriminantes.

Para dois grupos,

$$
\mathbf a_1
\propto
\mathbf T_{\mathrm{intra}}^{-1}
\left(
\overline{\mathbf x}_1
-
\overline{\mathbf x}_2
\right).
$$

No nível populacional, se as duas classes compartilham

$$
\boldsymbol{\Sigma},
$$

então

$$
\mathbf a_F
\propto
\boldsymbol{\Sigma}^{-1}
\left(
\boldsymbol{\mu}_1
-
\boldsymbol{\mu}_2
\right),
$$

que é também a orientação da fronteira da LDA.

Essa coincidência não transforma Fisher e LDA no mesmo problema.

Fisher constrói direções de separação.

A LDA constrói uma regra de decisão e incorpora também probabilidades *a priori* e, no problema geral de Bayes, uma função de perda.

A formulação clássica exige ainda condições de posto.

Para Fisher e LDA estimada,

$$
\operatorname{posto}
\left(
\mathbf T_{\mathrm{intra}}
\right)
\leq
\min(p,n-G),
$$

de modo que

$$
n-G\geq p
$$

é condição necessária para uma matriz intragrupos de posto completo.

Na QDA estimada,

$$
n_g-1\geq p
$$

é condição necessária em cada classe para que a covariância amostral usual possa ter posto completo.

A normalidade possui papel específico: ela não é necessária para definir Fisher, mas é utilizada para derivar as regras normais LDA e QDA apresentadas neste capítulo.

A Análise Discriminante reutiliza, assim, fundamentos dos Módulos 1 e 2 sobre esperança condicional, covariância, normal multivariada, teorema de Bayes, decisão estatística, distância de Mahalanobis, formas quadráticas, posto, raízes quadradas de matrizes positivas definidas, projeções e autovalores generalizados.

O capítulo mostra também por que uma mesma estrutura matemática pode sustentar técnicas diferentes. O problema espectral de Fisher coincide com aquele utilizado pela MANOVA na comparação de grupos, mas as raízes e autovetores recebem funções estatísticas distintas.

Ao final deste capítulo, o estudante deverá ser capaz de:

1. distinguir discriminação, representação discriminante e classificação;

2. formular a classificação como um problema de decisão envolvendo probabilidades *a priori*, densidades condicionais e perdas;

3. explicar por que, sob perda zero--um, a regra de Bayes seleciona a classe de maior probabilidade posterior;

4. derivar a função discriminante linear a partir da distribuição normal multivariada com covariância comum;

5. explicar por que a igualdade das covariâncias produz fronteiras lineares;

6. derivar a função discriminante quadrática quando as classes possuem covariâncias distintas;

7. interpretar o papel da distância de Mahalanobis nas regras normais de classificação;

8. distinguir parâmetros populacionais, estimadores, estimativas e regras discriminantes *plug-in*;

9. explicar por que as proporções amostrais dos grupos não são automaticamente probabilidades *a priori* populacionais;

10. decompor a covariância populacional em dispersão dentro e entre grupos;

11. formular os critérios populacional e amostral de Fisher;

12. explicar por que o critério de Fisher conduz a um problema de autovalores generalizados;

13. interpretar o branqueamento pela dispersão intragrupos e as raízes discriminantes;

14. justificar o limite

    $$
    r_F
    \leq
    \min(p,G-1);
    $$

15. distinguir raízes, direções e subespaços discriminantes, incluindo a não unicidade associada a raízes repetidas;

16. demonstrar, no caso de dois grupos, que

    $$
    \mathbf a_1
    \propto
    \mathbf T_{\mathrm{intra}}^{-1}
    \left(
    \overline{\mathbf x}_1
    -
    \overline{\mathbf x}_2
    \right);
    $$

17. explicar em quais condições Fisher e LDA possuem a mesma direção e por que isso não torna projeção e classificação problemas idênticos;

18. reconhecer as condições de posto e não singularidade necessárias às formulações clássicas de Fisher, LDA e QDA;

19. distinguir o papel da normalidade, da covariância comum, das probabilidades *a priori* e da função de perda nas diferentes formulações;

20. relacionar a Análise Discriminante com MANOVA, ACP, Análise de Agrupamentos e, conceitualmente, Correlação Canônica.

No próximo capítulo, a construção de combinações lineares será novamente central, mas os rótulos de grupos deixam de ser o objeto que organiza a análise.

Na **Correlação Canônica**, dois blocos de variáveis quantitativas serão considerados simultaneamente. O problema será construir uma combinação linear de cada bloco de modo que a correlação entre as duas combinações seja máxima.

A solução reutilizará matrizes de covariâncias particionadas, formas quadráticas, branqueamento, autovalores generalizados e SVD, permitindo integrar novamente os fundamentos dos Módulos 1 e 2 em uma nova pergunta estatística.

