...
O tratamento das matrizes de covariâncias e correlações distingue o nível populacional do nível descritivo dos dados observados. As propriedades inferenciais das estatísticas amostrais são tratadas no Capítulo 10. No Capítulo 9, a distribuição normal multivariada utiliza o vetor de médias e a matriz de covariâncias para especificar a distribuição conjunta.
...
e possui $r$ semieixos não nulos, com comprimentos $c\sqrt{\lambda_j}$ para os autovalores positivos. No Capítulo 9, a pseudoinversa de Moore--Penrose será utilizada para estender ao suporte efetivo a forma quadrática associada à distância de Mahalanobis quando $\boldsymbol{\Sigma}$ for singular.
...
A identidade @eq-esperanca-mahalanobis-quadratica exige apenas a existência de $\boldsymbol{\mu}$ e $\boldsymbol{\Sigma}\succ\mathbf 0$; a normalidade não é necessária. No Capítulo 9, a hipótese de normalidade multivariada permite caracterizar a distribuição completa da distância quadrática de Mahalanobis, da qual
...
No @sec-inferencia, uma estrutura correspondente será obtida a partir do modelo linear multivariado e de matrizes de projeção. Graus de liberdade, estimadores de covariâncias residuais e hipóteses lineares pertencem àquele contexto.
...
A variância generalizada não deve ser confundida com o volume do elipsoide de covariância. A relação entre as duas quantidades envolve a raiz quadrada do determinante [@johnson2002].
...
As esperanças das matrizes de dispersão por grupos fornecem uma interpretação probabilística adicional da decomposição da dispersão. Esses resultados não são necessários para a primeira leitura das decomposições algébricas e dos limites de posto.
:::
