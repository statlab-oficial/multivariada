...
- $\bar{\boldsymbol{\mathsf X}}$, $\boldsymbol{\mathsf S}$ e $\boldsymbol{\mathsf R}$ representam, respectivamente, a média amostral, a matriz de covariâncias amostral e a matriz de correlações amostral consideradas como objetos aleatórios; suas propriedades serão desenvolvidas no Capítulo 10;
...
O produto
$\boldsymbol{\mathsf X}\boldsymbol{\mathsf X}^{\top}$ é o produto externo definido em @def-produto-externo. Seus elementos são
...
A Figura @fig-covariancia-padroes apresenta configurações com covariâncias positiva, aproximadamente nula e negativa.
...
A matriz de covariâncias pertence à classe das matrizes simétricas positivas semidefinidas. A classificação de positividade de matrizes foi estabelecida em @def-classificacao-positividade; aqui ela adquire uma interpretação probabilística direta, pois a forma quadrática associada a $\boldsymbol{\Sigma}$ representa uma variância [@eaton2007multivariate].
...
A situação muda quando $\boldsymbol{\Sigma}$ é singular.
...
A última condição de @prp-singularidade-matriz-covariancias também pode ser escrita como
...
O símbolo $\boldsymbol{\mathcal R}$ é reservado à matriz populacional. Posteriormente, $\boldsymbol{\mathsf R}$ representará a matriz de correlações amostral como estatística aleatória, enquanto $\mathbf R$ representará seu valor calculado a partir de uma amostra observada.
...
Se $\boldsymbol{\Sigma}$ possui variâncias marginais positivas, a transformação
...
o resultado sobre os extremos do quociente de Rayleigh em @thm-extremos-quociente-rayleigh fornece
...
A Figura @fig-elipsoide-covariancia mostra essa geometria no caso bidimensional.
...
### Geometria, branqueamento e invariância
...
A raiz quadrada inversa de uma matriz positiva definida foi construída no Capítulo 5 a partir da raiz quadrada principal de @def-raiz-quadrada-principal. Aplicando-a à covariância, defina
...
também transforma a matriz de covariâncias em $\mathbf I_p$. As diferentes versões diferem por uma transformação ortogonal e, portanto, preservam as distâncias euclidianas no espaço branqueado.
...
que pode possuir elementos fora da diagonal. O branqueamento utiliza a matriz completa $\boldsymbol{\Sigma}^{-1/2}$ e produz covariância identidade.
...
são exatamente as fronteiras dos elipsoides definidos em @eq-elipsoide-covariancia.

A Figura @fig-distancia-mahalanobis compara dois deslocamentos com a mesma distância euclidiana ao centro, mas orientados segundo eixos de dispersão distintos.
...
Como ambas as distâncias são não negativas, segue @eq-invariancia-afim-mahalanobis.
:::
...
A definição em @def-distancia-mahalanobis utiliza $\boldsymbol{\Sigma}^{-1}$ e, portanto, exige
...
Quando $\boldsymbol{\Sigma}$ é singular, a pseudoinversa espectral definida em @def-pseudoinversa-espectral permite construir
...
Em todo o espaço $\mathbb{R}^p$, a expressão baseada em $\boldsymbol{\Sigma}^{+}$ é uma **pseudodistância**. Dois pontos distintos podem apresentar valor zero quando sua diferença pertence a
...
Como @prp-singularidade-matriz-covariancias mostrou que
...
quase certamente quando $\boldsymbol{\Sigma}$ é singular, esse subespaço contém a variabilidade efetivamente presente na população.
:::

A representação pela pseudoinversa também permite escrever o elipsoide degenerado de @eq-elipsoide-covariancia-singular como
...
Neste capítulo, a distância de Mahalanobis é uma construção geométrica baseada na estrutura de segunda ordem de $\boldsymbol{\mathsf X}$. No Capítulo 9, sob a distribuição normal multivariada, seu quadrado receberá uma distribuição probabilística específica.

Para dados observados, as estruturas populacionais $\boldsymbol{\Sigma}$ e $\boldsymbol{\mathcal R}$ serão representadas por matrizes de produtos cruzados, covariâncias e correlações calculadas a partir das observações.
...
### Centralização, SSCP e matriz de covariâncias observada
...
Se $\mathbf z_{\cdot j}$ representa a coluna $j$ de $\mathbf Z$, então
...
::: {.proof}
Para uma observação do grupo $g$,
...
Quando $\boldsymbol{\Sigma}\succ\mathbf 0$, a distância de Mahalanobis
