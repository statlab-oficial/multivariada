...
- $\mathbf{T}_{\mathrm{intra}}$ e $\mathbf{T}_{\mathrm{entre}}$ representam as parcelas intragrupos e entre grupos da dispersão total;
...
Os elementos da diagonal representam as variâncias das duas componentes, enquanto os elementos fora da diagonal representam sua covariância. O exemplo mostra como a retirada da parcela associada à localização transforma os segundos momentos em uma medida de dispersão em torno da média.
:::
...
A matriz $\boldsymbol{\Sigma}$ é singular porque uma combinação linear não trivial das três componentes é constante. Embora $\boldsymbol{\mathsf X}$ possua três componentes, sua variabilidade está restrita, após a centralização, a um subespaço de dimensão no máximo dois.
:::
...
### Traço e dispersão total

As matrizes $\mathbf T$, $\mathbf T_{\mathrm{intra}}$ e $\mathbf T_{\mathrm{entre}}$ descrevem a dispersão simultaneamente nas diferentes direções do espaço. Uma medida escalar da quantidade total de dispersão pode ser obtida por meio do traço dessas matrizes.
...
Portanto, $\operatorname{tr}(\mathbf T)$ é a soma dos quadrados das distâncias euclidianas das observações à média total.
...
\bar{\mathbf x}_g
\right\|^2
$$ {#eq-traco-dispersao-intragrupos}

mede a soma dos quadrados dos afastamentos das observações em relação aos centros de seus respectivos grupos, enquanto
...
\bar{\mathbf x}
\right\|^2
$$ {#eq-traco-dispersao-entre-grupos}
...
Dividindo por $n$, obtêm-se as médias dos quadrados desses afastamentos. Por exemplo,
...
representa a contribuição média da dispersão dentro dos grupos, enquanto
...
representa a contribuição média associada às diferenças entre os centros dos grupos.
...
O fator $n-G$ surge porque, dentro de cada grupo, a centralização pela média amostral reduz em uma unidade o número de direções independentes entre as observações. De fato, cada grupo contribui com $n_g-1$, e
...
separa a dispersão observada dentro dos grupos da dispersão associada às diferenças entre seus centros. Essa decomposição também pode ser expressa por meio do traço,
...
o que corresponde à decomposição da soma dos quadrados das distâncias das observações em relação aos respectivos centros.
...
| Dados agrupados | decomposição | $\mathbf T=\mathbf T_{\mathrm{intra}}+\mathbf T_{\mathrm{entre}}$ | separação da dispersão por grupos |
...
- interpretar, sob hipóteses probabilísticas apropriadas, as esperanças das matrizes de dispersão total, intragrupos e entre grupos.
