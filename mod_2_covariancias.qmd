# Matrizes de covariâncias e correlações {#sec-cov}
...
O tratamento das matrizes de covariâncias e correlações distingue o nível populacional do nível descritivo dos dados observados. As propriedades inferenciais das estatísticas amostrais são tratadas no @sec-amostrais. No @sec-normal, a distribuição normal multivariada utiliza o vetor de médias e a matriz de covariâncias para especificar a distribuição conjunta.
...
- $\bar{\boldsymbol{\mathsf X}}$, $\boldsymbol{\mathsf T}$, $\boldsymbol{\mathsf S}$ e $\boldsymbol{\mathsf R}$ representam, respectivamente, a média amostral, a matriz de somas de quadrados e produtos cruzados centralizados, a matriz de covariâncias amostral e a matriz de correlações amostral consideradas como objetos aleatórios; suas propriedades serão desenvolvidas no @sec-amostrais;
...
- $QM_{\mathrm{intra}}=SQ_{\mathrm{intra}}/(n-K)$ e $QM_{\mathrm{entre}}=SQ_{\mathrm{entre}}/(K-1)$ representam os quadrados médios intragrupos e entre grupos no caso escalar;
...
com realização $\mathbf{x}\in\mathbb{R}^p$. Quando $n$ realizações são observadas, sua organização em uma matriz $\mathbf{X}\in\mathbb{R}^{n\times p}$ segue a representação estabelecida na @def-matriz-dados.
...
Se $X$ e $Y$ são variáveis aleatórias com segundos momentos finitos e médias $\mu_X=E(X)$ e $\mu_Y=E(Y)$, a **covariância populacional** entre $X$ e $Y$ é
...
Reciprocamente, suponha que $\mathbf C$ seja real, simétrica e positiva semidefinida. Pelo @thm-teorema-espectral e pela raiz quadrada principal definida na @def-raiz-quadrada-principal, existe uma matriz simétrica positiva semidefinida $\mathbf C^{1/2}$ tal que
...
recupera, no nível matricial, o limite apresentado na @eq-limite-covariancia.
...
Seja $\boldsymbol{\mathsf X}\in\mathbb{R}^p$ um vetor aleatório com média $\boldsymbol{\mu}$ e matriz de covariâncias $\boldsymbol{\Sigma}$. As seguintes afirmações são equivalentes:
...
Sejam $\boldsymbol{\mathsf X}\in\mathbb R^p$ e $\boldsymbol{\mathsf Y}\in\mathbb R^q$ vetores aleatórios com matriz de covariâncias cruzadas $\boldsymbol{\Sigma}_{XY}$. Para quaisquer vetores fixos
...
não implica, em uma distribuição geral, independência entre os vetores. Essa relação será retomada no @sec-normal sob a distribuição normal multivariada.
...
e sua matriz de covariâncias pode ser representada por blocos, conforme a estrutura de matrizes particionadas definida na @def-matriz-particionada:
...
O complemento de Schur já foi definido na @def-complemento-schur. Aplicado à matriz de covariâncias particionada, ele recebe uma interpretação estatística de segunda ordem.
...
Pelo resultado de positividade do complemento de Schur no @thm-positividade-complemento-schur,
...
Essa interpretação depende apenas dos segundos momentos e não exige normalidade. Sob normalidade conjunta, o mesmo complemento de Schur aparecerá no @sec-normal como matriz de covariâncias da distribuição condicional de um bloco dado o outro.
...
As transformações afins foram definidas na @def-transformacao-afim. No contexto probabilístico, interessa determinar como elas modificam a média e a matriz de covariâncias.
...
Seja $\boldsymbol{\mathsf X}\in\mathbb R^p$ um vetor aleatório com média $\boldsymbol{\mu}_X$ e matriz de covariâncias $\boldsymbol{\Sigma}_{XX}$. Para
...
A ausência do vetor $\mathbf c$ na @eq-covariancia-transformacao-afim confirma que translações alteram a localização, mas não a matriz de covariâncias.
...
Sejam $\boldsymbol{\mathsf X}\in\mathbb R^p$ e $\boldsymbol{\mathsf Y}\in\mathbb R^q$ vetores aleatórios com matriz de covariâncias cruzadas $\boldsymbol{\Sigma}_{XY}$. Considere
...
Se $\boldsymbol{\mathsf X}\in\mathbb R^p$ possui média $\boldsymbol{\mu}$ e variâncias marginais positivas, o vetor aleatório marginalmente padronizado correspondente é
...
A **matriz de correlações populacional** de $\boldsymbol{\mathsf X}\in\mathbb R^p$ é
...
Reciprocamente, suponha que $\mathbf C$ seja simétrica, positiva semidefinida e possua diagonal unitária. Pela raiz quadrada principal definida na @def-raiz-quadrada-principal, existe $\mathbf C^{1/2}$ tal que
...
Essa condição não implica independência em uma distribuição geral, conforme ilustrado no @exm-dependencia-covariancia-nula.
...
Nesse caso, todas as componentes são duas a duas não correlacionadas. Essa propriedade não garante, por si só, independência conjunta. A relação especial entre ausência de correlação e independência sob normalidade conjunta será estabelecida no @sec-normal.
...
Esse é precisamente o tipo de problema de otimização com restrição estudado no Módulo 1. Como maximizar uma função equivale a minimizar seu negativo, a condição necessária de Lagrange estabelecida no @thm-multiplicadores-lagrange aplica-se também a este problema. Considere
...
Pelo resultado dos extremos do quociente de Rayleigh no @thm-extremos-quociente-rayleigh,
...
As direções espectrais obtidas acima caracterizam a geometria da matriz de covariâncias. Na Análise de Componentes Principais, essas direções também determinam uma representação de dimensão reduzida.
:::
...
A raiz quadrada principal definida na @def-raiz-quadrada-principal permite representar o elipsoide degenerado por
...
e possui $r$ semieixos não nulos, com comprimentos $c\sqrt{\lambda_j}$ para os autovalores positivos. No @sec-normal, a pseudoinversa de Moore--Penrose será utilizada para estender ao suporte efetivo a forma quadrática associada à distância de Mahalanobis quando $\boldsymbol{\Sigma}$ for singular.
...
Se $\boldsymbol{\mathsf X}\in\mathbb R^p$ possui matriz de covariâncias $\boldsymbol{\Sigma}$, sua **variância total** é
...
Pela definição de traço na @def-traco,
...
Se $\boldsymbol{\mathsf X}\in\mathbb R^p$ possui matriz de covariâncias $\boldsymbol{\Sigma}$, sua **variância generalizada** é
...
Se $\boldsymbol{\Sigma}\in\mathbb R^{p\times p}$ é positiva definida, a **distância de Mahalanobis** entre $\mathbf x,\mathbf y\in\mathbb{R}^p$, em relação a $\boldsymbol{\Sigma}$, é
...
são exatamente as fronteiras dos elipsoides definidos na @eq-elipsoide-covariancia. Isso reforça a interpretação geométrica da distância de Mahalanobis: pontos pertencentes à mesma superfície elipsoidal apresentam o mesmo afastamento em relação ao centro quando a estrutura de covariâncias é considerada.
...
A identidade @eq-esperanca-mahalanobis-quadratica exige apenas a existência de $\boldsymbol{\mu}$ e $\boldsymbol{\Sigma}\succ\mathbf 0$; a normalidade não é necessária. No @sec-normal, a hipótese de normalidade multivariada permite caracterizar a distribuição completa da distância quadrática de Mahalanobis, da qual
$E[d_M^2(\boldsymbol{\mathsf X},\boldsymbol{\mu})]=p$
é uma consequência.
...
As propriedades dessas estatísticas como estimadores serão desenvolvidas no @sec-amostrais.
...
A operação remove a localização das colunas sem alterar as diferenças entre as observações, conforme a propriedade estabelecida na @prp-centralizacao-preserva-distancias.
...
ela é uma matriz de Gram das colunas centralizadas no sentido de @def-matriz-gram. Assim, pelas propriedades estabelecidas na @prp-propriedades-matriz-gram,
...
Essa singularidade decorre da dimensão dos dados centralizados. Suas consequências para estimação e regularização serão tratadas no @sec-amostrais.
...
O divisor $n-1$ é utilizado aqui como convenção para a matriz de covariâncias observada. Sua relação com graus de liberdade, ausência de viés e máxima verossimilhança será estabelecida no @sec-amostrais.
...
Nesse ponto, $\bar{\mathbf x}$, $\mathbf S$ e $\mathbf R$ são valores calculados a partir da matriz de dados observada. Sua interpretação como realizações de estatísticas amostrais e suas propriedades inferenciais serão desenvolvidas no @sec-amostrais.
:::
...
::: {.callout-note collapse="true" appearance="simple" title="Demonstração"}
Pela decomposição vetorial apresentada na @eq-decomposicao-vetor-desvio-grupos,
...
A identidade @eq-decomposicao-dispersao-grupos é algébrica e depende somente da partição das observações e das definições das médias total e de grupo [@johnson2002]. Normalidade, independência e outros pressupostos probabilísticos não são necessários para estabelecê-la.
...
As propriedades estruturais a seguir decorrem diretamente das definições das matrizes de dispersão e não dependem de hipóteses probabilísticas sobre as observações.
...
O limite para $\mathbf T$ foi obtido na @eq-posto-sscp-total.
...
separa algebricamente a dispersão total observada nas parcelas intragrupos e entre grupos. Essa decomposição, juntamente com as propriedades de positividade e os limites de posto das três matrizes, não depende de hipóteses probabilísticas sobre as observações.

Sob independência entre as observações e matriz de covariâncias comum aos grupos, as mesmas matrizes também admitem uma interpretação probabilística por meio de suas esperanças. A formulação por matrizes de projeção, os estimadores de covariâncias residuais e as hipóteses lineares são tratados no @sec-inferencia.

## Aprofundamentos teóricos {#sec-covariancias-aprofundamentos-teoricos}

### Variância média e interpretação volumétrica da variância generalizada

A variância total e a variância generalizada resumem aspectos distintos da dispersão por meio do traço e do determinante de $\boldsymbol{\Sigma}$. A variância média reescala a variância total pelo número de componentes, enquanto a raiz quadrada da variância generalizada determina a escala de volume do elipsoide de covariância quando $\boldsymbol{\Sigma}\succ\mathbf 0$.

::: {#def-variancia-media}
## Variância média

Se $\boldsymbol{\mathsf X}\in\mathbb R^p$ possui matriz de covariâncias $\boldsymbol{\Sigma}$, com autovalores $\lambda_1,\ldots,\lambda_p$, sua **variância média** é
...
A variância generalizada definida em @def-variancia-generalizada não deve ser confundida com o volume do elipsoide de covariância de @eq-elipsoide-covariancia. Para $\boldsymbol{\Sigma}\succ\mathbf 0$, a relação entre essas quantidades envolve a raiz quadrada do determinante [@johnson2002].
...
Essa expressão é compatível com o fator de alteração de volume desenvolvido na @prp-determinante-volume-transformacao.
...
é uma identidade algébrica para os dados observados. Sob uma estrutura probabilística para as observações, as matrizes de dispersão tornam-se objetos aleatórios e suas esperanças permitem separar a variabilidade comum aos grupos da dispersão associada às diferenças entre seus vetores de médias populacionais.

Suponha que todos os vetores aleatórios
...
sejam mutuamente independentes, com
...
Tomando o traço na @eq-esperanca-dispersao-entre-grupos,
...
A @tbl-sintese-covariancias reúne os principais objetos do capítulo.
