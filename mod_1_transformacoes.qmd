# Transformações lineares {#sec-cap-transformacoes}

Vetores, matrizes, espaços vetoriais, bases e projeções fornecem a linguagem algébrica e geométrica para representar observações multivariadas e relações lineares entre suas coordenadas.

Uma matriz $\mathbf{A}$ também define uma aplicação $\mathbf{x}\mapsto\mathbf{A}\mathbf{x}$. Dependendo de $\mathbf{A}$, a aplicação pode alterar orientação, escala ou dimensão. Quando preserva combinações lineares, constitui uma **transformação linear**.

Para $T_{\mathbf{A}}(\mathbf{x})=\mathbf{A}\mathbf{x}$, o espaço nulo de $\mathbf{A}$ é o núcleo da transformação e o espaço coluna de $\mathbf{A}$ é sua imagem. Transformações lineares e afins também descrevem operações sobre matrizes de dados, composição, invertibilidade, perda de dimensão, mudanças de coordenadas, centralização, escalonamento e padronização.

::: {.callout-note appearance="simple" icon="false"}

## Convenções para transformações lineares

Além das convenções vetoriais e matriciais já adotadas, usam-se:
...
::: {#prp-transformacao-determinada-base}
...
::: {#exm-transformacao-determinada-base}
## Transformação determinada pelas imagens de uma base
...
Logo, existe uma matriz que representa $T$ nas bases canônicas.
...
::: {#prp-representacao-matricial-transformacao}
...
A matriz $\mathbf{A}$ nessa representação é relativa às bases canônicas do domínio e do contradomínio.
...
Para $T_{\mathbf A}(\mathbf{x})=\mathbf A\mathbf{x}$, o **espaço nulo** e o **espaço coluna** de $\mathbf A$ correspondem, respectivamente, ao **núcleo** e à **imagem** da transformação:
...
A segunda direção é eliminada, a imagem da transformação possui dimensão 1 e $\mathbf{e}_2$ pertence ao núcleo de $T_{\mathbf A}$.
...
A identidade $\mathbf{Y}=\mathbf{X}\mathbf{A}^{\top}$ admite duas leituras:
...
O domínio e o contradomínio possuem a mesma dimensão. A dimensão efetiva da imagem, entretanto, pode ser menor que $p$ quando $\mathbf{A}$ não possui posto completo.
...
Quando $\mathbf{X}$ está centralizada, $\mathbf{X}^{\top}\mathbf{X}$ reúne os produtos cruzados utilizados na matriz de covariâncias amostrais. A transformação $\mathbf{Y}=\mathbf{X}\mathbf{A}^{\top}$ modifica esses produtos segundo

$$
\mathbf{Y}^{\top}\mathbf{Y}
=
\mathbf{A}\mathbf{X}^{\top}\mathbf{X}\mathbf{A}^{\top},
$$

enquanto os produtos internos entre as observações transformadas são descritos por

$$
\mathbf{Y}\mathbf{Y}^{\top}
=
\mathbf{X}\mathbf{A}^{\top}\mathbf{A}\mathbf{X}^{\top}.
$$
...
Como a multiplicação matricial não é comutativa, a composição depende da ordem em que as transformações são aplicadas. Para duas transformações que atuam sobre o mesmo espaço,
...
::: {#prp-caracterizacoes-transformacao-linear}
...
Combinando o @prp-caracterizacoes-transformacao-linear com o @thm-equivalencias-invertibilidade, obtêm-se equivalências entre invertibilidade, núcleo, imagem, posto e determinante.

::: {#prp-equivalencias-transformacao-invertivel}
## Equivalências para uma transformação linear associada a uma matriz quadrada
...
7. $\operatorname{posto}(\mathbf{A})=p$;
8. $\det(\mathbf{A})\neq0$;
9. $\mathbf{A}^{-1}$ existe.
...
Em técnicas de redução dimensional, a matriz é escolhida para preservar determinadas direções e eliminar outras.
...
como a dimensão da imagem da transformação, isto é, como o número máximo de direções linearmente independentes que podem compor o espaço transformado.
...
As cinco transformações consideradas no plano são:
...
representa a reflexão em relação ao hiperplano ortogonal a $\mathbf{u}$. Essa transformação é denominada **reflexão de Householder**. [@gentle2017matrix]
...
Assim, o cisalhamento preserva áreas e volumes, mas altera comprimentos e ângulos. Retas e paralelismo são preservados. [@gentle2017matrix]

### Projeção

A projeção ortogonal definida em @def-projecao-ortogonal também pode ser interpretada como uma transformação linear.
...
Pelas propriedades das matrizes de projeção em @prp-propriedades-matriz-projecao,
...
A preservação de área ou volume não implica preservação de distâncias e ângulos. O cisalhamento possui determinante igual a 1, mas deforma a configuração. Transformações ortogonais são caracterizadas pela preservação da geometria euclidiana.
...
Na primeira interpretação, a matriz atua sobre o vetor e modifica sua posição no espaço. Na segunda, seus vetores-coluna definem um sistema de coordenadas. Para uma base ortonormal formada pelas colunas de $\mathbf{Q}$, as coordenadas de $\mathbf{x}$ são $\mathbf{Q}^{\top}\mathbf{x}$.
...
Pela @def-coordenadas-base, um mesmo vetor pode ter coordenadas distintas em bases distintas, embora o vetor geométrico permaneça o mesmo. Seja
...
uma base de $\mathbb{R}^p$, com a ordem dos vetores fixada. A matriz formada pelos vetores da base é
...
representado na base canônica por $\mathbf{A}$, e seja $\mathcal{B}$ uma base de $\mathbb{R}^p$ com matriz $\mathbf{B}$. Como

$$
\mathbf{x}
=
\mathbf{B}[\mathbf{x}]_{\mathcal{B}},
$$
...
$$
\begin{aligned}
[T(\mathbf{x})]_{\mathcal{B}}
&=
\mathbf{B}^{-1}T(\mathbf{x})\\
&=
\mathbf{B}^{-1}\mathbf{A}\mathbf{x}\\
&=
\mathbf{B}^{-1}\mathbf{A}\mathbf{B}[\mathbf{x}]_{\mathcal{B}}.
\end{aligned}
$$
...
Matrizes semelhantes representam o mesmo operador em bases diferentes e possuem os mesmos autovalores, propriedade utilizada na diagonalização.
...
Para a mesma matriz ortogonal $\mathbf{Q}$, a mudança de coordenadas usa $\mathbf{Z}=\mathbf{X}\mathbf{Q}$, enquanto a transformação ativa usa $\mathbf{Y}=\mathbf{X}\mathbf{Q}^{\top}$; a distinção é detalhada em @sec-transformacao-ativa-passiva.
...
Se uma aplicação possui a forma $T(\mathbf{x})=\mathbf{A}\mathbf{x}+\mathbf{b}$ com $\mathbf{b}\neq\mathbf{0}$, então $T(\mathbf{0})=\mathbf{b}\neq\mathbf{0}$. Essas aplicações são transformações afins.
...
A padronização combina centralização e escalonamento. [@gentle2017matrix]
...
A @tbl-lineares-afins compara transformações lineares, transformações afins, centralização, escalonamento e padronização quanto à linearidade e à preservação da origem.
...
## Aprofundamento teórico {#sec-transformacoes-aprofundamento-teorico}
...
::: {#prp-determinante-volume-transformacao}
...
No plano, o @prp-determinante-volume-transformacao descreve a alteração de área; em $\mathbb{R}^3$, a alteração de volume; em dimensão $p$, a alteração do volume $p$-dimensional.

Quando $\det(\mathbf{A})\neq0$, o sinal do determinante determina a orientação:
...
### Determinante e orientação em transformações ortogonais
...
A sequência das operações, da direita para a esquerda, é:
...
A fórmula $[T]_{\mathcal{C}\leftarrow\mathcal{B}}=\mathbf{C}^{-1}\mathbf{A}\mathbf{B}$ generaliza a representação matricial nas bases canônicas.
...
### Transformação ativa e mudança passiva {#sec-transformacao-ativa-passiva}
...
A mudança passiva $\mathbf{Z}=\mathbf{X}\mathbf{Q}$ deve ser distinguida da transformação ativa por $\mathbf{Q}$, que produz
...
A linearidade implica, entre outras consequências,

$$T(\mathbf{0})=\mathbf{0},$$

e determina a transformação linear a partir de sua ação sobre uma base do domínio.
...
Para $T_{\mathbf{A}}(\mathbf{x})=\mathbf{A}\mathbf{x}$, o núcleo e a imagem coincidem com os espaços fundamentais associados a $\mathbf{A}$:
...
A matriz situada à direita atua primeiro. Como o produto matricial não é comutativo, inverter a ordem pode produzir uma transformação composta diferente.

Rotações e reflexões são transformações ortogonais e preservam a geometria euclidiana. Escalonamentos alteram as escalas das direções coordenadas; cisalhamentos modificam, em geral, ângulos e comprimentos; projeções eliminam componentes pertencentes ao complemento do subespaço sobre o qual se projeta.
...
Uma matriz ortogonal pode, portanto, ser usada como matriz de uma base ortonormal.
...
Se um operador é representado na base canônica por $\mathbf{A}$ e $\mathbf{B}$ é a matriz de uma base de $\mathbb{R}^p$, sua representação nessa base é
...
As matrizes $\mathbf{A}$ e $\mathbf{A}_{\mathcal{B}}$ são semelhantes e representam o mesmo operador em sistemas de coordenadas distintos.

Quando as colunas de uma matriz ortogonal $\mathbf{Q}$ formam uma base ortonormal e as observações estão nas linhas de $\mathbf{X}$, a matriz de coordenadas nessa base é
...
A @tbl-sintese-transformacoes reúne as expressões centrais para transformações lineares, ortogonais e afins e para mudanças de coordenadas.
...
| Reflexão | $\mathbf{F}\mathbf{x}$ | Preserva a geometria e produz uma imagem especular |
...
| Operador em uma nova base | $\mathbf{A}_{\mathcal{B}}=\mathbf{B}^{-1}\mathbf{A}\mathbf{B}$ | Representa o mesmo operador em outro sistema de coordenadas |
| Coordenadas dos dados em base ortonormal | $\mathbf{Z}=\mathbf{X}\mathbf{Q}$ | Expressa as observações nas coordenadas da base formada pelas colunas de $\mathbf{Q}$ |
...
: Expressões centrais para transformações e mudanças de coordenadas. {#tbl-sintese-transformacoes}
...
- representar o mesmo operador linear em uma nova base e reconhecer matrizes semelhantes;
- obter as coordenadas de uma matriz de dados em uma base ortonormal;
...
Uma direção não nula $\mathbf{v}$ que satisfaz
...
é preservada pela transformação, exceto pelo fator de escala $\lambda$. Essa condição introduz os conceitos de autovetores, autovalores, autoespaços e decomposição espectral.
