...
A Análise de Agrupamentos organiza objetos em grupos a partir de uma representação dos dados, de uma medida de proximidade e de um critério que define a organização procurada [@johnson2007; @everitt2011cluster].
...
A propriedade de que esses centroides minimizam o critério para uma partição fixada decorre de @prp-centroide-media-amostral. Sua aplicação à notação dos grupos utilizada neste capítulo é registrada em @prp-centroide-minimiza-soma-quadrados.
...
A caracterização geral da média amostral como centroide foi estabelecida em @prp-centroide-media-amostral. A identidade de decomposição correspondente foi obtida em @eq-decomposicao-centroide-media.
...
Considere um grupo não vazio $C$ com
...
Aplicando @eq-decomposicao-centroide-media às observações do grupo $C$, para qualquer
...
A proposição é, portanto, a aplicação de @prp-centroide-media-amostral a cada subconjunto de observações definido pela partição.
...
A identidade geral de decomposição em torno do centroide, estabelecida em @eq-decomposicao-centroide-media e especializada em @eq-decomposicao-centroide, pode ser aplicada separadamente aos grupos $A$ e $B$.
...
A solução também depende das escolhas que definem o problema. A @tbl-dependencia-solucao-agrupamento resume essas dependências.
...
O centroide aparece porque a média minimiza a soma das distâncias euclidianas quadráticas às observações do grupo. A decomposição
...
A solução depende das variáveis, das escalas, da medida de proximidade, do número de grupos, da função objetivo, da regra de ligação ou do modelo adotado. Empates e simetrias também podem produzir não unicidade. Por isso, a organização encontrada deve ser interpretada relativamente à estrutura matemática ou probabilística que a definiu e não implica, por si só, a existência de classes substantivas externas aos dados.
