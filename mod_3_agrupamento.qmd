...
A formulação euclidiana desenvolvida no núcleo do capítulo é adequada para variáveis quantitativas. Quando os objetos são descritos por atributos de naturezas diferentes, a construção da dissimilaridade precisa considerar o tipo de cada variável.
:::
...
### Aprofundamento: escolha do número de grupos {#sec-escolha-k-agrupamentos}

::: {.callout-note appearance="simple" icon=false}
## Leitura de aprofundamento

A escolha do número de grupos acrescenta uma etapa de decisão à formulação das $K$-médias. Diferentes critérios podem ser utilizados para comparar soluções obtidas com valores distintos de $K$, mas nenhum deles estabelece, isoladamente, a existência de um número verdadeiro de grupos.
:::
...
### Aprofundamento: convenções computacionais para o método de Ward

::: {.callout-note appearance="simple" icon=false}
## Leitura de aprofundamento

A formulação estatística de Ward é definida pelo aumento da dispersão intragrupos. Algumas implementações utilizam convenções distintas para representar computacionalmente esse critério.
:::
...
Critérios utilizados para auxiliar essa escolha são apresentados como aprofundamento na @sec-escolha-k-agrupamentos.
...
As opções de agrupamento hierárquico correspondem às medidas de proximidade e às regras de ligação apresentadas na @sec-agrupamentos-hierarquicos. Particularidades computacionais do método de Ward são discutidas na leitura de aprofundamento associada à @sec-agrupamentos-ward.
...
Os critérios auxiliares para escolha do número de grupos são apresentados como aprofundamento na @sec-escolha-k-agrupamentos. Procedimentos adicionais de validação, reamostragem e diagnóstico pertencem à etapa aplicada.
:::
