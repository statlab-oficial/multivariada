...
Na @sec-acp, a ACP produz novas coordenadas para as observações. Quando todas as componentes de uma transformação ortogonal são mantidas, as distâncias euclidianas são preservadas por @prp-preservacao-geometria-transformacao-ortogonal.
...
Na Análise de Agrupamentos, os grupos fazem parte da solução. Na @sec-discriminante, os grupos são conhecidos previamente e fazem parte da informação utilizada para construir direções de separação ou regras de classificação.
...
A conexão com a @sec-manova aparece na decomposição
...
Nas atividades computacionais, a Análise de Agrupamentos será realizada pelo módulo snowCluster do jamovi. As opções disponíveis correspondem a diferentes formulações e também a diferentes procedimentos computacionais, de modo que os nomes apresentados na interface devem ser relacionados ao critério estatístico efetivamente utilizado.
...
No próximo capítulo, o tipo de dado e o problema estatístico mudam. A @sec-correspondencia parte de tabelas de contingência e utiliza perfis, massas, distância qui-quadrado e decomposição em valores singulares para representar associações entre categorias.