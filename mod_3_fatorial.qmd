...
Apesar da proximidade terminológica,

$$
\text{fatores ou eixos principais}
\neq
\text{componentes principais}.
$$
...
O escore por regressão não é a única forma de atribuir valores fatoriais às unidades observacionais. Métodos alternativos utilizam critérios distintos e podem produzir escores diferentes para a mesma solução fatorial.

O escore por regressão não é a única forma de atribuir coordenadas fatoriais às unidades observacionais. Como os fatores são latentes, diferentes critérios podem produzir estimativas distintas mesmo quando a matriz de cargas e as demais características do modelo permanecem fixadas.
...
Alguns métodos usuais são sintetizados a seguir.
...
| **ten Berge** | Procura preservar, nos escores estimados, a estrutura de correlações especificada para os fatores |
| **Anderson--Rubin** | Constrói escores não correlacionados e é particularmente associado a soluções com fatores ortogonais |
| **Harman** | Baseia a construção dos escores em variáveis idealizadas associadas à parte comum do modelo |
...
A existência de métodos distintos reforça a separação conceitual entre o fator latente e o escore fatorial estimado. O fator pertence ao modelo probabilístico, enquanto o escore é uma estimativa construída posteriormente. Por isso, a interpretação substantiva dos fatores não deve depender exclusivamente dos valores obtidos por um único método de escores. As derivações completas dos demais critérios e suas propriedades comparativas estão além do escopo básico deste capítulo.


::: {.callout-note appearance="simple" icon=false}
## Correspondência com a implementação utilizada

Nas atividades computacionais, a Análise Fatorial Exploratória será realizada pela análise `EFA` do núcleo do [jamovi](https://www.jamovi.org/) [@jamovi2026]. Os métodos de extração *Minimum residual* (MINRES), *Maximum likelihood* e *Principal axis* correspondem, respectivamente, às formulações por mínimos resíduos, máxima verossimilhança e fatores ou eixos principais discutidas na @sec-fatorial-estimacao.
