...
Esta parte reúne desenvolvimentos sobre identificação, invariâncias e critérios de rotação, fatores principais e mínimos resíduos, formulação probabilística, máxima verossimilhança, distribuições associadas à avaliação da matriz de correlações e propriedades dos métodos de escores fatoriais. Esses resultados ampliam a formulação apresentada no núcleo do capítulo, mas não são necessários para uma primeira leitura da técnica.
:::
...
A correção melhora a aproximação qui-quadrado, mas não transforma a estatística em uma variável com distribuição qui-quadrado exata em amostras finitas [@bartlett1950factor; @johnson2007].
...
O método de Bartlett trata $\mathbf f$ como o vetor de valores fatoriais a ser estimado e utiliza as variâncias específicas para ponderar os resíduos [@bartlett1937factors; @johnson2007].
...
|---|---:|---|
| **Elementos fora da diagonal da anti-imagem** | valores próximos de $0$ são favoráveis | pequena associação parcial residual |
| **$\operatorname{MSA}_i$** | $0{,}50$ como referência operacional mínima | valores menores indicam que a variável deve ser examinada antes de mantê-la na solução |
...
O **gráfico de declive** (*scree plot*) apresenta uma sequência de raízes em ordem decrescente. Uma mudança acentuada no padrão de queda pode indicar uma região a partir da qual as raízes restantes se tornam relativamente pequenas. Sua interpretação depende das raízes utilizadas na construção do gráfico [@hair2019multivariate].
...
Não existe um critério de rotação que seja superior em todas as aplicações [@hair2019multivariate]. A escolha entre rotação ortogonal e oblíqua depende da estrutura que se pretende admitir para os fatores. A rotação ortogonal fixa suas correlações em zero; a rotação oblíqua permite correlações fatoriais diferentes de zero, representadas por $\boldsymbol{\Omega}$ na @sec-fatorial-rotacao.
...
O escore por regressão não é a única forma de atribuir coordenadas fatoriais às unidades observacionais. Como os fatores são latentes, métodos diferentes podem produzir valores distintos para uma mesma observação mesmo quando a solução fatorial permanece fixada [@johnson2007].
