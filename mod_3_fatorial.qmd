...
O **Varimax** é um critério de rotação ortogonal que procura tornar mais contrastantes os quadrados das cargas dentro de cada fator, favorecendo uma solução em que cada fator apresente algumas cargas elevadas e outras próximas de zero [@kaiser1958varimax].

O **Quartimax** é também um critério de rotação ortogonal, mas enfatiza a simplificação das variáveis. Em termos interpretativos, procura favorecer situações em que cada variável apresente carga elevada em um número reduzido de fatores [@lawley1971factor].

A formulação matemática completa desses critérios e seus algoritmos não será desenvolvida aqui. Para a formulação deste capítulo, interessa que uma rotação ortogonal altera a orientação dos eixos fatoriais sem modificar a estrutura comum reproduzida nem introduzir correlações entre os fatores.
...
Entre os critérios utilizados para rotações oblíquas encontram-se o **Oblimin** e o **Promax**. O Promax procura uma estrutura simples oblíqua a partir de uma solução inicialmente rotacionada e constitui uma alternativa computacionalmente eficiente [@hendrickson1964promax]. Os algoritmos desses critérios não serão desenvolvidos neste capítulo.

A escolha entre rotação ortogonal e oblíqua possui conteúdo estatístico e substantivo. Uma solução ortogonal impõe fatores não correlacionados; uma solução oblíqua permite que associações entre os fatores façam parte da representação. A escolha não deve ser feita apenas pela aparência das cargas, mas também pela plausibilidade de fatores correlacionados no problema estudado.
...
## Avaliação da adequação da matriz de correlações à Análise Fatorial {#sec-fatorial-adequacao}
...
Nas atividades computacionais, a Análise Fatorial Exploratória será realizada pela análise EFA do núcleo do [jamovi](https://www.jamovi.org/) [@jamovi2026]. Os métodos de extração disponíveis correspondem aos procedimentos estudados na @sec-fatorial-estimacao: *Minimum residual* (MINRES), máxima verossimilhança (*Maximum likelihood*) e eixos principais (*Principal axis*). Na configuração inicial da implementação, o método de extração é MINRES.

A opção *Principal axis* corresponde à extração por fatores ou eixos principais e não à Análise de Componentes Principais. A distinção permanece aquela desenvolvida neste capítulo: a ACP procura representar a variabilidade total da matriz analisada, enquanto a Análise Fatorial procura reproduzir sua estrutura comum por meio de fatores latentes e variâncias específicas.

As opções de rotação correspondem às estruturas discutidas na @sec-fatorial-rotacao. Rotações como Varimax e Quartimax mantêm fatores ortogonais, enquanto Oblimin e Promax permitem fatores correlacionados. Na configuração inicial da implementação, a rotação é Oblimin; por isso, uma análise executada sem alteração dessa opção produz uma solução oblíqua e pode apresentar correlações entre os fatores.

A adequação da matriz de correlações pode ser examinada pelo KMO e pelo teste de esfericidade de Bartlett, cujos significados foram desenvolvidos na @sec-fatorial-adequacao. Essas medidas avaliam aspectos da adequação da estrutura de correlações e não determinam, por si só, o número de fatores.

A escolha do número de fatores pode utilizar diferentes fontes de evidência, incluindo a análise paralela discutida na @sec-fatorial-escolha-fatores. Na implementação utilizada, o procedimento computacional empregado na análise paralela depende também do método de extração selecionado. Por isso, o princípio estatístico da análise paralela deve ser distinguido dos detalhes do algoritmo utilizado pelo programa.

Quando são calculados escores fatoriais, a implementação permite utilizar métodos como Thurstone, Bartlett, ten Berge, Anderson--Rubin e Harman. Esses métodos correspondem a critérios distintos para estimar ou predizer as realizações dos fatores, conforme discutido na @sec-fatorial-escores.
:::
...
A matriz $\boldsymbol{\Phi}\boldsymbol{\Phi}^{\top}$ representa a estrutura comum, enquanto a matriz diagonal $\boldsymbol{\Psi}$ reúne as variâncias específicas. Para variáveis padronizadas,
...
A solução fatorial apresenta indeterminação de sinal e rotação. Rotações ortogonais mantêm os fatores não correlacionados e modificam a orientação das cargas sem alterar a estrutura comum reproduzida. Rotações oblíquas permitem fatores correlacionados e exigem distinguir a matriz de padrões, a matriz de correlações fatoriais e a matriz de estrutura.

No nível amostral, a adequação da matriz de correlações pode ser examinada por meio da matriz anti-imagem, das medidas MSA e KMO e do teste de esfericidade de Bartlett. O número de fatores deve ser escolhido combinando evidências estatísticas, parcimônia e interpretação. A estimação pode seguir diferentes critérios. O método de fatores ou eixos principais utiliza a decomposição espectral de uma matriz reduzida; o MINRES pertence à família dos mínimos quadrados não ponderados e minimiza resíduos da estrutura reproduzida; sob normalidade multivariada, a máxima verossimilhança fornece uma formulação probabilística e permite avaliar o ajuste global do modelo.

Os fatores permanecem latentes após o ajuste. Os escores fatoriais são estimativas ou predições associadas às unidades observacionais e dependem do critério utilizado. O método de regressão ou Thurstone constitui uma possibilidade, enquanto métodos como Bartlett, ten Berge e Anderson--Rubin utilizam critérios distintos.
