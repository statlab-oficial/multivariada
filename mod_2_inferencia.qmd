...
O capítulo começa com a distribuição do vetor de médias amostral e a distribuição de Wishart. Em seguida, essas distribuições são aplicadas ao modelo linear multivariado, à estatística $T^2$ de Hotelling e à comparação entre uma ou duas populações. A parte final reúne as condições de validade da inferência clássica, delimita sua aplicação em problemas de alta dimensão e introduz os elementos de decisão estatística necessários à posterior formulação de regras de classificação.
...
Um **resultado assintótico** descreve o comportamento limite de uma estatística quando o tamanho amostral tende ao infinito. Sua utilização em amostras finitas constitui uma aproximação cuja qualidade depende, entre outros aspectos, da distribuição populacional, da dimensão e do tamanho da amostra.
:::
...
A distribuição transformada pode ser singular quando
...
não possui posto completo. Essa possibilidade é compatível com a extensão da distribuição normal multivariada ao suporte efetivo discutida no Capítulo 9.
...
Como essa convergência vale para toda combinação linear fixa, o @thm-cramer-wold fornece a convergência multivariada apresentada em @eq-tcl-multivariado.
:::

O teorema fundamenta, para $p$ fixo, a aproximação da distribuição de
...
Na construção por somas de produtos externos, a condição
...
também suficiente para que

$$
\boldsymbol{\mathsf W}\succ\mathbf0
$$
...
#### Aprofundamento: densidade e extensão da distribuição de Wishart

::: {.callout-note appearance="simple" icon="false"}
## Leitura de aprofundamento

A construção por somas de produtos externos, as propriedades de posto, os momentos e as transformações da distribuição de Wishart são suficientes para as distribuições amostrais utilizadas na sequência principal do capítulo. Esta seção complementa essa construção apresentando a densidade explícita da família, a função gama multivariada e a extensão da distribuição não singular para graus de liberdade reais.
:::

A construção em @eq-construcao-distribuicao-wishart utiliza um número inteiro positivo de vetores normais e, portanto, produz graus de liberdade inteiros.

A família Wishart não singular admite também uma extensão para valores reais

$$
\nu>p-1,
$$

definida pela densidade no cone das matrizes simétricas positivas definidas. Essa extensão será mencionada para completar a definição da família, embora as distribuições amostrais utilizadas nas seções seguintes produzam graus de liberdade inteiros.
...
Nesse caso, a distribuição está concentrada no conjunto de matrizes semidefinidas positivas singulares. Sob a construção em @eq-construcao-distribuicao-wishart, com
...
Considere a distribuição de Wishart obtida pela construção em @eq-construcao-distribuicao-wishart, com
...
A esperança e a variabilidade dos elementos crescem linearmente com $\nu$. Em contraste, a matriz média por grau de liberdade,
...
de modo que a variabilidade de seus elementos diminui à ordem $1/\nu$ quando $\boldsymbol{\Sigma}$ é mantida fixa.
...
Considere a distribuição construída em @eq-construcao-distribuicao-wishart, com
...
A forma quadrática em @eq-forma-quadratica-wishart-qui-quadrado mostra que, para qualquer direção fixa $\mathbf a\neq\mathbf0$, a matriz Wishart induz uma soma de quadrados padronizada com distribuição qui-quadrado.
...
::: {.proof}
Pela construção da Wishart, podem-se escrever
...
em que todos os vetores são independentes e possuem distribuição
...
produtos externos independentes com a mesma matriz de covariâncias, o que fornece
...
obtida pela construção em @eq-construcao-distribuicao-wishart, com
...
permanece semidefinida positiva e fica restrita ao subespaço imagem de $\mathbf A$. Essa situação corresponde a uma extensão singular da distribuição de Wishart e não será necessária para a sequência principal deste capítulo.
...
Como as linhas de $\boldsymbol{\mathcal X}$ são independentes e possuem matriz de covariâncias comum $\boldsymbol{\Sigma}$, a covariância entre as linhas transformadas $j$ e $k$ é
...
\right)
=
[\mathbf Q\mathbf Q^\top]_{jk}
\boldsymbol{\Sigma}
=
\delta_{jk}\boldsymbol{\Sigma}.
$$

Assim, linhas distintas possuem covariância cruzada nula. Como são conjuntamente normais, elas são independentes. Em particular,
...
A distribuição da matriz de covariâncias amostral é determinada por
...
Essa igualdade em distribuição representa apenas um reescalonamento de uma matriz Wishart e não uma nova parametrização da família.
...
seja invertível com probabilidade 1.
...
A primeira linha da matriz transformada é uma função de
...
são funções apenas das demais linhas transformadas.
...
são propriedades da amostragem normal. Sem essa hipótese, a distribuição exata da covariância amostral depende, em geral, da distribuição populacional e não possui a forma Wishart. Momentos de ordem superior também passam a participar de resultados sobre a variabilidade e a distribuição assintótica de seus elementos.
:::
...
- a geometria e a escala das colunas da matriz de delineamento, representadas pelos elementos de
  $(\mathbf Z^\top\mathbf Z)^{-1}$;
- a covariância residual entre as respostas, representada pelos elementos de $  \boldsymbol{\Sigma}$.
...
é singular. Portanto, a distribuição em @eq-distribuicao-representante-pseudoinversa é uma normal matricial degenerada, de acordo com a extensão da distribuição normal matricial adotada no Capítulo 11.
...
Restrições redundantes ou combinações redundantes das respostas produzem distribuições normais singulares. Para uma formulação inferencial não redundante, é preferível substituir essas representações por bases dos espaços correspondentes.
...
não existem graus de liberdade residuais e
...
### Independência entre funções estimáveis e resíduos
...
enquanto $\boldsymbol{\mathsf E}_M$ é função da matriz residual. A independência estabelecida no @prp-independencia-estimador-residuo-modelo-normal fornece o último resultado.
:::

A matriz $\boldsymbol{\mathsf H}$ não precisa possuir posto completo. Quando
...
com probabilidade 1 sob $H_0$. Essa singularidade não constitui, por si só, um problema: ela apenas reflete que a hipótese introduz no máximo $g$ direções independentes no espaço das $s$ combinações de respostas.
...
as propriedades de posto da distribuição de Wishart fornecem
...
As duas matrizes possuem a mesma escala

$$
\boldsymbol{\Sigma}_M,
$$

mas graus de liberdade distintos:

- $g=\operatorname{posto}(\mathbf C)$ é a dimensão do espaço de combinações lineares independentes dos parâmetros do modelo envolvidas na hipótese;
...
representa, sob essas condições de posto, $gs$ restrições escalares linearmente independentes. O número $g$ aparece como graus de liberdade da distribuição Wishart de $\boldsymbol{\mathsf H}$ porque a SSCP é construída a partir de $g$ vetores aleatórios independentes no espaço das $s$ combinações de respostas.
...
Sob a hipótese alternativa, a distribuição de

$$
\boldsymbol{\mathsf H}
$$

é uma Wishart não central. Como existem diferentes convenções para parametrizar sua não centralidade, neste livro será utilizada a parametrização simétrica obtida pelo branqueamento das respostas.
...
-   a escala e a estrutura de covariâncias entre as respostas transformadas, por meio de $\boldsymbol{\Sigma}_M^{-1/2}$.
...
possui matrizes de covariâncias linha e coluna iguais às identidades correspondentes e matriz de médias determinada por
...
é o produto de Gram das médias dessas linhas padronizadas.
...
A matriz de não centralidade será utilizada para interpretar o poder dos testes. Seu espectro descreve a intensidade e as direções do afastamento padronizado em relação à hipótese nula. O efeito desse afastamento sobre o poder depende do critério multivariado utilizado, pois diferentes estatísticas resumem de maneiras distintas as direções representadas em $\boldsymbol{\Omega}$.
...
seus autovalores são não negativos. Eles quantificam o afastamento da hipótese nula em direções ortogonais do espaço branqueado das respostas. O posto de $\boldsymbol{\Omega}$ satisfaz
...
não depende da validade de $H_0$. Sob a alternativa, $\boldsymbol{\mathsf H}$ possui distribuição Wishart não central, enquanto $\boldsymbol{\mathsf E}_M$ preserva sua distribuição Wishart central e permanece independente da matriz da hipótese.

As distribuições central e não central obtidas nesta seção são resultados exatos sob o modelo normal matricial, com matriz de delineamento tratada como fixa e com as condições de estimabilidade e posto especificadas anteriormente. Elas fornecem a base para construir testes a partir das matrizes de hipótese e erro. A próxima seção introduzirá nível de significância, poder, não centralidade e razão de verossimilhanças antes de aplicar esses princípios à inferência sobre vetores de médias.
...
Uma hipótese é **simples** quando especifica completamente o parâmetro considerado no modelo. Por exemplo, se o único parâmetro em estudo é
...
Se o modelo também contém parâmetros desconhecidos adicionais, como

$$
\boldsymbol{\Sigma},
$$

fixar apenas $\boldsymbol{\mu}$ não especifica completamente o parâmetro do modelo e a hipótese é composta.
...
Diz-se que o teste possui **tamanho $\alpha$** quando
...
O supremo não precisa, em geral, ser atingido por algum valor específico do parâmetro sob $H_0$.
...
Sob normalidade e para um vetor de médias verdadeiro $\boldsymbol{\mu}$,
...
\lambda
\right),
$$ {#eq-qui-quadrado-nao-central-media}
...
Sob $H_0$, $\lambda=0$ e recupera-se a distribuição qui-quadrado central.
...
O poder não aumenta necessariamente com a inclusão de novas respostas. Variáveis que acrescentam informação sobre a diferença investigada podem aumentar o sinal multivariado disponível. Em procedimentos que estimam a matriz de covariâncias, respostas pouco informativas também aumentam a dimensão do problema e exigem a estimação de novas variâncias e covariâncias.
...
Considere uma estatística $\mathsf T$ para a qual valores elevados fornecem evidência contra $H_0$.

Quando a distribuição de $\mathsf T$ sob $H_0$ é completamente especificada e não depende de parâmetros de perturbação desconhecidos, o valor-p observado pode ser escrito como
...
Para uma hipótese nula composta, uma definição geral baseada na cauda superior é
...
t_{\mathrm{obs}}
\right),
$$ {#eq-valor-p-hipotese-composta}

quando essa construção é apropriada para a estatística utilizada.

Em muitos testes clássicos, a distribuição nula da estatística é livre dos parâmetros de perturbação ou pode ser reduzida a uma distribuição conhecida, simplificando o cálculo do valor-p.
...
Quando os máximos existem e são atingidos, denote por
...
um estimador irrestrito de máxima verossimilhança e por
...
um estimador restrito de máxima verossimilhança.
...
No modelo linear multivariado normal, a comparação entre modelos encaixados conduz à razão de determinantes
...
Essa quantidade é denominada **lambda de Wilks** e constitui um dos critérios clássicos da análise de variância multivariada [@wilks1932].
...
é estritamente crescente em $\Lambda_{\mathrm W}$, os dois critérios produzem regiões críticas equivalentes quando calibrados no mesmo modelo.
...
Essa expressão mostra que $\Lambda_{\mathrm W}$ depende conjuntamente das raízes generalizadas associadas às matrizes de hipótese e erro, resumindo em um único escalar a separação relativa presente nessas direções. O desenvolvimento dos critérios multivariados baseados nessas raízes pertence à formulação da MANOVA no Módulo 3.

::: {.callout-caution appearance="minimal" icon="false"}
## Existência da razão de determinantes clássica

A forma clássica em @eq-lambda-wilks-modelos-encaixados exige, em particular,

$$
\mathbf E_F\succ\mathbf0.
$$
...
essa condição implica também

$$
\mathbf E_R\succ\mathbf0.
$$
...
::: {.callout-note appearance="simple" icon="false"}
## Lambda de Wilks e teorema de Wilks

Embora compartilhem o nome de Wilks, os dois conceitos são distintos.
...
é um critério baseado em determinantes que aparece em problemas multivariados específicos, como a MANOVA.

O **teorema de Wilks** é um resultado assintótico geral sobre a estatística de razão de verossimilhanças:
...
Em determinados modelos multivariados, os dois objetos estão relacionados porque a razão de verossimilhanças é uma função da lambda de Wilks, mas eles não são a mesma definição.
:::
...
localmente no parâmetro verdadeiro, isto é, $d$ é a codimensão da hipótese nula regular. Sob as condições do enunciado, esse valor coincide com o número de restrições suaves linearmente independentes.
:::
...
Em modelos regulares, sob alternativas locais contíguas que se aproximam de $H_0$ à taxa

$$
n^{-1/2},
$$

a estatística de razão de verossimilhanças possui, sob condições adicionais usuais, limite qui-quadrado não central. O parâmetro de não centralidade depende da direção local do afastamento e da informação de Fisher.
...
No caso univariado, denote por

$$
\mathsf S^2
$$

o único elemento da matriz

$$
\boldsymbol{\mathsf S},
$$

isto é, a variância amostral, e por

$$
\mathsf S
$$

seu desvio-padrão positivo.
...
Sob as condições da @prp-distribuicao-t2-uma-amostra, a região aleatória
...
a dualidade entre as normas induzidas por
...
Assim, o evento de cobertura simultânea de todas as combinações lineares coincide exatamente com o evento
...
Como a região possui cobertura

$$
1-\alpha,
$$

obtém-se a igualdade em @eq-cobertura-intervalos-simultaneos-hotelling.
:::
...
Mantidas as variâncias marginais:
...
Assim, uma hipótese de dimensão reduzida pode ser testável mesmo quando

$$
p\geq n
$$

e, portanto, a matriz completa

$$
\boldsymbol{\mathsf S}
$$

é singular. O requisito para este procedimento é a não singularidade da covariância amostral no subespaço efetivamente envolvido na hipótese:
...
constituem uma amostra aleatória de
...
é exatamente a estatística $T^2$ de Hotelling de uma amostra aplicada ao problema $h$-dimensional
...
O resultado segue da @prp-distribuicao-t2-uma-amostra com $p$ substituído por $h$.
:::
...
Dois casos particulares resumem a relação com os procedimentos anteriores:
...
- se
  ...
  \mathbf A=\mathbf I_p,
  \qquad
  \mathbf d=\boldsymbol{\mu}_0,
  $$
  recupera-se o teste $T^2$ de Hotelling para o vetor completo de médias;

- se
  $$
  h=1,
  $$
  recupera-se o teste $t$ para uma única combinação linear previamente especificada.

Assim, a dimensão inferencial relevante é determinada pelo espaço das combinações lineares efetivamente testadas.
...
funciona como fator de escala da variância da diferença entre as médias. Ele não representa o número de observações disponíveis nem os graus de liberdade do procedimento e pode assumir valores não inteiros.
...
A distribuição da forma quadrática normal--Wishart fornece
...
Sob uma alternativa fixa, o mesmo argumento conduz à distribuição $F$ não central com parâmetro
...
A rejeição indica que a diferença entre os vetores de médias não coincide com

$$
\boldsymbol{\delta}_0.
$$

Equivalentemente, existe pelo menos uma combinação linear das respostas para a qual
...
$$
\boldsymbol{\delta}_0=\mathbf0,
$$

isso significa que os vetores de médias diferem em pelo menos uma combinação linear das variáveis.
...
a variância amostral agrupada, isto é, o único elemento da matriz
...
Esses intervalos possuem cobertura simultânea para todas as combinações lineares de
...
A igualdade das matrizes de covariâncias não é necessária para definir ou estimar

$$
\boldsymbol{\mu}_1-\boldsymbol{\mu}_2.
$$

Ela é uma hipótese específica do procedimento clássico com covariância agrupada e de sua distribuição exata de referência.
...
De fato, se as covariâncias populacionais forem
...
=
\frac{
(n_1-1)\boldsymbol{\Sigma}_1
+
(n_2-1)\boldsymbol{\Sigma}_2
}{
n_1+n_2-2
},
$$

que é uma média ponderada das duas matrizes e não uma matriz de covariâncias comum às duas populações.
...
denota o número de pares completos para os quais o vetor de diferenças pode ser calculado.
...
A covariância dentro dos pares influencia diretamente a precisão das diferenças. Para qualquer combinação linear
...
Assim, uma associação intrapar positiva na direção $\mathbf a$ reduz a variância da diferença nessa direção em relação ao que seria obtido ignorando a covariância intrapar. Uma associação negativa produz o efeito oposto.
...
A covariância cruzada amostral é
...
A identidade em @eq-decomposicao-covariancia-amostral-diferencas depende de essas covariâncias cruzadas serem calculadas utilizando os pares correspondentes.
...
formam, pelas hipóteses do enunciado, uma amostra aleatória de
...
é exatamente a estatística $T^2$ de Hotelling de uma amostra aplicada a esses vetores. Portanto, as distribuições central e não central seguem diretamente da @prp-distribuicao-t2-uma-amostra.
:::
...
a variância amostral das diferenças, isto é, o único elemento da matriz
...
Permutar arbitrariamente as observações de uma das condições rompe a correspondência original entre os pares. Com isso, as covariâncias cruzadas calculadas deixam de representar a associação intrapar do delineamento e a matriz
...
Em particular, desaparecem da variância da diferença os termos
...
que são justamente os termos responsáveis por incorporar a associação dentro dos pares.
:::
...
Para a validade exata deste procedimento, não é necessário assumir separadamente normalidade marginal em cada condição. É suficiente que os vetores de diferenças sejam independentes entre os pares e possuam distribuição normal multivariada com matriz de covariâncias não singular.
...
| Estrutura de covariância requerida    | Covariância comum entre populações              | Covariância não singular das diferenças               |
| Estrutura entre unidades              | Independência entre as duas amostras e entre unidades distintas | Independência entre pares, com dependência intrapar permitida |
...
No modelo linear multivariado, a normalidade dos erros sustenta, no caso geral, a independência entre o representante de mínimos quadrados
...
Consequentemente, qualquer função linear estimável de $\mathbf B$ é independente da SSCP residual. Quando $\mathbf Z$ possui posto coluna completo,
...
e permanecem válidas as distribuições exatas das matrizes de hipótese e erro sob as condições de posto especificadas anteriormente.

Sem normalidade:

- se os primeiros momentos existem, a média amostral permanece não viesada para $\boldsymbol{\mu}$;
- se os segundos momentos existem, a covariância amostral com divisor $n-1$ permanece não viesada para $\boldsymbol{\Sigma}$;
- sob condições usuais de amostragem i.i.d. e existência dos momentos necessários, a consistência da média e da covariância amostrais pode permanecer válida;
...
Uma configuração potencialmente problemática ocorre quando grupos de tamanhos diferentes apresentam também magnitudes de dispersão diferentes. Em particular, quando o grupo menor possui maior variabilidade, a covariância agrupada pode atribuir peso relativamente reduzido justamente ao grupo cuja média apresenta maior incerteza.

A magnitude e a direção da distorção dependem, entretanto, da estrutura completa das covariâncias e do contraste considerado.
...
Quando as funções representadas por $\mathbf C$ são estimáveis, redundâncias em suas linhas tornam singular a matriz

$$
\mathbf G_C.
$$

Da mesma forma, redundâncias nas colunas de $\mathbf M$ tornam singular a matriz de covariâncias transformada
...
Por isso, as linhas de $\mathbf C$ devem representar funções estimáveis linearmente independentes e as colunas de $\mathbf M$ devem representar combinações linearmente independentes das respostas.
...
$$
\boldsymbol{\mathsf S}\succ\mathbf0.
$$

Se a matriz for singular, o número de condição espectral usual é infinito.
...
-   análise por grupos;
-   resíduos;
-   alavancagem, quando houver uma matriz de delineamento;
...
A existência de segundos momentos é suficiente, sob amostragem i.i.d., para definir $\boldsymbol{\Sigma}$ e para a consistência usual da matriz de covariâncias amostral. Já a distribuição assintótica dos elementos de $\boldsymbol{\mathsf S}$ envolve, em geral, momentos de quarta ordem.

Se esses momentos não existem, aproximações usuais para a covariância amostral podem falhar.

#### Aprofundamento: procedimentos robustos

::: {.callout-note appearance="simple" icon="false"}
## Leitura de aprofundamento

A inferência clássica desenvolvida neste capítulo utiliza a média e a matriz de covariâncias amostrais usuais. Esta seção apresenta, como extensão, o que muda quando esses estimadores são substituídos por versões robustas. Os procedimentos específicos de estimação robusta e suas calibrações não são necessários para a sequência principal do livro.
:::
...
possuir uma componente no núcleo de

$$
\boldsymbol{\mathsf S},
$$

essa componente é anulada por

$$
\boldsymbol{\mathsf S}^+
$$

e não contribui para a forma quadrática. Portanto, a pseudoinversa utiliza apenas as direções pertencentes ao espaço imagem da covariância amostral.
...
#### Aprofundamento: estratégias para alta dimensão

::: {.callout-note appearance="simple" icon="false"}
## Leitura de aprofundamento

A singularidade, o mau condicionamento e os limites da pseudoinversa pertencem ao núcleo da inferência clássica e delimitam quando os procedimentos estudados anteriormente deixam de ser aplicáveis. Esta seção apresenta estratégias utilizadas fora desse regime clássico, como regularização, redução da dimensão, reamostragem e testes especializados. Esses métodos possuem teoria e calibração próprias e não são necessários para a formulação das técnicas multivariadas clássicas desenvolvidas nos capítulos posteriores.
:::
...
Na regularização em @eq-covariancia-regularizada-inferencia, se
...
$$
\mathbf T\succ\mathbf0,
$$

então

$$
\boldsymbol{\mathsf S}_{\lambda}\succ\mathbf0.
$$

Da mesma forma, a regularização em @eq-regularizacao-identidade-inferencia é positiva definida para todo

$$
\lambda>0.
$$

Além de garantir invertibilidade nessas condições, a regularização pode:

-   reduzir o número de condição;
-   estabilizar distâncias;
-   diminuir a variabilidade da inversa.

Em geral, essa modificação introduz viés na estimação de $\boldsymbol{\Sigma}$ em troca de maior estabilidade. Ela também modifica a estatística inferencial. O parâmetro de regularização e a distribuição de referência precisam ser definidos pelo procedimento adotado.
...
-   testes por permutação compatíveis com o delineamento e com as condições de permutabilidade exigidas sob $H_0$;
...
#### Síntese das limitações dimensionais

A tabela a seguir reúne as principais consequências da relação entre dimensão, tamanho amostral e posto das matrizes utilizadas na inferência clássica.
...
quantifica a consequência de escolher a ação $a$ quando o verdadeiro valor do parâmetro é $\boldsymbol{\vartheta}$.
:::
...
A rejeição de $H_0$ quando o parâmetro pertence a $\mathcal P_0$ e a não rejeição quando o parâmetro pertence à alternativa podem receber perdas distintas. Essas perdas podem ainda variar com o valor específico do parâmetro. A formulação por nível de significância controla a probabilidade do erro do tipo I, enquanto a formulação decisória permite representar explicitamente as consequências das diferentes ações.
...
A abordagem de Bayes utiliza a distribuição posterior para comparar as ações segundo suas perdas posteriores esperadas [@berger1985].

A perda posterior esperada de uma ação
...
Quando o mínimo é atingido, uma regra de Bayes escolhe uma ação que minimiza a perda posterior esperada:
...
Na classificação, a expressão **probabilidade a priori da classe** refere-se à probabilidade populacional de pertencimento a cada grupo antes de observar as características da nova unidade. Esse objeto não deve ser confundido com uma distribuição a priori sobre parâmetros desconhecidos do modelo.
...
a perda esperada de escolher a classe $a$ é
...
Portanto, a ação que minimiza essa quantidade é, por definição, a ação de Bayes para a observação $\mathbf x$.
:::
...
a perda posterior esperada de escolher a classe $a$ é
...
Portanto, minimizar a perda posterior esperada equivale a maximizar a probabilidade posterior, e a regra escolhe a classe de maior probabilidade posterior:
...
Alterações nas probabilidades a priori podem modificar a classificação mesmo quando as densidades condicionais
...
a mesma regra pode ser escrita como
...
Aqui interessa apenas estabelecer que uma regra de classificação depende não somente da separação estatística entre os grupos, mas também das probabilidades a priori e das consequências atribuídas aos erros.
...
| 12. Inferência multivariada | Wishart, razão de verossimilhanças e $T^2$ | Distribuições amostrais, testes, regiões de confiança e comparação entre vetores de médias |

: Objetos e contribuições dos capítulos do Módulo 2. {#tbl-sintese-modulo-dois}

Os cinco capítulos compartilham algumas estruturas recorrentes. Formas quadráticas medem afastamentos na escala da variabilidade. Determinantes quantificam aspectos de escala volumétrica associados a transformações lineares e a matrizes de dispersão e também participam de razões de verossimilhanças. Projetores decompõem a informação entre ajuste e resíduo. Distribuições normais e Wishart fornecem as distribuições exatas necessárias aos testes clássicos.
...
-   representar hipóteses por funções lineares estimáveis e pelas matrizes que as especificam;
-   distinguir resultados exatos de aproximações assintóticas;
-   aplicar a estatística $T^2$ em problemas de uma amostra, hipóteses lineares, duas amostras independentes e observações pareadas;
