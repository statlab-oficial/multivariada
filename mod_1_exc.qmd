...
Na segunda parte, o `R` será utilizado para estender essas mesmas ideias a conjuntos de dados de maior dimensão. O objetivo não é substituir os cálculos matriciais por funções prontas, mas utilizá-los para analisar matrizes de dados reais e verificar computacionalmente propriedades estudadas no Módulo I.
...
4. Determine
   ...
   $$
   \mathcal N(\mathbf H_4),
   $$

   $\mathcal C(\mathbf H_4)$ e o posto de $\mathbf H_4$.
...
8. Explique por que $\mathbf H_4$ é o projetor ortogonal sobre

   $$
   \operatorname{span}(\mathbf1_4)^\perp.
   $$
:::
...
7. Explique como os autovetores determinam os eixos principais das curvas de nível da forma quadrática.
:::
...
7. Explique por que o complemento de Schur pode ser interpretado como uma quantidade remanescente após considerar a informação contida no primeiro bloco.
:::
...
## Direções sucessivas sob ortogonalidade

Utilize a matriz $\mathbf S$ do @exr-fix-m1-maxima-dispersao.

Seja $\mathbf a_1$ uma direção unitária que maximiza
...
$$
\mathbf a^\top\mathbf a_1=0.
$$

1. Determine a solução $\mathbf a_2$.

2. Calcule o valor da forma quadrática nessa direção.
...
5. Explique por que esse procedimento permite construir sucessivamente direções ortogonais ordenadas segundo os valores da forma quadrática.
:::
...
7. Explique por que a vetorização permite representar transformações de matrizes como transformações lineares de vetores.
:::
...
5. Explique por que funções envolvendo simultaneamente log-determinantes, matrizes inversas e traços são naturais quando o parâmetro de interesse é uma matriz positiva definida.
:::
...
## Transformação de um estimador e aproximação assintótica

Suponha que um estimador vetorial satisfaça
...
5. Utilizando o método delta multivariado, escreva a distribuição assintótica de
...
6. Identifique a matriz de covariâncias assintótica resultante.

7. Explique a diferença entre calcular
   ...
   para uma amostra de tamanho fixo e estudar seu comportamento quando
...
## Mínimos quadrados com duas variáveis-resposta
...
10. Explique por que a mesma matriz de delineamento permite estimar simultaneamente várias variáveis-resposta por meio do cálculo matricial.
:::
...
7. Explique como a vetorização transforma um modelo matricial com duas respostas em uma única representação linear vetorial.

8. Discuta por que essa representação poderá ser útil quando forem estudadas conjuntamente várias variáveis-resposta.

9. Relacione a identidade utilizada à expressão geral
