...
A MANOVA não introduz um novo modelo para essas respostas. Seu conteúdo específico está na forma como uma hipótese estimável sobre $\mathbf B$ é comparada com a variação residual conjunta. Essa comparação conduz às matrizes de hipótese e erro e, a partir delas, a um problema de autovalores generalizados. Os diferentes critérios clássicos de MANOVA são funções das raízes obtidas desse mesmo problema [@anderson2003; @rencher2012].

O objetivo deste capítulo é mostrar como uma hipótese do modelo linear multivariado produz as matrizes de hipótese e erro, por que sua comparação conduz a um quociente de Rayleigh generalizado e como as raízes características resultantes fundamentam os critérios clássicos de Wilks, Pillai, Hotelling--Lawley e Roy.

A formulação algébrica e geométrica será distinguida da teoria probabilística utilizada para a inferência clássica. Algoritmos, diagnósticos operacionais, comparações práticas entre os critérios, procedimentos posteriores à rejeição da hipótese, extensões heterocedásticas, robustas e de alta dimensão serão tratados posteriormente.
...
Considere inicialmente $G\geq2$ grupos e um vetor de $p$ respostas para cada unidade,
...
Se o vetor de médias do grupo $h$ é
...
Por isso, a MANOVA considera simultaneamente:
...
A dependência entre as respostas faz com que o problema não seja equivalente a realizar $p$ análises de variância independentes. A estrutura conjunta é representada por matrizes de somas de quadrados e produtos cruzados.
...
conforme a condição estabelecida no Módulo 2.
...
No problema de comparação entre $G$ grupos, uma matriz $\mathbf C_1$ apropriada pode representar os contrastes necessários para expressar @eq-hipotese-medias-grupos-manova. Quando se testa o efeito completo de grupos, existem
...
No caso do efeito completo de $G$ grupos,
...
Defina os graus de liberdade residuais por
...
No Módulo 2, para modelos encaixados, foram definidas:

- a SQPC residual do modelo completo,
  $$
  \mathbf E_F;
  $$

- a SQPC residual do modelo reduzido,
  $$
  \mathbf E_R.
  $$

A diferença produzida pelas restrições satisfaz
...
a matriz de covariâncias agrupada definida no Módulo 2.
...
A formulação bilateral mais geral
...
já foi estabelecida no Módulo 2. Neste capítulo foi utilizado
...
Na MANOVA, o problema central é inferencial:

> existem diferenças na função paramétrica multivariada especificada pela hipótese?

Na Análise Discriminante, as estruturas entre e dentro dos grupos serão utilizadas para outro objetivo:

> quais combinações das variáveis separam os grupos e como utilizar essa informação para discriminação ou classificação?

Assim, as mesmas matrizes e os mesmos autovalores generalizados podem possuir interpretações estatísticas diferentes.

Esse contraste constitui a transição direta para o próximo capítulo.
...
Neste capítulo, a hipótese foi escrita sobre todas as respostas originais como
...
e define uma função estimável dos parâmetros.
...
já foi estabelecida no Módulo 2.

A hipótese produz uma matriz observada
...
enquanto a variabilidade residual é representada por
...
uma combinação linear das respostas definida por
...
A otimização desse quociente conduz ao problema de autovalores generalizados
...
converte o problema em uma decomposição espectral simétrica na geometria em que a dispersão residual foi padronizada.
...
Essas raízes quantificam a intensidade da hipótese relativamente ao erro nas direções características.

Os quatro critérios clássicos são construídos a partir da mesma coleção de raízes:
...
As raízes nulas não alteram esses critérios, de modo que as expressões podem ser restritas às
...
Os critérios diferem na maneira como agregam a estrutura espectral:

- Wilks utiliza um produto;
- Pillai utiliza uma soma de raízes transformadas;
- Hotelling--Lawley utiliza a soma direta das raízes;
- Roy utiliza somente a maior raiz.

A estrutura algébrica não exige normalidade.
...
as estatísticas matriciais aleatórias satisfazem
...
e são independentes condicionalmente a

$$
\mathbf Z.
$$

Essa estrutura normal--Wishart fornece a base da inferência clássica.

A lambda de Wilks possui ainda uma ligação direta com a razão de verossimilhanças entre modelos encaixados.

A formulação recupera procedimentos mais simples em casos particulares:

- com uma única resposta, reduz-se à estrutura da ANOVA;
- com dois grupos, possui uma única direção independente e relaciona-se ao $T^2$ de Hotelling.

As raízes são invariantes a transformações lineares não singulares das respostas. As coordenadas das direções mudam com a base, mas a comparação espectral entre hipótese e erro permanece a mesma.

A existência da formulação clássica exige distinguir condições de naturezas diferentes:

- estimabilidade da hipótese;
- posto suficiente da matriz residual;
- positividade definida de $\mathbf E$ para a formulação baseada em inversas;
- independência entre unidades;
- covariância residual comum;
- não singularidade de $\boldsymbol{\Sigma}$;
- normalidade para a teoria inferencial clássica.

A MANOVA utiliza, assim, resultados dos Módulos 1 e 2 sobre formas quadráticas, produtos internos induzidos, autovalores generalizados, raízes quadradas de matrizes positivas definidas, modelo linear multivariado, estimabilidade, distribuição normal multivariada, Wishart e razão de verossimilhanças.

O ponto central é que a técnica não consiste simplesmente em testar várias respostas simultaneamente. Sua forma matemática surge porque uma hipótese multivariada precisa ser comparada com a dispersão residual **em todas as combinações lineares possíveis das respostas**. Essa comparação é exatamente o problema de razão entre duas formas quadráticas que conduz aos autovalores generalizados.

Ao final deste capítulo, o estudante deverá ser capaz de:

1. situar a MANOVA dentro do modelo linear multivariado;

2. formular uma hipótese estimável da forma
...
3. distinguir as funções das matrizes
...
4. explicar o significado do quociente
...
5. explicar por que a maximização da razão hipótese/erro conduz ao problema
...
6. relacionar o problema generalizado à decomposição espectral de
...
7. interpretar as raízes características como magnitudes da hipótese relativamente à dispersão residual;

8. explicar por que o número de raízes positivas é limitado por
...
9. distinguir as formas de agregação utilizadas pela lambda de Wilks, pelo traço de Pillai, pelo traço de Hotelling--Lawley e pela maior raiz de Roy;

10. separar a construção algébrica dos critérios de sua teoria inferencial clássica baseada no modelo normal--Wishart;

11. reconhecer as condições de estimabilidade, posto e não singularidade necessárias à formulação clássica;

12. distinguir raízes características, direções características e subespaços associados a raízes repetidas;

13. explicar a invariância das raízes sob transformações lineares não singulares das respostas;

14. relacionar MANOVA com ANOVA, $T^2$ de Hotelling, regressão multivariada e Análise Discriminante.

No próximo capítulo, os grupos permanecem conhecidos, mas o objetivo estatístico muda.

As matrizes que na MANOVA foram utilizadas para testar diferenças entre grupos serão empregadas na **Análise Discriminante** para construir direções de separação. Além da formulação geométrica de Fisher, será introduzida uma formulação probabilística destinada a classificar novas observações.

Essa passagem mostrará novamente que o mesmo problema de autovalores generalizados pode sustentar técnicas diferentes quando mudam o objetivo estatístico e a interpretação dos objetos envolvidos.