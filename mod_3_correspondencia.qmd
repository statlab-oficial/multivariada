...
Proximidades entre uma categoria de linha e uma categoria de coluna podem indicar associação entre as duas margens [@johnson2002; @hair2009]. Essa proximidade não possui a interpretação métrica da distância qui-quadrado, definida entre perfis pertencentes à mesma margem.
...
A formulação da Análise de Correspondência é geométrica e descritiva e possui relativa liberdade de pressupostos estatísticos [@hair2009]. A comparabilidade das categorias e a composição da tabela continuam fazendo parte da avaliação da análise. As condições necessárias à construção da geometria são distintas das hipóteses adicionais utilizadas na inferência sobre independência.
...
Sob a formulação multinomial da @sec-ca-formulacao-probabilistica, a rejeição do teste global fornece evidência contra a hipótese populacional de independência. A rejeição indica associação populacional entre as variáveis; não estabelece precedência temporal nem exclui explicações alternativas e, portanto, não sustenta interpretação causal [@hair2009].
...
A interpretação do mapa deve considerar as frequências marginais e os perfis. Pela geometria qui-quadrado, diferenças associadas a categorias de pequena massa recebem maior peso na comparação dos perfis da margem oposta. A solução também é sensível a linhas ou colunas atípicas e ao conjunto de categorias incluídas na análise [@hair2009].
...
A Análise de Correspondência é utilizada principalmente como técnica exploratória e gráfica [@hair2009; @johnson2002]. Sua construção geométrica não depende da realização de um teste de independência.
