...
Assim, o critério $d_\ell>0{,}2$ corresponderia a $\lambda_\ell>0{,}04$. Esse limite é uma orientação prática de Hair et al. e não determina matematicamente o número de dimensões retidas.
...
### Sensibilidade da representação {#sec-ca-estabilidade}

A configuração obtida pela Análise de Correspondência depende das categorias incluídas na tabela. Hair et al. destacam que a omissão de objetos ou atributos pode modificar a representação e limitar sua generalização [@hair2009].

A sensibilidade ao conjunto de categorias pode ser examinada comparando análises obtidas após inclusões, exclusões ou recodificações justificadas. Mudanças nas dimensões retidas ou nas posições relativas das categorias indicam dependência da representação em relação à composição da tabela.

Quando tabelas comparáveis estão disponíveis em amostras distintas, a estabilidade pode ser examinada pela proporção de inércia representada e pelas posições relativas das categorias comuns. Essa comparação requer definições compatíveis das categorias nas diferentes amostras.
...
A inversão simultânea do sinal de um eixo não caracteriza mudança da representação. Quando valores singulares iguais ou próximos tornam a orientação dos eixos individualmente instável, a comparação deve privilegiar as configurações e os subespaços representados, conforme a @sec-ca-nao-unicidade-eixos.
...
Nas atividades computacionais, a Análise de Correspondência será realizada pelo módulo [`MEDA`](https://sebastien-le.github.io/medasite/) [@meda2026] do [jamovi](https://www.jamovi.org/) [@jamovi2026], com cálculos baseados no pacote `FactoMineR` [@le2008factominer].
...
O teste qui-quadrado avalia a hipótese global de independência discutida em @sec-ca-formulacao-probabilistica e @sec-ca-uso-teste-global. As coordenadas e as porcentagens de inércia descrevem a representação geométrica da tabela e não constituem testes separados para dimensões ou categorias.
