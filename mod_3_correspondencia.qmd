...
Nas atividades computacionais, a Análise de Correspondência será realizada pelo módulo [`MEDA`](https://sebastien-le.github.io/medasite/) [@meda2026] do [jamovi](https://www.jamovi.org/) [@jamovi2026]. O `MEDA` fornece uma interface para procedimentos de análise multivariada implementados no pacote `FactoMineR` [@le2008factominer]. A entrada da análise é uma tabela de contingência, isto é, uma matriz de frequências conjuntas das categorias, correspondente ao objeto desenvolvido na @sec-ca-tabela-contingencia. Códigos numéricos atribuídos às categorias não devem ser interpretados como valores quantitativos; o objeto analisado é a estrutura de frequências da tabela.
...
A relação com o @sec-acp decorre do uso de decomposições de baixa dimensão, mas as duas técnicas possuem objetos e geometrias diferentes: a ACP representa variabilidade de dados quantitativos, enquanto a Correspondência representa associações entre perfis de uma tabela de contingência.

No próximo capítulo, o problema estatístico passa da representação de associações entre categorias para a comparação conjunta de várias respostas quantitativas entre grupos ou condições previamente definidos. O @sec-manova utilizará o modelo linear multivariado, as matrizes de hipótese e erro e critérios baseados em sua estrutura espectral.