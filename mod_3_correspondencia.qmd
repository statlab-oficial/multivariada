...
### Aprofundamento: reconstrução dos desvios da independência {#sec-ca-reconstrucao-desvios}

::: {.callout-note appearance="simple" icon=false}
## Leitura de aprofundamento

A SVD não apenas produz as dimensões utilizadas na representação geométrica. Ela também permite decompor e reconstruir, dimensão por dimensão, os desvios observados em relação à estrutura de independência.
:::
...
### Aprofundamento: contribuição e qualidade de representação {#sec-ca-contribuicao-cos2}

::: {.callout-note appearance="simple" icon=false}
## Leitura de aprofundamento

As coordenadas principais fornecem a representação geométrica das categorias. Depois de construída essa representação, medidas adicionais podem ser utilizadas para identificar quais categorias participam mais intensamente da construção de cada dimensão e quanto da posição de cada categoria é representado pelas dimensões consideradas.
:::
...
### Aprofundamento: formulação probabilística e teste de independência

::: {.callout-note appearance="simple" icon=false}
## Leitura de aprofundamento

A identidade $\chi^2=n\mathcal I$ é uma propriedade algébrica da tabela observada. Para utilizar a estatística de Pearson em um teste de independência populacional, é necessário acrescentar um modelo probabilístico para a geração das frequências e considerar sua distribuição amostral.
:::
...
Categorias suplementares permitem acrescentar informação a uma solução já construída sem modificar os eixos, as inércias principais ou as coordenadas das categorias ativas. Elas são projetadas posteriormente na geometria determinada pelas categorias que participaram da construção da análise.
:::
...
As dimensões, inércias principais e coordenadas apresentadas pela implementação correspondem à decomposição por SVD e aos sistemas de coordenadas desenvolvidos nas @sec-ca-svd e @sec-ca-coordenadas.

Medidas adicionais de contribuição e qualidade de representação, o teste global de independência e a projeção de categorias suplementares devem ser interpretados segundo as respectivas formulações apresentadas neste capítulo.
:::
