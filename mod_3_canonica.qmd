...
A Correlação Canônica estuda a associação linear entre dois blocos de variáveis quantitativas observados sobre as mesmas unidades. A técnica constrói uma combinação linear de cada bloco e procura maximizar a correlação entre essas combinações [@anderson2003; @johnson2007; @mardia2024].
...
Pela estrutura da matriz de covariâncias de um vetor particionado estabelecida em @eq-covariancia-vetor-particionado, para o vetor conjunto
...
Pela variância de combinações lineares estabelecida em @prp-momentos-combinacoes-lineares,
...
Além disso, aplicando @prp-covariancia-combinacoes-blocos,
...
Essa interpretação pode ser vista diretamente sobre os vetores aleatórios. O branqueamento simétrico foi definido em @eq-transformacao-branqueamento-ponto, e @eq-covariancia-vetor-branqueado estabeleceu que essa transformação produz matriz de covariâncias identidade.
...
Por @eq-covariancia-vetor-branqueado,
...
Combinando a desigualdade de Cauchy--Schwarz de @thm-cauchy-schwarz com a caracterização do maior valor singular em @prp-extremos-comprimentos-svd,
...
Pela relação entre SVD e os espectros das matrizes de Gram estabelecida em @prp-espectros-gram-svd, as matrizes
...
### Associação com a variável canônica do próprio bloco
...
A @tbl-coeficientes-cargas-cca sintetiza a diferença de interpretação entre coeficientes e cargas canônicas.
...
### Associação com a variável canônica do outro bloco
...
As matrizes de covariâncias amostrais foram definidas como estatísticas aleatórias em @def-matriz-covariancias-amostral-aleatoria, enquanto suas realizações observadas foram definidas em @def-matriz-covariancias-observada.
...
A covariância cruzada observada definida em @def-covariancia-cruzada-observada é
...
e sua contraparte como estatística aleatória foi definida em @def-covariancia-cruzada-amostral. Sua contraparte como estatística aleatória,
...
O limite de posto da matriz de covariâncias observada foi estabelecido em @eq-posto-covariancia-observada. Aplicando-o separadamente aos dois blocos,
...
### Dimensão amostral e sensibilidade
...
A @tbl-manova-fisher-cca sintetiza os diferentes objetivos associados a essa estrutura compartilhada.
