...
No Módulo 3, a SVD aparece em contextos distintos. no @sec-acp, é aplicada à matriz de dados centralizada. no @sec-correspondencia, será aplicada a uma matriz de desvios padronizados em relação à independência.
...
O modelo linear multivariado definido na @def-modelo-linear-multivariado organiza conjuntamente várias respostas, um delineamento e uma matriz de parâmetros. A hipótese linear geral da @def-hipotese-linear-geral-multivariada fornece a estrutura para testar combinações desses parâmetros. O @sec-manova será formulada diretamente a partir desses resultados, acrescentando a construção e a interpretação dos critérios multivariados associados às matrizes de hipótese e erro.

Outras técnicas acrescentam estruturas probabilísticas específicas à sua formulação. no @sec-fatorial, o modelo de fatores comuns especifica uma estrutura para a matriz de covariâncias, e a estimação por máxima verossimilhança acrescenta a hipótese de normalidade multivariada. no @sec-discriminante, distribuições condicionais aos grupos podem ser utilizadas para construir regras de classificação. Quando a normalidade multivariada é utilizada, sua definição e suas propriedades são as estabelecidas na @def-normal-multivariada.
