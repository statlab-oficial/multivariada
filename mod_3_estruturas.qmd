...
A distância euclidiana definida em @def-distancia-euclidiana fornece uma medida de proximidade já estabelecida no Módulo 1. Outras medidas de proximidade e dissimilaridade serão introduzidas quando necessárias nos capítulos específicos. Na Análise de Agrupamentos, a medida adotada passa a participar da geometria do problema. Métodos baseados em centroides, critérios de ligação ou outras estruturas podem produzir agrupamentos diferentes para os mesmos dados.
...
No MDS clássico, a relação entre distâncias e produtos internos permite reconstruir uma matriz de Gram e obter coordenadas por decomposição espectral. Essa formulação utilizará diretamente a distância euclidiana definida em @def-distancia-euclidiana, a matriz de Gram de @def-matriz-gram, a preservação das distâncias pela centralização estabelecida em @prp-centralizacao-preserva-distancias e os resultados espectrais sintetizados em @tbl-sintese-espectral.
...
As combinações lineares definidas em @def-combinacao-linear constroem novas variáveis e novas direções a partir das variáveis originais. Para um vetor aleatório $\boldsymbol{\mathsf X}$, sua forma é
...
As formas quadráticas definidas em @def-forma-quadratica aparecem na variância de combinações lineares, em distâncias ponderadas e em diferentes medidas de dispersão. Quando o critério depende da direção escolhida, surgem problemas de otimização espectral.
...
Distâncias organizam relações de proximidade entre observações; quando uma técnica admite dissimilaridades mais gerais, elas serão especificadas no capítulo correspondente. A distância euclidiana já foi definida em @def-distancia-euclidiana. A matriz de Gram definida em @def-matriz-gram organiza produtos internos e estabelece a ligação entre coordenadas e distâncias.
...
A distância de Mahalanobis, definida em @def-distancia-mahalanobis, acrescenta a estrutura de dispersão à geometria. A expressão em eixos espectrais apresentada em @eq-mahalanobis-eixos-espectrais mostra que o afastamento é ponderado pela variabilidade existente em cada direção. Essa estrutura será retomada na formulação probabilística da Análise Discriminante.
...
Da mesma forma, quando uma técnica utilizar máxima verossimilhança ou comparação de modelos por verossimilhanças, serão retomadas a função de verossimilhança definida em @def-funcao-verossimilhanca e a razão de verossimilhanças definida em @def-razao-verossimilhancas. Os resultados inferenciais também preservarão a distinção entre resultados exatos e assintóticos estabelecida em @def-resultados-exatos-assintoticos.
