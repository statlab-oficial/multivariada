...
As projeções ortogonais definidas na @def-projecao-ortogonal permitem representar observações em subespaços. Essa geometria é utilizada na @sec-acp para interpretar a ACP como projeção sobre um subespaço principal e será retomada na @sec-discriminante na construção de direções de separação entre grupos.
...
O quociente de Rayleigh definido na @def-quociente-rayleigh relaciona a otimização de uma forma quadrática aos autovalores de uma matriz simétrica. O resultado de @thm-extremos-quociente-rayleigh é aplicado na @sec-acp à matriz de covariâncias para formular as componentes principais.

Quando duas formas quadráticas são comparadas, o quociente de Rayleigh generalizado de @def-quociente-rayleigh-generalizado conduz ao problema de autovalores generalizados descrito no @thm-extremos-rayleigh-generalizado. Essa estrutura será retomada na @sec-discriminante e na @sec-manova.
...
No Módulo 3, a SVD aparece em contextos distintos. Na @sec-acp, é aplicada à matriz de dados centralizada. Na @sec-correspondencia, será aplicada a uma matriz de desvios padronizados em relação à independência.
...
Distâncias organizam relações de proximidade entre observações; quando uma técnica admite dissimilaridades mais gerais, elas serão especificadas no capítulo correspondente. A distância euclidiana já foi definida na @def-distancia-euclidiana e será retomada na @sec-agrupamentos na construção de medidas de proximidade entre objetos.

A distância de Mahalanobis, definida na @def-distancia-mahalanobis, incorpora a estrutura de dispersão à geometria. A expressão em eixos espectrais apresentada em @eq-mahalanobis-eixos-espectrais mostra que o afastamento é ponderado pela variabilidade existente em cada direção. Essa estrutura será retomada na @sec-discriminante na formulação probabilística das regras de classificação.

### Modelos lineares e probabilísticos

O modelo linear multivariado definido na @def-modelo-linear-multivariado organiza conjuntamente várias respostas, um delineamento e uma matriz de parâmetros. A hipótese linear geral de @def-hipotese-linear-geral-multivariada fornece a estrutura para testar combinações desses parâmetros. A @sec-manova será formulada diretamente a partir desses resultados, acrescentando a construção e a interpretação dos critérios multivariados associados às matrizes de hipótese e erro.

Outras técnicas acrescentam estruturas probabilísticas específicas à sua formulação. Na @sec-fatorial, o modelo de fatores comuns especifica uma estrutura para a matriz de covariâncias, e a estimação por máxima verossimilhança acrescenta a hipótese de normalidade multivariada. Na @sec-discriminante, distribuições condicionais aos grupos podem ser utilizadas para construir regras de classificação. Quando a normalidade multivariada é utilizada, sua definição e suas propriedades são as estabelecidas na @def-normal-multivariada.
...
A relação entre ACP e Análise Fatorial é desenvolvida na @sec-fatorial. A conexão entre Agrupamentos e Análise Discriminante será retomada na @sec-discriminante, enquanto a relação entre MANOVA e Discriminante será estabelecida a partir das estruturas de dispersão associadas aos grupos. A @sec-correspondencia retomará a relação com a ACP a partir de sua própria decomposição de baixa dimensão.
...
Os capítulos seguintes utilizarão esses fundamentos sem reconstruí-los. A @sec-acp inicia esse percurso formulando a Análise de Componentes Principais a partir da variância de combinações lineares, do quociente de Rayleigh, da decomposição espectral, das projeções e da SVD.