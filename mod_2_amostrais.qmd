...
- $\boldsymbol{\mathsf T}$ representa a matriz total de somas de quadrados e produtos cruzados (SQPC) como estatística aleatória, e $\mathbf T$ sua realização observada, definida em @def-matriz-sscp-total;
...
A convenção $\mathbf T$ para a matriz SQPC observada mantém a notação estabelecida em @def-matriz-sscp-total.
:::
...
Quando existem momentos de primeira e segunda ordem, o vetor de médias e a matriz de covariâncias definidos em @eq-vetor-medias-populacional e @def-matriz-covariancias-populacional satisfazem
...
No modelo normal multivariado definido em @def-normal-multivariada,
...
Neste capítulo, as principais quantidades populacionais de interesse são o vetor de médias $\boldsymbol{\mu}$, definido em @eq-vetor-medias-populacional, a matriz de covariâncias $\boldsymbol{\Sigma}$, definida em @def-matriz-covariancias-populacional, e a matriz de correlações $\boldsymbol{\mathcal R}$, definida em @def-matriz-correlacoes-populacional.
...
No modelo normal multivariado não singular definido em @def-normal-multivariada, o parâmetro completo pode ser representado por
...
ambos construídos a partir da matriz SQPC $\boldsymbol{\mathsf T}$ definida em @def-matriz-sscp-total.
...
O operador $\operatorname{vec}$, definido em @def-operador-vec, transforma a matriz estimada em um vetor e permite representar conjuntamente a variabilidade de todos os seus elementos.
...
\right].
$$
A decomposição em @eq-decomposicao-eqm-vetorial também pode ser vista como uma aplicação de @eq-esperanca-forma-quadratica-geral, tomando a matriz identidade e considerando $\widehat{\boldsymbol{\theta}}$ como vetor aleatório com média $E(\widehat{\boldsymbol{\theta}})$.
...
A eficiência assintótica compara as matrizes que aparecem nas distribuições-limite de sequências de estimadores. Sob condições de regularidade, estimadores de máxima verossimilhança podem atingir limites associados à informação de Fisher definida em @def-informacao-fisher. Os resultados assintóticos correspondentes não serão desenvolvidos neste capítulo e serão retomados no contexto inferencial do Capítulo 12.
...
No modelo normal multivariado definido em @def-normal-multivariada, o vetor de médias amostrais e a matriz SQPC formam conjuntamente uma estatística suficiente para $\boldsymbol{\mu}$ e $\boldsymbol{\Sigma}$ [@anderson2003].
...
em que $\mathbf T$ é a matriz SQPC observada definida em @def-matriz-sscp-total:
...
A propriedade que define o método é a correspondência entre os momentos populacionais e seus análogos amostrais expressa em @eq-estimador-metodo-momentos. Essa construção, isoladamente, não garante que o estimador obtido seja não viesado, eficiente, suficiente, equivariante ou robusto.

A consistência também não decorre apenas do nome do método. Sob condições como existência dos momentos utilizados, convergência dos momentos amostrais para seus correspondentes populacionais, identificação do parâmetro por esses momentos e continuidade adequada da solução, estimadores de momentos podem ser consistentes.
...
A matriz no numerador é a versão aleatória da matriz total de somas de quadrados e produtos cruzados (SQPC) definida, para dados observados, em @def-matriz-sscp-total. Será denotada por
...
A matriz $\boldsymbol{\mathsf T}$ será desenvolvida em detalhes na seção dedicada à SQPC. Para cada realização da amostra, ela produz a matriz $\mathbf T$ definida em @def-matriz-sscp-total.
...
Considere a função de verossimilhança definida em @def-funcao-verossimilhanca. Um **estimador de máxima verossimilhança** de $\boldsymbol{\theta}$ é uma estatística
...
Considere uma amostra do modelo normal multivariado definido em @def-normal-multivariada:
...
Pela densidade normal multivariada em @eq-densidade-normal-multivariada e pela independência das unidades amostrais, para uma amostra observada a verossimilhança é
...
Considere a matriz SQPC observada $\mathbf T$, definida em @def-matriz-sscp-total:
...
\right).
\end{aligned}
$$ {#eq-log-verossimilhanca-normal-decomposta-2}
...
Essa expressão possui exatamente a forma do critério estudado em @prp-minimizador-criterio-covariancia, com
...
::: {.proof}
A maximização em relação a $\boldsymbol{\mu}$ decorre de @eq-log-verossimilhanca-normal-decomposta-2. Como
...
Para a maximização em relação a $\boldsymbol{\Sigma}$, a log-verossimilhança perfilada em @eq-log-verossimilhanca-perfil-covariancia mostra que é equivalente minimizar
...
Pelo limite de posto da matriz SQPC estabelecido em @eq-posto-sscp-total,
...
Considere a matriz de correlações populacional definida em @def-matriz-correlacoes-populacional:
...
Como correlações são preservadas por mudanças positivas de escala, conforme @eq-correlacao-mudanca-escala, o fator comum que distingue as matrizes de covariâncias com divisores $n$ e $n-1$ se cancela na padronização. Assim, a matriz em @eq-mv-matriz-correlacoes coincide com a matriz de correlações calculada a partir da covariância amostral com divisor $n-1$.
...
Retomando @eq-media-amostral-aleatoria, o vetor de médias amostrais é
...
Pelo método dos momentos, @eq-estimador-momentos-media fornece
...
Sob o modelo normal multivariado, quando o estimador de máxima verossimilhança existe, @eq-mv-media-normal fornece
...
Pela propriedade de centralização estabelecida em @eq-soma-colunas-centralizadas,
...
Como $\bar{\boldsymbol{\mathsf X}}$ é não viesado, a decomposição do erro quadrático médio em @eq-decomposicao-eqm-vetorial reduz-se ao termo de variabilidade. Sob a perda quadrática euclidiana,
...
Como a variância total populacional foi definida em @eq-variancia-total por
...
Aplicando @prp-momentos-combinacoes-lineares ao vetor aleatório $\bar{\boldsymbol{\mathsf X}}$, cuja média e matriz de covariâncias são dadas em @prp-media-amostral-momentos,
...
A Figura @fig-variabilidade-media-amostral apresenta 250 realizações de $\bar{\boldsymbol{\mathsf X}}$ para amostras de tamanhos 5 e 50 geradas de uma mesma distribuição normal bivariada. A normalidade é utilizada apenas para gerar a ilustração; o resultado em @eq-covariancia-media-amostral depende apenas da independência das unidades amostrais e da existência dos segundos momentos.
...
O Capítulo 8 definiu a matriz total de somas de quadrados e produtos cruzados (SQPC) em @def-matriz-sscp-total e a matriz de covariâncias observada em @def-matriz-covariancias-observada. A seção sobre métodos de estimação mostrou ainda que o método dos momentos conduz à matriz SQPC dividida por $n$ e que, sob o modelo normal multivariado e as condições de existência do máximo, a máxima verossimilhança conduz à mesma expressão.
...
Considere a amostra aleatória multivariada definida em @eq-amostra-aleatoria-multivariada, com unidades independentes e
...
Retomando a matriz de centralização utilizada em @eq-centralizacao-por-hn,
...
A matriz $\mathbf T$ é exatamente a matriz SQPC observada definida em @def-matriz-sscp-total e escrita matricialmente em @eq-definicao-sscp-total. A diferença de notação é probabilística: $\boldsymbol{\mathsf T}$ representa a estatística aleatória, enquanto $\mathbf T$ representa sua realização observada.
...
Após a observação, obtém-se a matriz de covariâncias observada definida em @def-matriz-covariancias-observada:
...
ela é uma matriz de Gram, conforme @def-matriz-gram, e suas propriedades de simetria e semidefinição positiva decorrem de @prp-propriedades-matriz-gram. A multiplicação por $1/(n-1)>0$ preserva essas propriedades em $\mathbf S$.
...
utilizando o operador definido em @def-operador-vec. De modo correspondente, o erro quadrático médio matricial sob a perda baseada na norma de Frobenius definida em @def-norma-frobenius envolveria
...
Pelo método dos momentos, @eq-estimador-momentos-covariancia-sscp fornece
...
Sob o modelo normal multivariado não singular e quando o máximo existe, @eq-mv-covariancia-normal fornece a mesma estatística:
...
As propriedades algébricas da matriz SQPC observada decorrem de sua representação de Gram em @eq-definicao-sscp-total e do limite de posto estabelecido em @eq-posto-sscp-total. Como essas propriedades valem para cada realização da amostra, elas também caracterizam as estatísticas $\boldsymbol{\mathsf T}$ e $\boldsymbol{\mathsf S}$ amostra a amostra.
...
A translação $\mathbf d$ desaparece pela centralização. Como a matriz populacional satisfaz, por @prp-transformacao-afim-covariancia,
...
**Simetria e semidefinição positiva.** Pela representação de Gram em @eq-definicao-sscp-total e pelas propriedades de matrizes de Gram em @prp-propriedades-matriz-gram, para qualquer realização da amostra,
...
**Posto.** Pelo resultado estabelecido em @eq-posto-sscp-total, para qualquer realização da amostra,
...
A distância de Mahalanobis foi definida em @def-distancia-mahalanobis utilizando os parâmetros populacionais. Sob normalidade, @prp-mahalanobis-qui-quadrado estabelece a distribuição qui-quadrado da forma correspondente quando $\boldsymbol{\mu}$ e $\boldsymbol{\Sigma}$ são conhecidos.
...
O Capítulo 8 definiu a matriz de covariâncias cruzadas populacional em @def-covariancia-cruzada, a matriz de covariâncias cruzadas observada em @def-covariancia-cruzada-observada e a matriz de correlações observada em @def-matriz-correlacoes-observada. Nesta seção, os objetos correspondentes são tratados como estatísticas aleatórias e utilizados na estimação de suas contrapartes populacionais.
...
foi definida em @def-covariancia-cruzada. Sua orientação satisfaz, conforme @eq-transposta-covariancia-cruzada,
...
Após a observação da amostra, obtém-se a matriz de covariâncias cruzadas observada definida em @def-covariancia-cruzada-observada:
...
Pelo não viesamento da matriz de covariâncias amostral estabelecido em @eq-nao-viesamento-covariancia-amostral,
...
A consistência também pode ser obtida diretamente a partir da matriz de covariâncias do vetor conjunto. Sob as condições de @prp-consistencia-covariancia-amostral, aplicadas ao vetor de dimensão $p+q$,
...
\boldsymbol{\Sigma}_{XY}.
$$
A transformação da covariância cruzada amostral acompanha a transformação do parâmetro populacional estabelecida em @eq-transformacao-covariancia-cruzada. Se
...
Após a observação da amostra, obtém-se a matriz $\mathbf R$ definida em @def-matriz-correlacoes-observada:
...
então, no regime de dimensão fixa, a continuidade da transformação
...
e o teorema do mapeamento contínuo fornecem
...
Portanto, a correlação amostral pode ser viesada em amostras finitas e, ainda assim, ser consistente.

### Transformações de localização e escala

O efeito populacional das mudanças de localização e escala sobre a correlação foi estabelecido em @eq-correlacao-mudanca-escala. Para uma realização observada, considere
...
O limite de posto estabelecido em @eq-posto-sscp-amostral,
...
Sob as condições de @prp-consistencia-covariancia-amostral,
...
Considere, por exemplo, a contraparte amostral da forma quadrática utilizada na distância de Mahalanobis definida em @def-distancia-mahalanobis:
...
A pseudoinversa de Moore--Penrose foi definida em @def-pseudoinversa-svd. Para uma matriz de covariâncias amostral com decomposição espectral reduzida
...
A matriz $\mathbf S_X$ é a realização observada do estimador não viesado $\boldsymbol{\mathsf S}_X$ de $\boldsymbol{\Sigma}_{XX}$, cujo não viesamento foi estabelecido em @eq-nao-viesamento-covariancia-amostral.

Sob o modelo normal multivariado definido em @def-normal-multivariada, nesta amostra
...
Portanto, a condição de posto completo de @prp-estimadores-normais-mv é satisfeita. Assim, a estimativa de máxima verossimilhança da matriz de covariâncias é
...
Logo, pela definição de covariância cruzada observada em @def-covariancia-cruzada-observada,
...
em que $\boldsymbol{\mathsf T}$ é a matriz SQPC amostral definida em @def-sscp-amostral-aleatoria.
...
Os resultados em @prp-media-amostral-momentos e @eq-nao-viesamento-covariancia-amostral fornecem
