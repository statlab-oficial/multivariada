...
A avaliação de $\widehat{\boldsymbol{\theta}}$ depende de sua distribuição amostral. Viés, variabilidade e erro quadrático médio descrevem seu comportamento para um tamanho amostral fixo. Consistência e eficiência assintótica descrevem seu comportamento quando o tamanho da amostra aumenta. Suficiência, equivariância e robustez examinam outros aspectos da informação utilizada e da estabilidade do procedimento.
...
A comparação entre estimadores exige a especificação do que se considera um erro relevante. Essa escolha é formalizada por uma função de perda.
...
A consistência descreve o comportamento de uma sequência de estimadores quando o tamanho da amostra aumenta.
...
e $g$ é contínua em $\boldsymbol{\theta}$, então
...
Uma estatística suficiente concentra toda a informação da amostra sobre o parâmetro, no sentido de que a distribuição condicional da amostra, dado o valor da estatística, não depende do parâmetro.
...
O Teorema da Fatoração de Neyman–Fisher fornece um critério para verificar essa propriedade.
...
No modelo normal multivariado, o vetor de médias amostrais e a matriz de somas de quadrados e produtos cruzados formam uma estatística suficiente.
...
Um estimador pode ser construído a partir de diferentes princípios. O método dos momentos procura reproduzir, na amostra, características da distribuição populacional. A máxima verossimilhança escolhe os valores dos parâmetros que tornam os dados observados mais compatíveis com o modelo probabilístico especificado.

Os dois métodos podem produzir o mesmo estimador em determinados modelos. Essa coincidência não ocorre em geral. Também não garante, por si só, ausência de viés, consistência ou eficiência, pois essas propriedades precisam ser analisadas separadamente.
...
As propriedades de primeira e segunda ordens do vetor de médias amostrais podem ser obtidas diretamente da independência entre as unidades amostrais.
...
A utilização da média amostral consome uma direção do espaço das observações. Essa perda aparece no valor esperado da SSCP.
...
Os divisores $n$ e $n-1$ correspondem a critérios diferentes:

- o divisor $n$ decorre do método dos momentos e da maximização da verossimilhança normal;
- o divisor $n-1$ corrige o valor esperado e produz um estimador não viesado.

A propriedade de máxima verossimilhança depende da especificação normal. A ausência de viés de $\boldsymbol{\mathsf S}$ exige somente independência, distribuição idêntica e segundos momentos finitos.
...
A regularização modifica o estimador amostral para reduzir sua variabilidade ou melhorar seu condicionamento. Essa modificação geralmente introduz viés e ilustra o compromisso apresentado em @eq-decomposicao-eqm-matricial.

Uma forma de regularização combina a matriz de covariâncias amostral com uma matriz-alvo de estrutura mais simples.
...
O vetor de médias e a matriz de covariâncias amostral utilizam diretamente os valores e os produtos dos desvios. Por isso, observações extremas podem exercer grande influência sobre ambos.
...
Estimadores robustos de localização e dispersão procuram reduzir alterações excessivas diante de determinadas perturbações. Sua avaliação pode considerar:
