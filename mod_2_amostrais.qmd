...
Considere uma amostra aleatória $\boldsymbol{\mathcal X}$ cuja distribuição depende de um parâmetro populacional $\boldsymbol{\theta}$ e uma estatística
...
A função $G$ reduz a amostra à estatística $\boldsymbol{\mathsf G}$. A suficiência estabelece que, após conhecida essa estatística, a parte restante da variabilidade da amostra não contém dependência adicional em relação a $\boldsymbol{\theta}$.
...
em que $q_{\boldsymbol{\theta}}$ contém a dependência em relação ao parâmetro por meio da estatística $G(\mathbf x)$, enquanto $r(\mathbf x)$ não depende de $\boldsymbol{\theta}$. [@casella2002inference]
:::
...
Se uma transformação $h$ da amostra induz a transformação
...
do parâmetro, o estimador é **equivariante** em relação a $h$ e $g$ quando
...
O **estimador pelo método dos momentos** de
...
reúne os momentos populacionais escolhidos, com
...
reúne seus correspondentes amostrais, supondo que as esperanças utilizadas existam.
:::

A existência e a unicidade do estimador dependem dos momentos escolhidos e da forma da função $\mathbf m$. Assim, o método substitui os momentos populacionais, que dependem do parâmetro desconhecido, pelos correspondentes momentos calculados a partir da amostra e resolve as equações resultantes em relação a $\boldsymbol{\theta}$. 
...
No evento amostral em que o máximo da verossimilhança existe, os estimadores no modelo normal multivariado são
