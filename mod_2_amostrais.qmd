...
para todo $i$ e, temos, para unidades distintas,
...
Uma estatística suficiente resume a amostra preservando, segundo o modelo adotado, a informação relevante para o parâmetro de interesse. A ideia é que, depois de conhecida essa estatística, os detalhes restantes da amostra não fornecem informação adicional sobre o parâmetro [@casella2002inference].
...
O vetor de médias populacional $\boldsymbol{\mu}$, definido em @eq-vetor-medias-populacional, descreve a localização da população em $\mathbb R^p$. Sua estimação será baseada no centro das observações que compõem a amostra.
...
Por @eq-media-transformacao-afim,
...
A Figura @fig-variabilidade-media-amostral apresenta os vetores de médias obtidos em 250 amostras de tamanhos 5 e 50 geradas de uma mesma distribuição normal bivariada. A normalidade é utilizada apenas para gerar a ilustração; o resultado em @eq-covariancia-media-amostral depende apenas da independência das unidades amostrais e da existência dos segundos momentos.
...
O Capítulo 8 definiu a matriz total de somas de quadrados e produtos cruzados (SQPC) em @def-matriz-sscp-total e a matriz de covariâncias observada em @def-matriz-covariancias-observada. Nesta seção, os objetos correspondentes passam a ser tratados como **estatísticas aleatórias** e utilizados na estimação da matriz de covariâncias populacional $\boldsymbol{\Sigma}$.

Considere a amostra aleatória multivariada definida em @eq-amostra-aleatoria-multivariada, com
...
Sob o modelo normal multivariado não singular, quando o máximo existe, @eq-mv-covariancia-normal fornece
...
O resultado amostral reproduz a transformação populacional estabelecida em @prp-transformacao-afim-covariancia:
...
A covariância cruzada amostral possui a contraparte amostral da transformação populacional estabelecida em @eq-transformacao-covariancia-cruzada. Se
...
O efeito populacional das mudanças de localização e escala sobre a correlação foi estabelecido em @eq-correlacao-mudanca-escala. Para a contraparte amostral, considere
...
O vetor de médias e a matriz de covariâncias amostrais podem apresentar sensibilidade não limitada a observações suficientemente afastadas da maior parte dos dados [@maronna2019robust].
...
Para o primeiro bloco, a matriz SQPC observada, conforme @def-matriz-sscp-total, é
...
Sob o modelo normal multivariado definido em @def-normal-multivariada, e como nesta amostra
...
a condição de posto completo necessária ao resultado em @eq-mv-covariancia-normal é satisfeita. Assim, a estimativa de máxima verossimilhança da matriz de covariâncias é
...
A partir de $\mathbf S_X$, a matriz de correlações observada definida em @def-matriz-correlacoes-observada é
...
Como existem seis observações e cinco variáveis, o limite de posto em @eq-posto-sscp-amostral fornece
...
1. $\bar{\mathbf x}$ e $\bar{\mathbf y}$ são estimativas dos respectivos vetores de médias populacionais;
2. $\mathbf T_X$ é a realização observada da matriz SQPC aleatória $\boldsymbol{\mathsf T}_X$;
3. $\mathbf S_X$ é a realização observada de um estimador não viesado de $\boldsymbol{\Sigma}_{XX}$;
4. sob o modelo normal e as condições de existência do máximo, $\mathbf T_X/n$ fornece a estimativa de máxima verossimilhança de $\boldsymbol{\Sigma}_{XX}$;
5. $\mathbf R_X$ padroniza as covariâncias pelas variabilidades marginais;
6. posto completo não implica necessariamente bom condicionamento;
7. $\mathbf T_{XY}$ e $\mathbf S_{XY}$ dependem do pareamento entre as unidades dos dois blocos;
8. $\mathbf S_{\mathrm{conj}}$ reúne as covariâncias internas e cruzadas em uma única estrutura.
...
Sob as condições de @prp-media-amostral-momentos e @eq-nao-viesamento-covariancia-amostral,
...
Sob o modelo normal multivariado, quando o máximo existe, @eq-mv-media-normal e @eq-mv-covariancia-normal fornecem
