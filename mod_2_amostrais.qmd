...
A notação $\mathrm{iid}$ reúne duas condições. A distribuição idêntica estabelece que todos os vetores são provenientes da mesma população. A independência estabelece que o resultado observado em uma unidade não altera a distribuição dos resultados das demais unidades.
...
A normalidade não é necessária para definir médias, covariâncias ou correlações amostrais. Neste capítulo, a máxima verossimilhança será aplicada especificamente ao modelo normal multivariado; por isso, as expressões dos estimadores de máxima verossimilhança deduzidas adiante dependerão dessa especificação. A normalidade também será utilizada nos resultados exatos desenvolvidos no Capítulo 12.
...
Após a observação da amostra, uma realização da matriz aleatória $\boldsymbol{\mathcal X}$ será denotada por
...
A matriz $\mathbf X$ é, portanto, a matriz de dados observada correspondente a uma realização de $\boldsymbol{\mathcal X}$.
...
Como cada vetor observado assume valores em $\mathbb R^p$, o espaço ambiente das possíveis amostras pode ser representado por
...
ou, de forma equivalente, pelo conjunto das matrizes reais de dimensão $n\times p$. Se o suporte da distribuição estiver restrito a um subconjunto $\mathcal X\subseteq\mathbb R^p$, o espaço das possíveis amostras será, de forma mais precisa, $\mathcal X^n$.
...
Os elementos fora da diagonal são as covariâncias entre os componentes do estimador. Como a subtração de constantes não altera covariâncias, elas também correspondem às covariâncias entre os respectivos erros de estimação:
...
e um vetor de funções de momento
...
cujas componentes representam as funções da variável aleatória escolhidas para construir os momentos. Sua esperança depende de $\boldsymbol{\theta}$:
...
independente e identicamente distribuída, em que cada vetor possui densidade ou função de probabilidade
...
Sob independência, a densidade ou função de probabilidade conjunta da amostra é o produto das contribuições individuais.
...
A máxima verossimilhança possui uma propriedade de invariância que permite estimar funções do parâmetro sem realizar uma nova maximização desde o início [@casella2002inference].
...
O tamanho amostral reduz pelo mesmo fator as variâncias das médias individuais e as covariâncias entre seus erros de estimação. A estrutura de covariâncias e as direções principais de dispersão permanecem determinadas por $\boldsymbol{\Sigma}$.
...
Aplicando a mesma combinação linear ao vetor de médias amostrais, obtém-se o estimador
...
No modelo normal, esse resultado também é compatível com a propriedade de invariância da máxima verossimilhança.
...
Para ilustrar a variabilidade entre amostras, a Figura @fig-variabilidade-media-amostral apresenta os vetores de médias obtidos em amostras de tamanhos 5 e 50 geradas de uma mesma distribuição normal bivariada. O símbolo em forma de cruz indica o vetor de médias populacional.
...
A orientação e a estrutura de covariâncias da dispersão entre as médias amostrais permanecem determinadas por $\boldsymbol{\Sigma}$, enquanto sua magnitude diminui com o crescimento de $n$.
...
::: {#prp-decomposicao-centro-arbitrario}
## Decomposição em torno de um centro arbitrário
...
::: {#prp-nao-vies-covariancia-amostral}
## Ausência de viés da matriz de covariâncias amostral

Para uma amostra aleatória i.i.d. de uma população com segundos momentos finitos,
...
Sob o modelo normal multivariado não singular, se
...
então $\boldsymbol{\mathsf A}$ possui posto $p$ com probabilidade 1. Nesse caso, o estimador de máxima verossimilhança de $\boldsymbol{\Sigma}$ existe, com probabilidade 1, no espaço das matrizes positivas definidas e é
...
Os divisores $n$ e $n-1$ correspondem a propriedades distintas:

-   o divisor $n$ surge na construção pelo método dos momentos e, sob o modelo normal multivariado, também na estimação por máxima verossimilhança;
...
com dimensão fixa $p$ e segundos momentos finitos. Então,
...
Além disso, a matriz com divisor $n$,
...
\xrightarrow{P}
\boldsymbol{\Sigma}.
$$ {#eq-consistencia-covariancia-divisor-n}

Sob o modelo normal multivariado não singular, quando o estimador de máxima verossimilhança existe,
...
de modo que o estimador de máxima verossimilhança também é consistente.
:::

::: proof
A matriz com divisor $n$ pode ser escrita como
...
Os segundos momentos finitos garantem a integrabilidade dos produtos $X_jX_k$. Além disso,
...
Sob o modelo normal multivariado não singular, para $n\geq p+1$, a SSCP possui posto $p$ com probabilidade 1 e
...
A translação $\mathbf d$ não altera a matriz de covariâncias. A transformação linear $\mathbf C$ atua sobre o estimador da mesma forma que atua sobre a matriz de covariâncias populacional:
...
::: {#prp-posto-covariancia-amostral}
## Posto da matriz de covariâncias amostral

Para uma amostra com $n\geq2$ observações e $p$ variáveis,
...
Uma condição necessária para a definitude positiva é
...
Mesmo quando $n\geq p+1$, a matriz pode ser singular se existir dependência linear exata entre as variáveis centralizadas. Assim, a relação entre $n$ e $p$ fornece uma condição necessária, mas a definitude positiva depende também da configuração observada.

A singularidade da matriz de covariâncias observada $\mathbf S$ impede o cálculo de sua inversa ordinária e afeta procedimentos que utilizam uma matriz de precisão estimada, distâncias de Mahalanobis amostrais ou problemas de autovalores generalizados. A pseudoinversa atua nas direções associadas aos autovalores positivos de $\mathbf S$, mas não produz o mesmo objeto que uma estimativa regularizada positiva definida.
...
As covariâncias cruzadas descrevem a associação linear entre os blocos em suas unidades originais. As correlações padronizam essas relações e permitem comparar variáveis medidas em escalas diferentes.
...
Os blocos diagonais descrevem a estrutura de covariâncias interna de cada conjunto. Os blocos fora da diagonal descrevem as associações lineares entre variáveis pertencentes aos dois conjuntos.
...
a matriz SSCP do vetor conjunto possui posto $p+q$ com probabilidade 1. Nesse caso, o estimador de máxima verossimilhança positivo definido de $\boldsymbol{\Sigma}_Z$ existe com probabilidade 1, e seu bloco cruzado é
...
Assim, $\boldsymbol{\mathsf S}_{XY}$ contém as covariâncias amostrais entre as variáveis dos dois blocos e permite obter a covariância amostral entre quaisquer combinações lineares desses blocos.
...
Os valores extremos $r_{jk}=\pm1$ correspondem a uma relação linear exata entre as duas colunas centralizadas e padronizadas.
...
A condição de definitude positiva pode ser expressa como
...
-   independência linear das colunas da matriz de dados centralizados;
...
é necessária, mas não garante isoladamente a definitude positiva. Quando
...
Sob o modelo normal multivariado, quando o estimador de máxima verossimilhança de $\boldsymbol{\Sigma}$ existe, o estimador de máxima verossimilhança da matriz de correlações populacional $\boldsymbol{\mathcal R}$ coincide com a matriz de correlações amostral $\boldsymbol{\mathsf R}$. De fato,
...
Quando $\boldsymbol{\mathsf S}$ é consistente para $\boldsymbol{\Sigma}$ e todas as variâncias populacionais são positivas,
...
a probabilidade de todas as variâncias amostrais serem positivas tende a 1. Como a transformação que leva uma matriz de covariâncias com diagonal positiva à matriz de correlações correspondente é contínua, o teorema do mapeamento contínuo fornece
...
Esta seção amplia a estimação clássica desenvolvida anteriormente e examina situações em que a dimensão, o condicionamento ou a sensibilidade das estatísticas amostrais exigem cuidados adicionais. Sua leitura pode ser realizada após o núcleo do capítulo, sem comprometer a sequência conceitual das seções anteriores.
:::

Esta seção amplia a estimação clássica desenvolvida anteriormente e examina situações em que a dimensão, o condicionamento ou a sensibilidade das estatísticas amostrais exigem cuidados adicionais.
...
Em um regime de dimensão crescente, pode-se escrever $p=p_n$ e considerar, como situação de referência,
...
Nesse regime, a dimensão cresce juntamente com o tamanho amostral. A matriz de covariâncias amostral pode possuir posto completo e, ainda assim, apresentar elevada variabilidade em suas propriedades espectrais.

Quando a dimensão cresce, o controle elemento a elemento dos erros de estimação não implica, por si só, controle da matriz em norma espectral. Consequentemente, esse tipo de convergência não garante isoladamente que:
...
Uma matriz com número de condição elevado é denominada mal condicionada. Nesse caso, a inversão e a solução de sistemas lineares envolvendo $\mathbf S$ podem ser altamente sensíveis a pequenas perturbações.

O determinante também pode tornar-se numericamente sensível quando o espectro contém autovalores muito pequenos, pois
...
A estabilidade dos autovetores envolve uma questão adicional: ela depende da separação entre os autovalores correspondentes. Quando dois ou mais autovalores estão muito próximos, pequenas perturbações podem produzir grandes alterações nos autovetores individuais, mesmo que a matriz permaneça invertível.
...
Considere uma matriz-alvo simétrica e semidefinida positiva,
...
Nesse caso, a aleatoriedade de $\widehat{\boldsymbol{\Gamma}}$ e de $\widehat{\lambda}$ também deve ser considerada na análise do viés, da variabilidade e do risco do estimador. Em geral, suas propriedades incondicionais não podem ser obtidas tratando essas quantidades como constantes.

Os resultados algébricos sobre semidefinitude ou definitude positiva continuam válidos amostra a amostra, desde que o alvo estimado e o peso de contração satisfaçam as condições correspondentes.
...
-   a matriz diagonal formada pelas variâncias amostrais, que preserva essas variâncias e reduz as covariâncias;
...
Para $0<\lambda\leq1$ e $\tau>0$, a contração esférica reduz algebricamente as diferenças entre os autovalores e afasta o menor autovalor de zero, melhorando o condicionamento da matriz. Isso não implica, por si só, que o alvo ou o valor de $\lambda$ sejam estatisticamente adequados.
...
No regime $p<n$, a matriz amostral simulada possui posto completo. A regularização desloca seus autovalores na direção do alvo e reduz a dispersão do espectro.
...
Para comparar esse comportamento com a regularização, considere uma decomposição espectral completa,
...
Nas direções pertencentes ao núcleo de $\mathbf S$, para as quais

$$
\ell_j=0,
$$

a inversa regularizada atribui o peso
...
-   a pseudoinversa atua como a inversa ordinária na imagem de $\mathbf S$ e anula as direções pertencentes ao núcleo;
-   para $\ell_j>0$, a pseudoinversa utiliza o peso $1/\ell_j$, enquanto, para $\ell_j=0$, utiliza peso zero;
-   a regularização modifica os autovalores antes da inversão e, no caso esférico com $\lambda>0$ e $\tau>0$, atribui variância estimada positiva também às direções que pertenciam ao núcleo amostral;
-   a pseudoinversa não corrige o mau condicionamento causado por autovalores positivos muito pequenos, pois seus recíprocos permanecem elevados;
-   a regularização pode limitar essa amplificação ao afastar os autovalores regularizados de zero.
...
Essa matriz é a estimativa observada de $\boldsymbol{\Sigma}_{XX}$ obtida a partir de $\mathbf S_X$. Sob amostragem aleatória i.i.d. e existência de segundos momentos, o estimador correspondente $\boldsymbol{\mathsf S}_X$ é não viesado para $\boldsymbol{\Sigma}_{XX}$.

Se for adotado o modelo normal multivariado, a estimativa de máxima verossimilhança de $\boldsymbol{\Sigma}_{XX}$ é
...
A matriz é invertível, mas o menor autovalor é muito inferior ao maior. Assim, a matriz de covariâncias amostral apresenta uma direção de variabilidade relativamente reduzida, e a inversão de $\mathbf S_X$ pode ser sensível a pequenas perturbações nos dados.
...
3.  os divisores $n-1$ e $n$ correspondem a estimadores distintos da matriz de covariâncias, sendo o primeiro não viesado sob amostragem i.i.d. com segundos momentos finitos e o segundo, sob o modelo normal multivariado, associado à máxima verossimilhança;
...
A consistência descreve o comportamento quando o tamanho amostral aumenta. A eficiência depende de uma ordem matricial ou de uma função de perda. A suficiência trata da informação preservada por uma estatística. A equivariância examina a coerência do estimador sob transformações dos dados, enquanto a robustez avalia sua sensibilidade a perturbações, contaminações e afastamentos das condições de referência.

O método dos momentos e a máxima verossimilhança fornecem princípios gerais de estimação. No modelo normal multivariado não singular,
...
a matriz SSCP possui posto $p$ com probabilidade 1, e o estimador de máxima verossimilhança de $\boldsymbol{\Sigma}$ existe, com probabilidade 1, no espaço das matrizes positivas definidas, sendo
...
é um estimador não viesado de $\boldsymbol{\Sigma}$. Para $p$ fixo e sob existência de segundos momentos,
...
Sob o modelo normal multivariado, quando existe, o estimador de máxima verossimilhança de $\boldsymbol{\Sigma}$ coincide com $\boldsymbol{\mathsf A}/n$.
...
A regularização modifica o estimador para reduzir variabilidade ou melhorar o condicionamento. Um estimador por contração pode ser escrito como
...
em que $\boldsymbol{\Gamma}$ é uma matriz-alvo simétrica e semidefinida positiva. Se $\boldsymbol{\Gamma}\succ0$ e $\lambda>0$, o estimador resultante é positivo definido.

A regularização deve ser distinguida da pseudoinversa. A pseudoinversa atua como a inversa ordinária nas direções associadas aos autovalores positivos e atribui peso zero às direções pertencentes ao núcleo. A regularização, por sua vez, modifica o espectro e pode produzir uma matriz positiva definida. Nenhuma dessas substituições preserva automaticamente as distribuições inferenciais derivadas para a inversa clássica.
...
-   determinar as condições de posto e definitude positiva;
